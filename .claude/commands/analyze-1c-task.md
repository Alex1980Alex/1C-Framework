...
8. **Источники исследования (Infostart / GitHub / RAG)** (ОБЯЗАТЕЛЬНО, Правило 5): фиксировать данные внешнего поиска (вкл. отрицательные). Infostart — **сначала агент `scripts/onec_search.py`** (ADR-040), WebSearch только fallback; GitHub — `scripts/ecosystem_scan.py`; RAG — `pdf-vector-graph`
