...
2. **Параметры (опционально):**
   ...
   - `--retries <N>` — макс. попыток на секцию при транзиентных ошибках (по умолчанию 3)
   - `--no-retry` — отключить auto-retry (эквивалент `--retries 1`)
   - `--fix-mode <MODE>` — режим auto-fix: `suggest` (default), `auto`, `off`
   - `--verbose` — подробный вывод pre-check и post-check (полная таблица — в §«Параметры команды»)
