...
(например, `НомерДокументаПоставщика = 'ARM-TM1-001'`).

**Post-check extraction** (после успешного прогона секции):

```python
# Найти созданные объекты по маркеру
mcp__1c-mcp-crud__execute_query(
    query="""ВЫБРАТЬ
        Ссылка.УникальныйИдентификатор() КАК UUID,
        Номер, Дата, Проведен
    ИЗ Документ.<Тип>
    ГДЕ НомерДокументаПоставщика ПОДОБНО &Marker""",
    parameters={"Marker": "ARM-TM1-%"}
)
```
...
```powershell
powershell -File tools\vanessa\run-bdd.ps1 -Feature "<slug>/<section>.feature" -TimeoutSec <N>
```
