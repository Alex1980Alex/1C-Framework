...
class AutoGitSave(BaseHook):
    ...
    def execute(self, inp: HookInput) -> HookOutput | None:
        ...
        msg = (
            ...
            "ОБЯЗАТЕЛЬНАЯ ЗАДАЧА: закоммитить изменения перед завершением работы.\n"
            "ДЕЙСТВИЕ: Немедленно добавь задачу 'Закоммитить незакоммиченные изменения' "
            "в свой TodoWrite список (status: pending). "
            "Это гарантирует что задача видна в твоём рабочем списке "
            "и не потеряется при компакции контекста.\n"
            f"Файлы: {files_list}\n"
            "После завершения текущей работы — проверь git status, "
            "сгруппируй по логике, закоммить."
        )
