...
_METHOD_HEADER = re.compile(r"^\s*(Процедура|Функция)\b", re.IGNORECASE)
_METHOD_END = re.compile(r"^\s*(КонецПроцедуры|КонецФункции)\b", re.IGNORECASE)
_EXPORT = re.compile(r"\bЭкспорт\b", re.IGNORECASE)

# Экспорт стоит в сигнатуре: на строке заголовка либо на строках многострочной сигнатуры
# (до тела). Реальные сигнатуры ≤ этого числа строк; больше — тело, где Экспорт не бывает.
...
def exported_method_spans(text: str) -> list[tuple[int, int]]:
    ...
    Метод экспортный, если `Экспорт` встречается на строке заголовка ИЛИ в пределах
    первых `_SIGNATURE_MAX_LINES` строк метода (многострочная сигнатура). Тело метода
    Экспорт не содержит, поэтому окно сигнатуры отсекает ложные срабатывания."""
    ...
    while i < n:
        if _METHOD_HEADER.match(lines[i]):
            start = i + 1  # 1-based
            j = i + 1
            exported = bool(_EXPORT.search(lines[i]))
            while j < n and not _METHOD_END.match(lines[j]):
                if not exported and (j - i) < _SIGNATURE_MAX_LINES and _EXPORT.search(lines[j]):
                    exported = True
                j += 1
...
def edits_exported_method(
    ...
    *,
    ...
    инъекция зависимостей — для юнит-тестов без git. Fail-safe: любая ошибка чтения/дифа
    → пропуск файла (не роняем; хуже false-negative applicability, чем краш гейта).
    `line_ranges is None` (untracked/дифф не получен) + есть экспорт → applicable (fail-closed
    в сторону «применимо», т.к. impact-analysis тут точно уместен)."""
