...
def _is_config_bsl(rel: str) -> bool:
    ...
    `MFM/Конфигурация` (УправлениеМатериальнымиПотоками) — В скоупе, свой проект `utp-mfm`
    (ADR-048 A7 вар.а: сканировать, слепых зон нет) → детектится наравне с транспортными.

    Исключены (НЕ продакшн 1С-конфиги в Sonar-скоупе → иначе detected-but-not-scanned
    dead-block): `configuration/` (ведение задач), `external/` (внешние зависимости, вкл.
    BSL-расширение MCP), `tools/` (dev-тулинг + тест-фикстуры `.bsl`). Точная привязка
    детекта к реестру `sonar_projects.roots()` — P3.B (проводка реестра в детект)."""
    r = rel.replace("\\", "/").lower()
    if r.startswith(("configuration/", "external/", "tools/")):
        return False
