...
> **W3 A2 (2026-07-10, roadmap [260708](../../../docs/roadmap/260708_ROADMAP_AUTONOMOUS_1C_DEBUGGING.md) §7.6):** +1 tool — `debug_root_cause(exception_message?, exception_module?, phase, timeout_sec, max_frames, context_radius)`: автономный root-cause на необработанном исключении → structured diagnosis-record (`fault_location` / `runtime_symptom {code,message}` / `propagation_path` [весь стек outermost-first + resolved_source] / `frames_inspected` [top-`max_frames` innermost с локалями+source]). Two-phase (arm=опц.фильтр исключений+silent break-on-next / collect=ждёт rteProcessing event-driven → record → Continue в finally). Контракт `{diagnosis, raw}`. **Bounded top-N** (halt-окно 1-2с), усечение видно в `frames_bounded`/`frames_total`. Плюс root-fix `exception_bps` (message-фильтр теперь матчит живое поле `info`). W2 B3 (2026-07-10): `debug_autotrace` collect ждёт `asyncio.Event`, не poll-loop (trap-latency ↓).
>
