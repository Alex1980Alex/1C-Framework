---
name: 1c-debug-hmr
description: "1c-debug-hmr — MCP debug-сервер для 1С:Предприятие с hot-module-reload + persistent session (RDBG, 42 tools; полный список в теле скилла): breakpoint_locations, set_function_breakpoint, hypothesis (W4′), set_variable (C1 — правка переменной в halt'е), autotrace, inspect_frame, root_cause, trace_variable (W1/W3 — BP-верификация, корень и value-timeline одним вызовом), connect/ping, set_breakpoint, set_logpoint, stack_trace, variables, evaluate, step, arm_warm_rphosts. ИСПОЛЬЗУЙ при отладке BSL-кода (BP, stack, locals, evaluate, step) и при разработке самого debug-wrapper'а — правки подхватываются без /mcp reconnect, сессия RDBG переживает HMR-restart. Триггеры: 'отладка 1С', 'breakpoint BSL', 'callStack 1С', 'debug rphost', 'debug_set_breakpoint', 'debug_stack_trace', 'debug_variables', 'debug_evaluate', 'logpoint', 'tracepoint', 'warm pool', 'cascade halt'. НЕ для написания BSL-кода (→ bsl-development), НЕ для запросов к БД (→ 1c-mcp-crud), НЕ для VA BDD UI-тестов (→ va-bdd-testing)."
---
...
> **W4′ строки-и-гипотезы (2026-07-11, roadmap 260708 §8, code-verify PASS ×3):** +3 tools —
> - `debug_breakpoint_locations(object_id, from_line, to_line, module_type, property_id)` (**C3**): какие строки модуля реально BP-settable (RDBG молча игнорит BP на комментах/`КонецЕсли`/`Перем`/продолжении строкового литерала `|`). Классификатор `bsl_locals.classify_lines` (9 классов; header/structural=uncertain), advisory. `debug_set_breakpoint` теперь отдаёт `location: {class, executable, nearest_executable}`. Live-совпал с B1-эмпирикой.
> - `debug_set_function_breakpoint(method_fqn, condition, hit_condition)` (**C2**): BP по ИМЕНИ метода (`ОбщийМодуль.X.Метод` / `Документ.Y.МодульОбъекта.Обработка`), устойчиво к src↔deployed сдвигу (имя не сдвигается). Резолв `uuid_index.find_by_fqn` (обратная карта fqn→object_id) → первая executable строка → делегирует `debug_set_breakpoint`. ⚠ пре-существующий баг resolve() для регистров (RecordSetModule→ManagerModule.bsl).
> - `debug_hypothesis(assertions[{object_id,line,expr,expected?,label?}], phase, timeout_sec)` (**A5**): батч runtime-гипотез за ОДИН прогон - logpoint per assertion → verdict per-assertion PASS/FAIL/NO_HIT/INCONCLUSIVE/HIT. Two-phase arm/collect (как A3). SECURITY: expr исполняется как BSL в rphost.
...
> - `debug_trace_variable(name, object_id, method?, phase, max_lines, upstream)` (**A3**): InspectCoder trace-variable — авто-logpoint'ы на строки присваивания `name` (+ upstream RHS-входы) → прогон → таймлайн значений `[{line, value, ts}]`. Two-phase (arm статически находит `name = ...` в области `method` → per-строка logpoint `name={name} u={u}` / collect читает logpoint-JSONL этого трейса, снимает свои logpoint'ы).
>
> **B4** (не tool — reliability): `_ping_loop` heartbeat детектит устойчивую потерю dbgs (3 подряд ping-исключения) → bounded auto-reconnect (cooldown 30с); состояние в `debug_target_state.recovery`. **B3**: `debug_autotrace` collect ждёт `asyncio.Event`, не poll-loop. Root-fix `exception_bps` (message-фильтр матчит живое `info`).
>
> **W1 Autonomy (2026-07-08, roadmap 260708 §7, live-verified PASS):** +4 tools — `debug_inspect_frame` (A0: frame-bundle = стек+resolved_source+локали+исходник±radius в 1 вызов), `debug_autotrace` (A1: two-phase `arm`→триггер→`collect`, контракт `{verdict, raw}`, Continue в finally; сворачивает Шаблоны 5/5a/6), `debug_collection_info`/`debug_collection_page` (C0: paging больших ТаблицаЗначений/Массив, cap 200). Плюс B2: `set_breakpoint`/`set_logpoint` авто-применяют offset из `calibrate_result` (`applied_offset` в ответе, persist в `.active.json`). ⚠ Ф-2: callStack-массив outermost-first (BP-фрейм ПОСЛЕДНИЙ), eval-`stackLevel` innermost-first (0 = фрейм BP) — bundle/locals индексируют правильно с 89fbc5d.
...
| `debug_set_function_breakpoint(method_fqn, condition?, hit_condition?)` | **(C2, 2026-07-11)** BP по ИМЕНИ метода (`ОбщийМодуль.X.Метод`), устойчив к src↔deployed сдвигу. Резолв fqn→object_id (`uuid_index.find_by_fqn`) → первая executable → делегирует `debug_set_breakpoint` |
| `debug_breakpoint_locations(object_id, from_line, to_line, module_type?, property_id?)` | **(C3, 2026-07-11)** какие строки BP-settable (не коммент/`КонецЕсли`/`Перем`/`\|`-строка). Advisory-эвристика `bsl_locals.classify_lines`; `debug_set_breakpoint` отдаёт `location.nearest_executable` |
| `debug_hypothesis(assertions, phase, timeout_sec)` | **(A5, 2026-07-11)** батч runtime-гипотез `{object_id,line,expr,expected?}` за ОДИН прогон → per-assertion PASS/FAIL/NO_HIT/INCONCLUSIVE/HIT. Two-phase arm/collect (logpoint-based как A3) |
...
### Шаблон 7: Reproduce-and-inspect — воспроизведение серверного вызова с параметрами ВЫЗЫВАЮЩЕГО (2026-07-12)

**Когда:** просят «пройтись отладкой» по серверной функции (экспорт обработки/общего модуля): заполнить параметры, выполнить, показать результат. Часто быстрее и информативнее BP-трассы — результат = живая ТаблицаЗначений + все параметры на руках. BP/logpoint-трасса (Шаблоны 5/6) — дополнение, когда нужен пошаговый стек/локали, НЕ замена этого протокола.

**Протокол (урок live-прогона `гкс_АРМПромежуточныйКомпозит.ДанныеДляСписка`, SVETLY):**

1. **Параметры вызова = часть контракта.** Найти вызывающего (`Grep` по имени функции → форма/модуль) и **дочитать производителей КАЖДОГО значения до конца цепочки**: реквизит формы → кто его заполняет → домен-функция → константа. Пример цепочки: `Параметры.НачалоПериода` ← реквизит формы ← `УстановитьПериод()` ← `гкс_ПриемкаТранспорта.НачалоПроизводственныхСуток(Сутки)` ← `Константы.гкс_СмещениеПроизводственныхСуток` (=8ч).
2. **Домен-значения вычислять ТЕМИ ЖЕ функциями, что вызывающий, — не руками.** Ловушка «производственные сутки»: смещённый день `[Сутки 08:00, Сутки+1 07:59:59]`, и параметры образуют СВЯЗАННУЮ тройку от одного значения `Сутки` (полночь): `НачалоПериода=НачалоПроизводственныхСуток(Сутки)`, `КонецПериода=ОкончаниеПроизводственныхСуток(Сутки)`, `ПроизводственныеСутки=Сутки`. Ручное «окно пошире» рвёт инвариант (равенство по суткам в ГДЕ × размазанный период) → семантически неверный результат.
3. **Реквизиты объекта-обработки** (отборы АРМ: `ТочкаМаршрута`/`ВидПеревозки`/…) — сверить с ТЕКСТОМ запроса: жёсткий `= &Парам` (особенно внутри `СрезПоследних(...)`) = обязателен к заполнению реальной ссылкой; `&ТекстУсловия`-подстановка через `СформироватьТекстУсловияОтбора` = пусто → «ИСТИНА», можно не заполнять.
4. **Данные искать двухфазно:** сперва дешёвый пробинг картины данных (группировка по измерениям исходного регистра: какие сочетания/даты реально есть, `execute_query`), затем прогон с параметрами, взятыми ИЗ ФАКТОВ. Дефолтная база пуста → проверить соседние профили (`1c-mcp-crud` transport / `-svetly` / `-mfm`...) и явно сказать, в какой базе прогон.
5. **Harness через `execute_code`:** `Обработка = Обработки.X.Создать(); Обработка.<Реквизит> = <ссылка>; ТЗ = Обработка.Метод(Парам, ...)` — эквивалент формы (`РеквизитФормыВЗначение("Объект")`). ТЗ сериализовать вручную: список колонок + первые N строк через `Строка(Значение)` (сериализатор расширения падает на `ПеречислениеСсылка` — см. skill `1c-mcp-crud`).
6. **В вывод ОБЯЗАТЕЛЬНО класть дамп заполненных параметров рядом с результатом** — по нему пользователь/следующий агент верифицирует семантику (именно так была поймана ошибка «окно 30 суток вместо производственных суток»: дамп показал `НачалоПериода` ≠ ожидаемому `25.06 08:00`).
7. Отклоняться от параметров вызывающего (расширить период ради поиска данных) можно только ОТДЕЛЬНЫМ прогоном с явной пометкой «зонд, не семантика формы» — ПОСЛЕ честного прогона.

**Анти-урок (зафиксированная ошибка 2026-07-12):** для ветки «ПроизвольныйКомпозит» период заполнен `МаксПериод − 30 суток` + `КонецДня()` вместо `НачалоПроизводственныхСуток(Сутки)`/`ОкончаниеПроизводственныхСуток(Сутки)` → 3021 строка вместо корректных 114; тройка `{Период, ПроизводственныеСутки}` рассогласована. Корень: `УстановитьПериод()` вызывающего не был дочитан до производителей значений, окно расширено «чтобы гарантированно найти данные» без пометки. Память: [[feedback-1c-reproduce-caller-params]].
...
| Заполнять параметры воспроизводимого вызова «на глаз» (окно пошире, `НачалоДня`/`КонецДня` вместо домен-функций) | Домен-значения образуют связанные наборы от одного источника (напр. тройка производственных суток `{НачалоПериода, КонецПериода, ПроизводственныеСутки}` от `Сутки` + константа смещения 8ч); ручные значения рвут инварианты ГДЕ-условий → ложный объём (3021 vs 114) или ложная пустота | Шаблон 7: дочитать производителей значений у ВЫЗЫВАЮЩЕГО (реквизит → `УстановитьПериод` → домен-функция → константа) и звать ТЕ ЖЕ функции в harness; отклонение — только отдельным помеченным зондом |
| Показывать результат воспроизведённого вызова без дампа заполненных параметров | Ошибку заполнения некому поймать — семантика скрыта; live-кейс 2026-07-12: именно дамп параметров вскрыл неверный период | Всегда «параметры + результат» вместе (Шаблон 7 п.6) |
