...
Каждый ANALYSIS-REPORT.md **обязан** содержать секцию **«13. Источники исследования (Infostart / GitHub / RAG)»** — фиксация данных внешнего поиска, выполненного в ходе анализа (traceability). Записывать **все три канала, включая отрицательные результаты** («искали — не нашли» ≠ «не искали»).

- **13.1 Infostart — порядок ОБЯЗАТЕЛЕН:** **сначала** настроенный 1С/RU-агент **`scripts/onec_search.py`** (ADR-040, Infostart+web, relevance×trust(домен)×recency; для таргета — `--site infostart.ru`), и **только если агент не нашёл** — fallback на `WebSearch`. В отчёте — ранжированный список найденных ссылок (с пометкой score/trust) + ключевой вывод. Голый `WebSearch` по Infostart **без предварительного `onec_search`** не допускается.
- **13.2 GitHub:** `scripts/ecosystem_scan.py "<query>"` (ADR-039). **Порядок — эскалация:** сначала **дефолт 30 дн** (свежее/трендовое = самое релевантное), и **только при пустом результате** расширять окно (`--days 3650`) и пробовать альтернативные/более широкие запросы. Записать **все попытки** (что искали, какое окно) + найденные репозитории ИЛИ явное «не найдено» + причину. ⚠ Источники англоязычные (HN/SO/GitHub-trending/Lobsters/Dev.to) → для RU-1С/БСП-тем канал часто **структурно пуст** даже при широком окне; не выдавать пустоту за «темы не существует».
- **13.3 RAG (pdf-vector-graph):** результат поиска по индексированной документации 1С 8.3.27 — найденные фрагменты ИЛИ явное «релевантного нет» с причиной (напр. тема — библиотечная подсистема БСП, а RAG индексирует платформенную документацию).
- В конце — ссылка на topic-кэш `1c-doc-research/cache/<тема>.md` (Фаза 5 skill `1c-doc-research`), если создавался.
...
## 13. Источники исследования (Infostart / GitHub / RAG)  <-- ОБЯЗАТЕЛЬНО (Правило 5)
### 13.1 Infostart — через onec_search.py (агент ПЕРВЫМ, WebSearch только fallback): ранжированные ссылки + вывод
### 13.2 GitHub — результат ecosystem_scan.py (репозитории ИЛИ «не найдено» + причина)
### 13.3 RAG — pdf-vector-graph по докам 8.3.27 (фрагменты ИЛИ «не найдено» + причина) + ссылка на cache-топик
```
