...
# Анализ задачи 1С — 5-фазная методология (v4.5)
...
**Улучшения v4.4 (2026-07-08, Jira-шаблон «Задача разработки»):**
- Новая ОБЯЗАТЕЛЬНАЯ секция **«14. Заполнение описания — Задача разработки»** (Правило 6): авто-заполняемый корпоративный Jira-шаблон задачи разработки (Предмет / Описание / Назначение / Требования / Метаданные / Ролевая модель / Что сделать / Результат / Пояснения) — готов к копированию в Jira при формировании отчёта. «Что сделать» строится из пошагового плана §12.
- **Стиль §14 (уточнение 2026-07-08):** текст копируется прямо в Jira → только русский (без английских слов), без внутренних инструментов фреймворка и без ссылок на другие главы отчёта, полное связное изложение в стиле ведущего 1С-программиста. Полные требования — в блоке «Стиль §14» Правила 6.
- Источник: [roadmap 260708](../../../docs/roadmap/260708_Pаполнение%20описания%20Задача%20разработки/260708_Pаполнение%20описания%20Задача%20разработки.md) (скриншоты шаблона — там же).

**Улучшения v4.5 (2026-07-13, live-проверка на реальных данных ОБЯЗАТЕЛЬНА - мандат пользователя):**
- **Фаза 2.5 Runtime Trace - обязательная по умолчанию** для каждой задачи с исполняемым BSL-кодом (прежний opt-in `--trace`/self-decision отменён): аналитика проверяется живыми точками останова (`1c-debug-hmr`) на реальных данных ДО построения плана. Opt-out - только `--no-trace` / нет исполняемого алгоритма / debug env недоступен; причина фиксируется в §10.
- **Новое Правило 7** - обязательная глава **«10. Проверка на реальных данных»**: база, конкретные реальные данные (идентификаторы), сценарий, инструменты, результаты; при SKIP - причина + `[STATIC-ASSUMPTION]`. Присутствие проверяет advisory-линтер `scripts/lint_1c_artifacts.py`.
- Закрывает мандат [[feedback-live-bp-trace-mandatory]] (2026-05-11, повторён 2026-07-13); решение - ADR-050.
...
- `1c-debug-hmr` инструменты для обязательной Фазы 2.5 (Runtime Trace на реальных данных, v4.5.0)
...
### Правило 6: Заполнение описания — Задача разработки (ОБЯЗАТЕЛЬНО)

Каждый ANALYSIS-REPORT.md **обязан** содержать секцию **«14. Заполнение описания — Задача разработки»** — заполненный корпоративный Jira-шаблон задачи разработки (критерии I.N.V.E.S.T.). Секция **заполняется автоматически при формировании отчёта** (не плейсхолдерами `<>` — реальным содержимым из проведённого анализа) и готова к копированию в Jira-задачу.

**Маппинг: откуда брать содержимое (переиспользуй секции отчёта, не сочиняй заново):**

| Пункт шаблона | Источник в отчёте | Правила заполнения |
|---|---|---|
| **Предмет** | §1 (суть задачи) | Шаблон наименования: **Глагол → Существительное → Уточнение** («Доработать документ „Лабораторный анализ" для целей качества»). Без сокращений. Предмет = главный результат задачи. В описательную часть не переносить. |
| **Описание** | §1.1 (требования из ТЗ) | Потребности **со слов инициатора**, как есть — задокументировать дословно из ТЗ |
| **Назначение (Для чего?)** | §1.2 | Для чего результат нужен пользователю; что станет лучше / какие риски снижены |
| **Требования → Список требований** | §1.1 | Проработанные потребности — краткое однозначное описание того, что должно быть реализовано |
| **Требования → Интерфейс** | §2 / §4 (формы) | Затронутые формы (документа/справочника/АРМ); скриншоты — отметить «приложить при заведении в Jira», если недоступны |
| **Требования → Функциональное описание** | §3 | Как пользователь будет работать с интерфейсом: что нажимает, какую реакцию получает |
| **Требования → Критерии приемки** | §5 / §7 | Перечень шагов самопроверки разработчика, по которым аналитик подтвердит результат («Как проверить: …») |
| **Требования → Метаданные** | §2 (маркеры) | Таблица `Вид изменения \| Вид объекта \| Имя \| Синоним \| Назначение`. Вид изменения: добавлен/изменен/удален (`[ADDED]`→добавлен, `[MODIFIED]`→изменен). Имя — полное с префиксом (напр. `гкс_СписаниеПотерь`) |
| **Требования → Ролевая модель** | §2 / §4 | Новые роли к новым объектам + существующие роли с расширением прав; типовые роли вендора не изменяем. Если прав не касается — `<не применимо>` |
| **Что сделать (план разработки)** | **§12.1 (ОБЯЗАТЕЛЬНО)** | Перечень шагов полной реализации **с оценкой часов на каждый шаг** — переносится из пошагового плана §12.1 (таблица `Шаг \| Действие \| Оценка, ч`) + итоговая сумма из §12.2. Если сумма > 16 ч — отметить необходимость декомпозиции на связанные задачи. Метаданные в шагах — только добавляемые/изменяемые/удаляемые реквизиты (детализация по реквизитам) |
| **Результат** | §4 / §9 | Добавленные артефакты, которые можно «пощупать руками» (документ, форма, настройка, рассылка…) |
| **Пояснения** | §6 / контекст ТЗ | Произвольный уточняющий комментарий: бизнес-контекст, ограничения («только для Светлого»), как сейчас/как надо |

**Стиль §14 (текст идёт НАПРЯМУЮ в описание Jira — отсюда требования; аналитик/заказчик не знают внутренней кухни):**
- **Только русский, без английских слов.** Термины-инструменты переводить: `ManagerModule`→«модуль менеджера», `ObjectModule`→«модуль объекта», `DataProcessor`→«обработка», `update_database`→«обновление конфигурации базы данных», RMQ→«очередь сообщений». Типы метаданных в прозе — раздельно по-русски («Регистр сведений», «Документ», «Роль»). Кириллические имена объектов (`гкс_…`) оставлять — это идентификаторы 1С, не английский.
- **Без внутренних инструментов фреймворка.** Не упоминать MCP-тулы (`get_access_rights`, `mutate_role_rights`, EDT-MCP), SonarQube, code-verify, коды стандартов (`v8std`, `#stdNNN` → «стандарт разработки 1С о …») — их знает разработчик, не заказчик.
- **Самодостаточно — без ссылок на другие главы отчёта** (`§6`, `§12.1`): §14 читается отдельно, при копировании в Jira такие ссылки станут висячими.
- **Нарратив, а не тезисы-указатели** «см. §…»; стиль ведущего 1С-программиста (ИТС/Infostart). Варианты решения называть «рекомендуемый способ»/«альтернативный способ», не «Вариант A/B». Часы — с запятой (`0,5 ч`).
- Ограничение — **только §14**. Секции §1–§13 (технические, для разработчика) сохраняют инструменты, ссылки на главы и англ. термины.

**Форматирование текста (ВСЕ секции §1-§14, не только §14) [правило пользователя 2026-07-08]:** в прозе, таблицах и числовых диапазонах ставить дефис "-", НЕ типографское тире "—" (em dash) и не "–" (en dash) - напр. "0,5-1,5 ч", "роль-флаг - признак". Причина: "—" плохо копируется в Jira/корпоративные системы и читается как признак ИИ-генерации. Markdown-синтаксис ("---", "|---|") и код не трогать.

**Инвариант:** «Что сделать» и §12 — один и тот же план (§12.1 — источник истины); расхождение шагов/оценок = дефект отчёта.

### Правило 7: Проверка на реальных данных (ОБЯЗАТЕЛЬНО - глава §10)

Каждый ANALYSIS-REPORT.md **обязан** содержать секцию **«10. Проверка на реальных данных»** - на каких живых данных проверялась аналитика и что показала проверка (мандат пользователя 2026-07-13, ADR-050):

- **База:** имя/строка подключения (`Srvr=...;Ref=...`) или MCP-профиль, на котором шла проверка.
- **Данные:** таблица `Объект | Идентификатор (номер/код/ссылка) | Дата | Роль в проверке` - конкретные **реальные** документы/записи (кандидаты подбираются `execute_query` по фактическому наличию; синтетика - только при пустой базе, с явной пометкой).
- **Сценарий/harness:** что исполнялось (`execute_query`; живой пользовательский сценарий при установленном BP [проведение/кнопка - выполняет пользователь]; `execute_code`-harness с параметрами вызывающего [Шаблон 7 skill `1c-debug-hmr`] - только по явному согласованию, исполняется вне read-only scope анализа).
- **Инструменты:** BP-trace `1c-debug-hmr` (`set_breakpoint`/`stack_trace`/`variables`/`evaluate` - Фаза 2.5), `execute_query`.
- **Результаты:** стек/значения/Branch evaluation + вердикт по каждой ключевой гипотезе анализа (подтверждена/опровергнута → как это изменило план Фазы 4).
- **При SKIP** live-проверки глава всё равно обязана существовать: явная причина (`--no-trace` / нет исполняемого кода / debug env недоступен) + перечень допущений `[STATIC-ASSUMPTION]`.

Присутствие главы проверяет advisory-линтер `scripts/lint_1c_artifacts.py` (через хук `pipeline-1c-advance`).
...
### Фаза 2.5: Runtime Trace (ОБЯЗАТЕЛЬНАЯ по умолчанию, v4.5.0)

**Активация (мандат пользователя 2026-07-13, [[feedback-live-bp-trace-mandatory]], ADR-050):** фаза выполняется **ПО УМОЛЧАНИЮ для каждой задачи, затрагивающей исполняемый BSL-код**, - аналитика проверяется живыми точками останова на реальных данных ДО построения плана (Фаза 4). Прежний opt-in (v4.2.0: флаг `--trace` / self-decision) отменён:
- `--trace` остался как **форс** - трассировать даже тривиальный алгоритм;
- прежний self-decision-критерий (**≥3 runtime-ветвлений** по значениям, невозможным для static reading: `Пользователи.ТекущийПользователь()`, `ПолучитьФункциональнуюОпцию(...)`, `Тип(Параметр)`, режимы проведения, `?:`-цепочки) теперь определяет **глубину** трассировки (сколько развилок step'ать на шаге 7), а не факт запуска.

**SKIP допустим ТОЛЬКО по трём причинам** (каждая фиксируется в §10 «Проверка на реальных данных»):
1. Явный `--no-trace` от пользователя.
2. Задача не содержит исполняемого алгоритма (чистые метаданные/роли/синонимы/макеты без BSL-логики).
3. Pre-condition не выполнен (debug env не ready - см. ниже) и поднять его невозможно → runtime-ветвления в плане помечаются `[STATIC-ASSUMPTION]`.
...
4. **Trigger (read-only scope Варианта C):** предпочтительно - `execute_query` (запросная логика) или живой пользовательский сценарий при установленном BP (проведение/кнопка - выполняет пользователь, Правило 7 implement-1c-task). Для серверных процедур - `execute_code`-harness с параметрами вызывающего (Шаблон 7 skill `1c-debug-hmr`) **по явному согласованию с пользователем**: исполняется оркестратором вне allowed-tools `/analyze-1c-task` (read-only дизайн Варианта C сохраняется). Данные - реальные из живой базы (подбор `execute_query`), не fabricate.
...
- ⚠️ SKIP - только по трём допустимым причинам (`--no-trace` / нет исполняемого кода / debug env недоступен); причина зафиксирована в §10, runtime-допущения помечены `[STATIC-ASSUMPTION]`

### Фаза 3: Алгоритм
- **ПОИСК ЭТАЛОНА (обязательно, В2 260718)**: для КАЖДОЙ нетривиальной точки — `bsl_search`/`bsl_hybrid_search`/`bsl_similar` найти ≥1 аналогичную реализацию в конфигурации (эталон стиля/структуры по 37k-чанковому индексу); найденный эталон (`Модуль.Метод`) записать в Фазу 4 как «образец из конфигурации» для соответствующей точки. Пропуск допустим ТОЛЬКО когда аналога в базе нет — с пометкой в отчёте. Причина мандата: presence эталон-поиска замерялась **0.6%** (W1 аудита 260718) — база эталонов недоиспользована, код пишется «с нуля» вместо сверки со стилем конфигурации.
...
- Тест-план
- Проверка на реальных данных - ОБЯЗАТЕЛЬНО (Правило 7, глава §10): live-подтверждение ключевых гипотез (Фаза 2.5 BP-trace + `execute_query`); SKIP только по трём причинам Фазы 2.5, с фиксацией в §10
...
### 3.Y Runtime Trace (обязательно при выполненной Фазе 2.5; при SKIP фазы - вместо секции причина в §10)
...
## 9. Резюме
## 10. Проверка на реальных данных  <-- ОБЯЗАТЕЛЬНО (Правило 7): база; таблица «Объект | Идентификатор | Дата | Роль»; сценарий/harness; инструменты (BP-trace 1c-debug-hmr / execute_query); результаты (стек, значения, вердикты гипотез); при SKIP - причина (--no-trace / нет кода / debug env) + [STATIC-ASSUMPTION]
...
## 14. Заполнение описания — Задача разработки  <-- ОБЯЗАТЕЛЬНО (Правило 6; заполняется АВТОМАТИЧЕСКИ, готово к копированию в Jira)

**Предмет:** <Глагол → Существительное → Уточнение, без сокращений>

**Описание** (со слов инициатора)
- <потребности дословно из ТЗ, из §1.1>

**Назначение** (отвечает на вопрос Для чего?)
- <из §1.2: зачем пользователю, что станет лучше>

**Требования**
- *Список требований*
  - <краткое однозначное описание реализуемого, из §1.1>
- *Интерфейс*
  - <затронутые формы (документ/справочник/АРМ); скриншоты — приложить при заведении в Jira>
- *Функциональное описание*
  - <как пользователь работает с интерфейсом: что нажимает, какую реакцию получает, из §3>
- *Критерии приемки*
  - <шаги самопроверки «Как проверить», из §5/§7>
- *Метаданные*

  | Вид изменения | Вид объекта | Имя | Синоним | Назначение |
  |---|---|---|---|---|
  | добавлен/изменен/удален | документ/справочник/... | гкс_Xxx | Xxx | <для чего объект> |

- *Ролевая модель*
  - <новые роли + расширяемые существующие; типовые роли вендора не изменяем; или «не применимо»>

**Что сделать** (план разработки — из §12.1, с оценкой часов на шаг)
- <Что сделать_1> — <N ч>
- <Что сделать_2> — <N ч>
- Итого: <сумма из §12.2> ч (если > 16 ч — декомпозировать на связанные задачи)

**Результат** (выписать артефакты)
- <добавленные артефакты, которые можно «пощупать руками», из §4/§9>

**Пояснения** (произвольный комментарий)
- <бизнес-контекст, ограничения, как сейчас/как надо, из §6/ТЗ>
```
