...
- **13.2 GitHub:** `scripts/ecosystem_scan.py "<query>" --days 3650` (ADR-039) — запускать **без 30-дневного ограничения** (дефолтное окно свежести пропускает canonical-репо). Записать найденные репозитории ИЛИ явное «не найдено» + причину. ⚠ Источники англоязычные (HN/SO/GitHub-trending/Lobsters/Dev.to) → для RU-1С/БСП-тем канал часто **структурно пуст**; не выдавать пустоту за «темы не существует».
