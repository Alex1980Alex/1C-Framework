...
### Правило 4: Оценка трудозатрат и пошаговый план (ОБЯЗАТЕЛЬНО)

Каждый ANALYSIS-REPORT.md **обязан** заканчиваться секцией **«12. Оценка трудозатрат и пошаговый план выполнения»** (см. шаблон ниже). Это финальный артефакт обоих этапов планирования — без него отчёт неполон.

- **12.1 Пошаговый план** — таблица `Шаг | Действие | Точки/артефакты | Оценка`. Шаги выводятся из **порядка выполнения Фазы 4** (Точки модификации) + обязательные шаги: ревью/снятие open questions (§6), наполнение TestDB и ручная проверка (§5/§7), написание тестов (§7), прогон + `code-verify` + дельта SonarQube. Каждому шагу — оценка **времени** (диапазон «мин–ожид» в часах).
- **12.2 Итоговая оценка** — отдельным параграфом: суммарно **Минимум / Ожидаемо / С запасом на риски** (часы), пересчёт в **идеальные рабочие дни (8 ч)**, метка **сложности** (тривиальная/средняя/сложная — согласована с §11), явные **допущения** (один разработчик, знакомый с подсистемой; без релиза/развёртывания) и **критический путь** (самый рискованный шаг). При неснятых open questions — диапазон смещать вверх и указывать это.
- **Калибровка оценок** (эвристика, адаптировать под задачу): новый/рефактор метода общего модуля ~0.5–1.5 ч; правка запроса с проверкой влияния ~1.5–2 ч; новый реквизит/объект метаданных ~0.5–1 ч; набор YAxUnit-тестов ~2–3 ч; ручная проверка матрицы сценариев + TestDB ~1–1.5 ч; прогон+фикс+code-verify+Sonar ~1–1.5 ч.

### Правило 5: Источники исследования (ОБЯЗАТЕЛЬНО фиксировать в отчёте)

Каждый ANALYSIS-REPORT.md **обязан** содержать секцию **«13. Источники исследования»** — фиксация данных внешнего поиска (traceability), **включая отрицательные результаты** («искали — не нашли» ≠ «не искали»).

**Иерархия источников 1С (приоритет — по типу вопроса):**

1. **Эталон 8.3.27 (платформа)** → RAG `pdf-vector-graph` (локально) или `/search`. ← **платформенные** вопросы (типы, проведение, запросы, формы). ⚠ RAG содержит **только платформенную** документацию.
2. **ИТС (its.1c.ru — полная: платформа + БСП + стандарты v8std)** → skill **`its-research`** / `scripts/its_fetch.py` (Playwright SSO; авторизация сам — `--auto-login` по `.env.its`; db-коды `bsp`/`metod8dev`/`v8std`/`pubextensions`). ← **авторитетный первоисточник для БСП / библиотечных подсистем / стандартов разработки**. Если тема — БСП (`РаботаСФайлами`, `ШаблоныСообщений`, `ОбменДанными`, …) или v8std-стандарт — ИТС **обязателен и приоритетнее Infostart** (RAG по БСП пуст). Атрибуция: `[its.1c.ru, db, content/N, #stdNNN]`.
   - **Глубина ОБЯЗАТЕЛЬНА (не «по заголовкам»):** запрещено останавливаться на заголовках/сниппетах веб-поиска и на странице-**описании подсистемы**. Протокол: (a) **несколько уточнённых** `site:its.1c.ru`-запросов (точные заголовки глав + термины кода, напр. `ВладелецПрисоединенныхФайлов`); (b) при необходимости fetch hub/оглавления нужной db (`v8std`/`metod8dev`/`bsp`) и навигация по ссылкам внутри; (c) **`its_fetch.py "<URL>"` → прочитать ДОСЛОВНУЮ главу/стандарт** (тело статьи, листинги, «Правильно/Неправильно»). В §13.2 — **минимум одна дословная выдержка** с атрибуцией ИЛИ явное «не найдено после N запросов» + перечень что пробовали. Учитывать: часть БСП-РР поставляется **с дистрибутивом БСП** (не отдельной ИТС-db-страницей) — тогда сверять с v8std-стандартами + кодом конфигурации.
   - **Механизм: глубокий разбор → делегировать агенту `its-researcher`** (subagent; **parallel fan-out** по нескольким db за один проход → всесторонне + бережёт контекст оркестратора + аудируемо; в промпт передавать точные термины/объекты/имена каталогов). **Точечная одиночная справка → inline `its_fetch.py`** (меньше overhead). Дистиллят агента (выдержки + атрибуция) вкладывать в §13.2; cache-топик агент обновляет сам.
3. **Infostart (сообщество RU)** → `scripts/onec_search.py` (агент **первым** → `WebSearch` только fallback; `--site infostart.ru`). ← **приоритет 2, дополнение** к вендорским источникам.
4. **GitHub** → `scripts/ecosystem_scan.py` (эскалация: **дефолт 30 дн** → при пустоте `--days 3650` + альт.запросы). ← готовые библиотеки/паттерны; для RU-1С часто структурно пуст (англоязычные источники).

**Маршрутизация:** платформенный вопрос → (1); **БСП / стандарт / библиотечная тема → (2) ИТС первым**, (3) Infostart дополнением; (4) GitHub — всегда проверить и зафиксировать (вкл. «не найдено»).

В §13 фиксировать по каждому **применимому** каналу: что искали, что нашли (ранжированные ссылки / выдержки с атрибуцией) ИЛИ явное «не найдено»/«не консультировались» + причину. В конце — ссылка на topic-кэш `1c-doc-research/cache/<тема>.md` (Фаза 5 skill `1c-doc-research`).

**Внутренние источники (§13.5, ОБЯЗАТЕЛЬНО):** помимо внешних каналов фиксировать **внутреннюю базу знаний**, на которую опирался анализ — это traceability mandatory-петли **recall** наравне с research:
- **Скиллы консультированы** — активированные skill'ы с ролью (`1c-doc-research` — иерархия источников; `1c-solution-architecture` — decision-tree объектов/регистров; `its-research` — ИТС-fetch; и т.д.).
- **Память (recall)** — результаты `mcp__memory-orchestrator__unified_search` / `search_patterns` по теме + релевантные `memory/*.md` блоки с **применимостью** (что именно учтено), ИЛИ явное «релевантных блоков нет». Recall обязателен по task-completion-гейту — §13.5 делает его аудируемым.
...
## 12. Оценка трудозатрат и пошаговый план выполнения  <-- ОБЯЗАТЕЛЬНО (Правило 4)
### 12.1 Пошаговый план (таблица: Шаг | Действие | Точки/артефакты | Оценка времени)
### 12.2 Итоговая оценка трудозатрат (Минимум/Ожидаемо/С запасом — часы + идеальные дни; сложность; допущения; критический путь)

## 13. Источники исследования  <-- ОБЯЗАТЕЛЬНО (Правило 5; внешние: эталон 8.3.27 → ИТС → Infostart → GitHub; + §13.5 внутренние: скиллы + память/recall)
### 13.1 Эталон 8.3.27 / RAG (pdf-vector-graph) — платформенные вопросы (фрагменты ИЛИ «не найдено» + причина)
### 13.2 ИТС (its-research / its_fetch.py) — БСП/библиотечные/стандарты, ПЕРВИЧНО для БСП. ГЛУБОКО: несколько точных site:-запросов + fetch ДОСЛОВНОЙ главы/стандарта (≥1 выдержка с атрибуцией [its.1c.ru, db, content/N]), НЕ по заголовкам/описанию подсистемы; ИЛИ «не найдено после N запросов» + что пробовали
### 13.3 Infostart (onec_search.py — агент ПЕРВЫМ, WebSearch fallback) — ранжированные ссылки + вывод (дополнение)
### 13.4 GitHub (ecosystem_scan.py — 30 дн → эскалация --days 3650) — репозитории ИЛИ «не найдено» + причина
+ ссылка на cache-топик 1c-doc-research/cache/<тема>.md
### 13.5 Внутренние источники: скиллы + память (ОБЯЗАТЕЛЬНО — traceability recall-петли) — консультированные скиллы (с ролью) + recall (`unified_search`/`search_patterns` + релевантные `memory/*.md` с применимостью) ИЛИ «релевантных нет»
```
