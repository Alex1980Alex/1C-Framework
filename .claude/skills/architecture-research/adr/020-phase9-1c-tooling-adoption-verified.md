...
- [ ] **sonar-plugin 1.18.1 — DEFER подтверждён (verified 2026-06-15)**: SonarQube = `docker/docker-compose.sonarqube.yml` → image `sonarqube:lts-community` (9.9-era LTA), **контейнер не запущен**. 1.18.1 требует SonarQube ≥2025.4 → это апгрейд **СЕРВЕРА**, не swap jar'а. Уже сделанного (`config_manager.py` 1.16.1) достаточно; реальная замена — после апгрейда сервера.
