...
>
> **Приём-путь OTLP верифицирован (2026-06-17):** лёгкий sink `docker/docker-compose.otel-collector.yml`
> (otel-collector, 1 контейнер) поднят; синтетический спан `claude_code.tool.execution` →
> `POST /v1/traces` = `200` → принят и залогирован (точная форма эмиссии Claude Code). Эмиссия самого
> `claude` требует рестарта сессии — за пользователем. Langfuse (дашборды) — официальный compose, не вендорим.
> P2-followup #1 (rollup → реальная Pre/Post-латентность вместо overhead) реализован.
