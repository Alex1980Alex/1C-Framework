...
**Полная композиция R3 (2026-06-21, сверх роадмапа):** [`shared/gate_policies.py`](../../../.claude/hooks/shared/gate_policies.py) (`build_context` — ОДИН проход, переиспует проверенные хелперы живых хуков через importlib; чистые `pipeline_policy`/`onec_completion_policy`) + opt-in оркестратор [`gate-orchestrator-stop.py`](../../../.claude/hooks/gate-orchestrator-stop.py): при `GATE_ORCHESTRATOR_ENABLE=1` заменяет 2 раздельных Stop-хука **одним** `evaluate_gates`-проходом → ОДИН консолидированный блок со всеми ✗ (вместо пинг-понга по одной проблеме за Stop-цикл); старые `pipeline-protocol-stop`/`onec-task-completion-stop` уступают early-return по тому же env. **Default OFF → поведение неизменно** (обратимо одним env / снятием записи из settings.json). Инварианты сохранены (per-policy opt-out, carve-out чистых вопросов, graceful→allow). 6 unit (`test_gate_policies.py`) + yield-smoke + import-smoke PASS.
