...
парсит `idle=` из `[hb]`-строк; `idle > IDLE_LIMIT` (1200 с — легит тяжёлый файл batch=1
≤~10 мин, истинный wedge = часы) → kill дерева (`taskkill /T /F`) → перезапуск с **меньшим
batch (лестница 32→8→1)**. `--skip-indexed` сохраняет прогресс; batch=1 гарантированно не
виснет. Эскалация batch выбрана вместо poison-by-path: буфер копит чанки нескольких файлов
