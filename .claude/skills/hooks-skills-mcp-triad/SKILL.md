...
| `docs-change-enforcer.py` | Блокировка если код изменён без обновления документации. `SKIP_PATTERNS` исключает инфра-файлы, не являющиеся product code: `pyproject.toml`, `.mcp.json`, `.env.example`, `.gitmodules`/`.gitignore`/`.gitattributes`, `tools/`, `scripts/`, `tests/`, `__init__.py`, `src/projects/`, `src/bsl/`. При добавлении нового типа инфра-файла его путь/имя нужно добавить в `SKIP_PATTERNS`, иначе `UNMAPPED` блокировка. |
