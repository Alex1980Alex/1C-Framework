...
> **YAxUnit-смоук серверной логики (advisory, рек.3 2026-07-19) — POST-change комплемент impact-чека.** На том же условии применимости (правка/добавление ЭКСПОРТНОГО метода = тестируемый инвариант, `edits_exported_method`) после реализации **рекомендуется** написать и прогнать YAxUnit-смоук на **YAxUnit-тестируемую** часть изменения: серверно-вызываемые методы (**общий / менеджер / объект** — `<Модуль>.<Метод>()` на сервере БЕЗ открытия формы), assert на инвариант. ⚠ **Детектор `edits_exported_method` намеренно широк (нудж, НЕ сужаем):** если правка — экспортный метод **модуля формы** / UI-логика, YAxUnit к нему неприменим (нужен контекст формы, серверный тест его не построит) → это **BDD** (`va-bdd-testing`, `qa_run`); YAxUnit-смоук покрывает серверную часть ТОЙ ЖЕ задачи (комплементарно, не вместо) или пропускается, если серверно-вызываемой части нет — принцип «тестируем то, что можно YAxUnit тестами». Правило выбора YAxUnit↔BDD (`<Модуль>.<Метод>()` без формы → YAxUnit; нужен UI → BDD) — skill [`yaxunit-unit-testing`](../yaxunit-unit-testing/SKILL.md). **Единственный маршрут — `mcp-onec-test-runner`** (`run_module_tests`/`run_all_tests`); НЕ дублировать через `edt run_yaxunit_tests` / `codepilot qa_*` (те же тесты, лишние маршруты), НЕ BDD (это для UI-сценариев). Методика написания — skill [`yaxunit-unit-testing`](../yaxunit-unit-testing/SKILL.md) + `/write-1c-unit-tests`→`/run-1c-unit-tests`. Advisory (opt-in по умолчанию, НЕ блок завершения) — сюрфейсится `•`-буллетом в `onec-task-completion-stop` и строкой LOOPS.md; follow-rate замеряется `onec_toolgate_validation.yaxunit_rate_on_applicable`. Прогон YAxUnit пишет `.run-state.json` → авто-закрытие Этапа 4 (F-1.6); при отсутствии прогона Этап 4 закрывается секцией «Тестирование на реальных данных» (ADR-050, `advance_test_done`).
...
**Правила записи:**
- **Свод правил пользователя ДО записи** (2026-07-24): прогнать код по чеклисту
  [bsl-development → «Стоячие правила пользователя»](../bsl-development/SKILL.md) (БСП-чтение реквизитов,
  СтрШаблон, предопределённые, ВТ вместо подзапроса, маркеры задачи...). Машинную часть продублирует
  хук `bsl-user-rules-check` (advisory в тот же ход) - его замечания исправлять СРАЗУ, до Sonar
...
**Шаги 0-5** (обновление конфигурации БД через `update_database` с программным gate на активные подключения → подготовка тестовых данных → пользователь проводит документ → проверка результата → очистка → фиксация в PROGRESS), альтернатива SQL-симуляции без обновления БД, **безопасный test-post в откатываемой транзакции** (проведение реального документа → сбор ошибок → `ОтменитьТранзакцию()` — live-проверка ПРОВЕДЕНИЯ без порчи данных, В3 260718), и **YAxUnit unit-тесты** (`/write-1c-unit-tests` → `yaxunit-unit-testing`, деплой расширения, smoke `run_module_tests`) — полный текст: [references/stage-details.md#этап-6-тестирование-на-живых-данных--детали-шаги-0-5](references/stage-details.md#этап-6-тестирование-на-живых-данных--детали-шаги-0-5).
