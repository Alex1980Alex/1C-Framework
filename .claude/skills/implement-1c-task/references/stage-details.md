...
**Test-post в откатываемой транзакции — БЕЗОПАСНАЯ live-проверка ПРОВЕДЕНИЯ (В3 260718, приём портирован из skiddgoddamn/1c-mcp).**
Для задач, где изменена логика **проведения** документа (`ОбработкаПроведения`, формирование движений регистров), лучший способ проверить на реальных данных БЕЗ порчи базы и без шага очистки — провести РЕАЛЬНЫЙ документ внутри транзакции и **всегда откатить**. Логика проведения исполняется на живых данных, движения считаются, ошибки всплывают — а `ОтменитьТранзакцию()` отменяет все изменения. Требует шаг 0 (`update_database`), иначе исполнится СТАРАЯ логика.

```bsl
// execute_code на живой базе. Ссылка — реальный документ, подобранный execute_query.
Отказ = Ложь;
ТекстОшибки = "";
НачатьТранзакцию();
Попытка
    ДокОбъект = Ссылка.ПолучитьОбъект();
    ДокОбъект.Записать(РежимЗаписиДокумента.Проведение);  // прогон ОбработкаПроведения на реальных данных
    // при необходимости — чтение сформированных движений для ассертов:
    // Движения = ДокОбъект.Движения.гкс_РегистрX.Выгрузить();
Исключение
    Отказ = Истина;
    ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
КонецПопытки;
ОтменитьТранзакцию();  // ВСЕГДА откат — база не меняется, очистка не нужна
Возврат Новый Структура("Отказ, ТекстОшибки", Отказ, ТекстОшибки);
```

Преимущества над обычным «провести → проверить → очистить»: (1) нулевой side-effect (не нужен шаг очистки, невозможна порча реальной базы); (2) применимо на production-подобной базе; (3) ловит реальные ошибки проведения (нехватка остатков, RLS, проверки заполнения) на фактических данных. Идентификаторы использованного документа — в главу «Тестирование на реальных данных» Этапа 7. ⚠ Побочные эффекты ВНЕ транзакции БД (запись в внешние системы через HTTP-сервисы, `ЗаписатьЖурналРегистрации`, temp-файлы) откатом не отменяются — для таких проверять точечно.
...
<!-- ОБЯЗАТЕЛЬНО (v2.9.0, ADR-050): на каких данных тестировалось и какие результаты.
     Проверяет lint_1c_artifacts (advisory) через pipeline-1c-advance. -->

- **База:** `Srvr=...;Ref=...` / MCP-профиль (на которых шли Этапы 5.x/6)

| Объект | Идентификатор (номер/код/ссылка) | Дата | Сценарий |
|---|---|---|---|
| Документ.гкс_Xxx | БАТМ-000000 | 01.01.2026 | BP-триггер Точки N / тест-план X.Y |

- **Инструменты:** BP-trace `1c-debug-hmr` (модуль:строка, стек frames[-1], значения переменных) / `execute_query` / `execute_code` / YAxUnit / VA BDD
- **Результаты:** per-сценарий PASS/FAIL + итоговый вердикт; синтетические данные или SKIP - явно, с причиной

## Открытые вопросы (если есть)

## Сообщение коммита

<!-- ОБЯЗАТЕЛЬНО (Этап 7/8): готовое сообщение git-коммита по скиллу git-commit-message.
     Пользователь копирует его отсюда без переспроса; им же коммитится Этап 8. -->
```
<тип>: <результат> (Доработано/Исправлено/Добавлено …)

Как было:
<состояние ДО>

Как стало/список результатов:
<что изменилось — термины 1С: Общий модуль/Документ/Регистр сведений/Запрос, не имена файлов>
- …

Изменённые/добавленные объекты:
- …

МЕТАДАННЫЕ: GKSTCPLK-XXXX
```
