...
| `batch_cli_concurrency` (`LLM_ROTATION_BATCH_CLI_CONCURRENCY`) | **3** | ⚠ семафор на ОДНОВРЕМЕННЫЕ спавны `claude-cli`, взводится в **точке спавна** (`_cli_spawn_gate` в `_call_provider`), а не на уровне батча. Каждый такой вызов = **полный второй Claude Code** (~2.8 ГБ commit); шесть параллельных кладут сессию (инцидент 2026-07-26 16:51). Место контроля важно: клемп на уровне батча был **fail-open** — там известен лишь *предпочитаемый* провайдер, а исполнителя выбирает ротация (`preferred_provider="ollama-local"` + лежащий ollama → фоллбэк на claude-cli → 6 полных Claude Code). В точке спавна инвариант держится при любом маршруте, а общая параллельность 6 остаётся живой для дешёвых провайдеров |
| `queue_credit_max_seconds` | **20** | ожидание в гейте спавнов возвращается в бюджет ротации (иначе очередь съедала его: под нагрузкой `complete()` вырождался в один выстрел, а терминальный `provider="none"` + `"All failed"` — ОБА маркера `_is_failure` — на полчаса разоружали `z-ai-write-guard` прямо из продового вызова). Кредит **ограничен**: связывающий инвариант — `total_budget + credit < клиентское окно` (270 + 20 = 290 < 300), он и запинен тестом. Больше кредита — только со снижением `total_budget_seconds`. Арифметика вынесена в чистую `apply_queue_credit` (внутри `complete()` она непроверяема: решение о попытке принимается ДО начисления кредита) |
| `LLM_ROTATION_BATCH_SLOW_THRESHOLD_S` | **180** (было 30) | порог `slow_call` → ÷2 конкурентности с записью на диск. 30с структурно неверны для claude-cli, чья ЧЕСТНАЯ латентность 25-150с: каждый нормальный вызов считался медленным, и поднятые до 6 значения сползли бы к 1 и остались там между перезапусками |
| `batch_budget_seconds` (`LLM_ROTATION_BATCH_BUDGET_SECONDS`) | **250** | агрегатный дедлайн ВСЕГО батча. `total_budget_seconds` ограничивает один `complete()`, а батч — это `ceil(N/concurrency)` волн: на claude-cli (25-150с) уже N≈10-15 выходит за клиентское окно. По истечении недовыполненные снимаются, **готовые возвращаются** с `TimeoutError` в своих слотах — частичный результат честнее пустого (голый `gather` терял всю работу при обрыве клиентом) |
