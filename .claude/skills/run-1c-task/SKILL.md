...
**P3.3 — адверсариальные критики ДИЗАЙНА перед авто-approve (medium/complex; trivial — пропустить).**
Человек дизайн НЕ ревьюит → компенсируем параллельными субагентами-критиками ANALYSIS-REPORT
(паттерн agentico: критики плана перед реализацией). Запусти **2-3 субагента одним сообщением**
(read-only: Read/Grep/Glob по ANALYSIS-REPORT + коду конфигурации), каждый со своим фокусом и
**структурным вердиктом** (`{verdict: ok|blocker, issues: [...]}`):
- **полнота ТЗ** — покрыты ли все требования; нет ли пропущенных объектов/сценариев;
- **объекты-метаданные** — верны ли точки модификации (существуют, тип/имя совпадают с конфигурацией);
- **риски-регрессы** — что сломается (экспортные методы, обмен, проведение); есть ли обратная совместимость.

Любой `verdict=blocker` → **НЕ авто-approve: деградируй в гейтованный поток** (выпиши блокеры,
спроси пользователя / отдай на ручной `/analyze`+`/implement`). Все `ok` → продолжай авто-approve.
...
**P3.6 — compound-learning на провале (паттерн Graybark: причина ошибки → правило).** При эскалации
`needs-human` ИЛИ повторяющемся FAIL одной причины — зафиксируй **причину провала** (не только успех):
`mcp__skill-learning__capture_pattern(pattern_type="error-fix", name=..., content="симптом → корень →
что попробовать в следующий раз")` + предложи пользователю конкретную правку SKILL/правила/чек-листа,
которая поймала бы это раньше. Петля памяти фиксирует успехи охотнее провалов — здесь выравниваем.
