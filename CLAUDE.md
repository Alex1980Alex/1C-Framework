...
- **Post-PR-#2-merge hook regressions (2026-05-23)**: `git merge -X theirs origin/master` в `feat/serena-audit-hybrid-refactor` тихо протащил 4 регрессии — `NameError 'Any'` в `shared/hook_lock.py` (typing import lost при ruff format), `ValueError unpack` в `posttooluse-bash-errors.py:37` (broken 4-tuple ERROR_PATTERNS entry), `UnicodeEncodeError '→'` в `memory-first-hook.py` (Windows cp1251 stdout), `NameError 'search_wiki'` (Layer 4 wiki stub missing). Все 4 fixed на `feat`, затем на `master` через [PR #8](https://github.com/Alex1980Alex/1C-Framework/pull/8) — реальный пример `Pattern A small focused PR` из [§11.2 roadmap 260523 unified branch topology](docs/roadmap/260523_ROADMAP_UNIFIED_BRANCH_TOPOLOGY.md). Memory lesson — [`feedback_post_merge_smoke_required`](file:///C:/Users/Tech.%20Boutique/.claude/projects/C--1--Framework/memory/feedback_post_merge_smoke_required.md): mandatory `importlib.spec_from_file_location` sweep всех 59 top-level hooks + audit log tail на `outcome=error` после любого `-X theirs` на 20+ conflict-diff. Lifecycle + tech analysis — [roadmap 260523_ROADMAP_FULL_DEV_LIFECYCLE_ANALYSIS](docs/roadmap/260523_ROADMAP_FULL_DEV_LIFECYCLE_ANALYSIS.md) §5.2 + §9.1 + §9.2 + §9.3
