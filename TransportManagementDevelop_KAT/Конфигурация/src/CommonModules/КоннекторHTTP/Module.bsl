...
Функция ПрочитатьGZip(СжатыеДанные) Экспорт
	...
	ПотокZip = Новый ПотокВПамяти(
				ZipРазмерLFH() + РазмерСжатыхДанных + ZipРазмерDD() + ZipРазмерCDH() + ZipРазмерEOCD());
...
Функция НастройкиПодключения(Сессия, Метод, URL, ДополнительныеПараметры)
	...
	КлиентскийСертификатSSL = ВыбратьЗначение(КлиентскийСертификатSSLИзДополнительныхПараметров, 
		Сессия, "КлиентскийСертификатSSL", Неопределено);
...
Процедура ПодготовитьАутентификациюBearer(ПодготовленныйЗапрос)   
	...
	Если Не ПодготовленныйЗапрос.Аутентификация.Свойство("Токен") или 
	Не ЗначениеЗаполнено(ПодготовленныйЗапрос.Аутентификация.Токен) Тогда 
		// Токен не заполнен.
		Возврат;	
	КонецЕсли;

	ПодготовленныйЗапрос.Заголовки.Вставить("Authorization", 
		СтрШаблон("Bearer %1", ПодготовленныйЗапрос.Аутентификация.Токен));
