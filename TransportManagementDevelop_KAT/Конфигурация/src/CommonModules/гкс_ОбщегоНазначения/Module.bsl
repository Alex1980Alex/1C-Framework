...
//  НазначениеТипа - НазначениеТипаXML, Неопределено - Вариант назначения типа элемента данных XDTO.
//                             Если параметр не указан, будет использовано НазначениеТипаXML.Явное.
//
// Возвращаемое значение: 
//   Строка - JSON-представление XDTO-объекта.
Функция ОбъектXDTOВСтрокуJSON(Знач ОбъектXDTO, Знач Фабрика = Неопределено, НазначениеТипа = Неопределено) Экспорт
	...
	Фабрика.ЗаписатьJSON(Запись, ОбъектXDTO, НазначениеТипа);
...
// Возвращает ФИО подписанта в формате «Фамилия И.О.» независимо от исходного
// порядка записи наименования: «Фамилия И.О.», «Инициалы Фамилия» или полное
// «Фамилия Имя Отчество». Устойчив к слитным инициалам, пропущенным точкам и
// хвостовым пробелам. Нераспознанные значения (должность, произвольный текст)
// возвращаются без изменений.
//
// Параметры:
//  Наименование - Строка - исходное наименование подписанта (реквизит
//                 Наименование справочника гкс_ОтветственныеЛица).
//
// Возвращаемое значение:
//  Строка - ФИО в формате «Фамилия И.О.» либо исходная строка, если это не ФИО.
//
Функция ФИОДляПечати(Наименование) Экспорт

	Если ТипЗнч(Наименование) <> Тип("Строка") Тогда
		Возврат Наименование;
	КонецЕсли;
	
	Исходное = СокрЛП(Наименование);
	Если ПустаяСтрока(Исходное) Тогда
		Возврат Исходное;
	КонецЕсли;
	
	МассивСлов = Новый Массив;
	МассивИнициалов = Новый Массив;
	Для Каждого Токен Из СтрРазделить(Исходное, " ", Ложь) Цикл
		РазобратьТокенНаименованияФИО(Токен, МассивСлов, МассивИнициалов);
	КонецЦикла;
	
	Возврат ФИОИзРазобранныхЧастей(Исходное, МассивСлов, МассивИнициалов);

КонецФункции

// Собирает ФИО в формате «Фамилия И.О.» из разобранных слов и инициалов,
// либо возвращает исходную строку, если это не похоже на ФИО.
//
// Параметры:
// Исходное - Строка - исходное наименование (fallback, если это не ФИО).
// МассивСлов - Массив - слова (Фамилия/Имя/Отчество), выделенные из наименования.
// МассивИнициалов - Массив - буквы-инициалы; при полном ФИО дополняется из слов.
//
// Возвращаемое значение:
// Строка - ФИО «Фамилия И.О.» либо Исходное.
//
Функция ФИОИзРазобранныхЧастей(Исходное, МассивСлов, МассивИнициалов)

	МаксимумСловВПолномФИО = 3;
	КоличествоСлов = МассивСлов.Количество();
	ЕстьИнициалы = МассивИнициалов.Количество() > 0;
	
	Если КоличествоСлов = 1 И НЕ ЕстьИнициалы Тогда
		// Одно слово без инициалов - моно-фамилия.
		Результат = МассивСлов[0];
	ИначеЕсли КоличествоСлов = 0 ИЛИ (КоличествоСлов > МаксимумСловВПолномФИО И НЕ ЕстьИнициалы) Тогда
		// Нет слов с заглавной буквы, либо больше трёх слов без инициалов - это не ФИО.
		Результат = Исходное;
	Иначе
		
		Если НЕ ЕстьИнициалы Тогда
			// Полное «Фамилия Имя Отчество» - инициалы из слов после фамилии.
			Для Индекс = 1 По КоличествоСлов - 1 Цикл
				МассивИнициалов.Добавить(Лев(МассивСлов[Индекс], 1));
			КонецЦикла;
		КонецЕсли;
		Результат = СтрШаблон("%1 %2.", МассивСлов[0], СтрСоединить(МассивИнициалов, "."));
	КонецЕсли;
	
	Возврат Результат;

КонецФункции
// Разбирает один токен наименования: отделяет ведущие инициалы (одиночные заглавные
// буквы, опционально с точкой, в том числе слитные) от слова (Фамилия/Имя/Отчество -
// заглавная буква с последующей строчной).
//
// Параметры:
//  Токен - Строка - разбираемый токен наименования.
//  МассивСлов - Массив - накопитель слов, пополняется по ссылке.
//  МассивИнициалов - Массив - накопитель букв-инициалов, пополняется по ссылке.
//
Процедура РазобратьТокенНаименованияФИО(Токен, МассивСлов, МассивИнициалов)
	
	ПозицияВторогоСимвола = 2;
	ПозицияПослеИнициалаСТочкой = 3;
	
	Остаток = Токен;
	Пока СтрДлина(Остаток) > 0 Цикл
		
		Символ = Лев(Остаток, 1);
		Если НЕ ЭтоЗаглавнаяБуква(Символ) Тогда
			// Не заглавная буква (строчная, точка, цифра) - разбор токена окончен.
			Прервать;
		КонецЕсли;
		
		Если СтрДлина(Остаток) = 1 Тогда
			МассивИнициалов.Добавить(Символ);
			Прервать;
		КонецЕсли;
		
		СледующийСимвол = Сред(Остаток, ПозицияВторогоСимвола, 1);
		Если СледующийСимвол = "." Тогда
			МассивИнициалов.Добавить(Символ);
			Остаток = Сред(Остаток, ПозицияПослеИнициалаСТочкой);
		ИначеЕсли ЭтоЗаглавнаяБуква(СледующийСимвол) Тогда
			// Две заглавные подряд - текущий символ считаем инициалом.
			МассивИнициалов.Добавить(Символ);
			Остаток = Сред(Остаток, ПозицияВторогоСимвола);
		Иначе
			// Заглавная и следом строчная - начало слова (Фамилия/Имя/Отчество).
			МассивСлов.Добавить(Остаток);
			Прервать;
		КонецЕсли;
...
// Определяет, является ли символ заглавной буквой (совпадает со своим верхним
// регистром и отличается от нижнего). Цифры, знаки препинания и пробел заглавной
// буквой не считаются.
//
// Параметры:
// Символ - Строка - проверяемый символ (ожидается длина 1).
//
// Возвращаемое значение:
...
Функция ЭтоЗаглавнаяБуква(Символ)
	...
	СимволВВерхнемРегистре = ВРег(Символ);
		Возврат Символ = СимволВВерхнемРегистре И СимволВВерхнемРегистре <> НРег(Символ);
