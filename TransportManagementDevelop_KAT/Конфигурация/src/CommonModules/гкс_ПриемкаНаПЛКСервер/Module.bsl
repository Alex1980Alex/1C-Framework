	...
	Если Свойства.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		...
		Если ТипЗнчДокумента = Тип("ДокументОбъект.гкс_УстановкаНастроекНазначенияРазгрузки") Тогда
			Параметры.ПодчиненныеРегистры.Добавить(
				Метаданные.РегистрыСведений.гкс_НастройкиНазначенияРазгрузки);
			Параметры.ПодчиненныеРегистры.Добавить(
				Метаданные.РегистрыСведений.гкс_ДополнительныеНастройкиНазначенияРазгрузки);
		КонецЕсли;	
...
Процедура ОтразитьДвижения(ТаблицыДляДвижений, Движения, Отказ) Экспорт
	...
	гкс_ПроведениеДокументов.ОтразитьДвижения(
		ТаблицыДляДвижений, Движения, "гкс_НастройкиНазначенияРазгрузки");
	гкс_ПроведениеДокументов.ОтразитьДвижения(
		ТаблицыДляДвижений, Движения, "гкс_ДополнительныеНастройкиНазначенияРазгрузки");
...
Процедура ПослеЗаписиДвиженийДокумента(Документ, МенеджерВременныхТаблиц, Отказ) Экспорт
	...
	Если ТребуетсяОменаЗаписиСостоянияРегистрации(ТипДокументовПриемки, ТипЗнчДокументаСсылка, 
		Документ, ДокументРегистрации) Тогда 
...
Процедура ЗаписатьДанные(ТаблицыДляДвижений, Документ, Отказ) Экспорт
	...
	ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.гкс_Взвешивание") Тогда
		...
		Если Не гкс_ПриемкаТранспорта.УстанавливатьСостояниеПоВзвешиванию(Документ) Тогда
			РегистрыСведений.гкс_АРМПриемкиНаПЛКСписокРегистраций
				.ЗарегистрироватьВАРМПриемкиНаПЛК(Документ.ДокументРегистрации);
...
Функция ПолучитьСписокРегистрацийВКомпозит(СтруктураПараметров) Экспорт
	...
	|ПОМЕСТИТЬ ВТ_ФормированиеНомераПробыНеПриемкаКомпозит
	|ИЗ
	...
	|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.гкс_ЛабораторныйАнализ КАК гкс_ЛабораторныйАнализ
	|			ПО гкс_ФормированиеНомераПробы.Ссылка = гкс_ЛабораторныйАнализ.гкс_ДокументРегистрации
	|		ПО ВТ_СписокРегистраций.КомпозитнаяПроба = гкс_ФормированиеНомераПробы.Ссылка
	|ГДЕ
	|	ВЫБОР
	|		КОГДА НЕ гкс_ЛабораторныйАнализ.Ссылка ЕСТЬ NULL
	|		И
	|			гкс_ЛабораторныйАнализ.гкс_НазначениеИспользованияКачества = ЗНАЧЕНИЕ(Перечисление.гкс_НазначенияИспользованияКачества.ПриемкаКомпозит)
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ
	|;
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ФормированиеНомераПробыНеПриемкаКомпозит КАК ВТ_ФормированиеНомераПробыНеПриемкаКомпозит
	|		ПО ВТ_СписокРегистрацийКорректные.ДокументРегистрации = ВТ_ФормированиеНомераПробыНеПриемкаКомпозит.ДокументРегистрации
	|		И ВТ_СписокРегистрацийКорректные.КомпозитнаяПроба = ВТ_ФормированиеНомераПробыНеПриемкаКомпозит.КомпозитнаяПроба
	|ГДЕ
	|	ВЫБОР
	|		КОГДА &ДетализацияПоПунктуПогрузки
	|		И &ЭтоВидПеревозкиЖД
	|			ТОГДА ВТ_СписокРегистрацийКорректные.ПунктПогрузкиРегистрации = &ПунктПогрузки
	|			И ВТ_СписокРегистрацийКорректные.Грузоотправитель = &Грузоотправитель
	|			И ВТ_СписокРегистрацийКорректные.КодСтанцииЖДПогрузки = &СтанцияОтправления
	|		КОГДА &ДетализацияПоПунктуПогрузки
	|		И &ЭтоВидПеревозкиАвто
	|			ТОГДА ВТ_СписокРегистрацийКорректные.ПунктПогрузкиРегистрации = &ПунктПогрузки
	|			И ВТ_СписокРегистрацийКорректные.Грузоотправитель = &Грузоотправитель
	...
	ОтборПоСобственнику = "";
	СтруктураПараметров.Свойство("Собственник", Собственник);
	
	Если ЗначениеЗаполнено(Собственник) Тогда
		ОтборПоСобственнику = " И ВЫБОР
		|	КОГДА РегистрацияНаПЛК.Собственник ССЫЛКА Справочник.Контрагенты
		|		ТОГДА ВЫРАЗИТЬ(РегистрацияНаПЛК.Собственник КАК Справочник.Контрагенты)
		|	ИНАЧЕ ВЫРАЗИТЬ(РегистрацияНаПЛК.Собственник КАК Справочник.Организации).КонтрагентОрганизации
		|КОНЕЦ = &Собственник" + Символы.ПС;
		Запрос.УстановитьПараметр("Собственник", Собственник);
	КонецЕсли;
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "И &ОтборСобственник", ОтборПоСобственнику);
	
	// Установка параметров для группировки по пункту погрузки
	ПунктПогрузки = Неопределено;
	Если СтруктураПараметров.Свойство("ДетализацияПоПунктуПогрузки") 
		И СтруктураПараметров.ДетализацияПоПунктуПогрузки Тогда
		
		Запрос.УстановитьПараметр("ДетализацияПоПунктуПогрузки", Истина);
	Иначе 
		Запрос.УстановитьПараметр("ДетализацияПоПунктуПогрузки", Ложь);
	КонецЕсли;
...
Функция ПолучитьПересекаемостьНормативныхПоказателейСерификации(СсылкаНаОбъект) Экспорт
	...
	|	И РезультатыНормативнойСертификацииНоменклатуры.ВидПеревозки = &ВидПеревозки
...
Функция ИнициализироватьСтруктуруДоступовПриемкаПЛК() Экспорт
	...
	СтруктураДоступов.Вставить("ДоступенДопускВетеринарный",
      							Пользователи.РолиДоступны("гкс_ВозможностьМенятьДопускВетеринарныйВАРМеПриемкиНаПЛК"));
	 ...
	 СтруктураДоступов.Вставить("ДоступенДопускаКВскрытиюВагонов",  
								...
								ИЛИ СтруктураДоступов.ДоступенДопускВетеринарный);
...
// Фильтрует данные таблицы по указанному документу регистрации
//
// Параметры:
//  ТаблицаДанных - ТаблицаЗначений - исходная таблица данных
//  ДокументРегистрации - ДокументСсылка.гкс_РегистрацияНаПЛК - документ регистрации для фильтрации
//
// Возвращаемое значение:
//  ТаблицаЗначений - отфильтрованная таблица
//
Функция ОтфильтроватьДанныеПоДокументуРегистрации(ТаблицаДанных, ДокументРегистрации)
	
	ОтборПоДокументу = Новый Структура("ДокументРегистрации", ДокументРегистрации);
...
// Требуется омена записи состояния регистрации.
// 
// Параметры:
//  ТипДокументовПриемки Тип документов приемки
//  ТипЗнчДокументаСсылка Тип знч документа ссылка
//  Документ Документ
//  ДокументРегистрации Документ регистрации
// 
// Возвращаемое значение:
//  Булево - Требуется омена записи состояния регистрации
Функция ТребуетсяОменаЗаписиСостоянияРегистрации(ТипДокументовПриемки, ТипЗнчДокументаСсылка, 
	...
	Возврат ЭтоОтменаПроведенияДокумента
		И ТипДокументовПриемки.Найти(ТипЗнчДокументаСсылка) <> Неопределено
		И Не ТребуетсяПропуститьЗаписьСостоянияРегистрации(Документ, ДокументРегистрации);

КонецФункции


// Определяет, нужно ли пропустить запись состояния регистрации.
// Для документов, отличных от РегистрацияНаПЛК, проверяет статус "Убыл".
//
// Параметры:
...
//
// Возвращаемое значение:
//  Булево - Истина, если запись состояния следует пропустить
//
Функция ТребуетсяПропуститьЗаписьСостоянияРегистрации(Документ, ДокументРегистрации)

	Если ТипЗнч(Документ) = Тип("ДокументОбъект.гкс_РегистрацияНаПЛК") Тогда
		Возврат Ложь;
	КонецЕсли;
