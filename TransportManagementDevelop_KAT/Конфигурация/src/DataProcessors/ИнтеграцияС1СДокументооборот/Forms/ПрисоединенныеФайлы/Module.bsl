...
&НаКлиенте
Перем ПоследняяСтрокаПросмотраPDF;
 
&НаКлиенте
Перем ДокументPDFПросмотрОкно;
...
Процедура ФайлыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ТекущиеДанные <> Неопределено И НРег(ТекущиеДанные.Расширение) = "pdf" Тогда
		гкс_ПоказатьPDFВОкне(ТекущиеДанные);
	Иначе
		ОткрытьНаЧтение(Неопределено);
	КонецЕсли;
...
Процедура ФайлыПриАктивизацииСтроки(Элемент)
	...
	гкс_ПриАктивизацииСтрокиОткрытьPDF();
КонецПроцедуры
...
Процедура ОбновитьПредпросмотр(ТекущаяСтрока)
	...
	Если НРег(ТекущиеДанные.Расширение) = "pdf" Тогда
		...
		АдресДанныхФайла = "";
		Элементы.КартинкаПредпросмотра.ТекстНевыбраннойКартинки =
			НСтр("ru = 'PDF откроется в отдельном окне при выборе строки или двойном клике.'");
		Возврат;
	КонецЕсли;
	...
	Иначе
		АдресДанныхФайла = "";
		Элементы.КартинкаПредпросмотра.ТекстНевыбраннойКартинки = 
			НСтр("ru = 'Предпросмотр файлов этого типа не поддерживается.'");
	КонецЕсли;
...
// Открыть PDF в отдельном окне просмотра 1С (ДокументPDF.Показать).
&НаКлиенте
Процедура гкс_ПоказатьPDFВОкне(ТекущиеДанные)

	АдресФайла = ТекущиеДанные.АдресДанныхФайла;
	...
	// Чтение PDF из потока без временного файла.
	Попытка	
		ДвоичныеДанные = ПолучитьИзВременногоХранилища(АдресФайла);
		ПотокПросмотраPDF = ДвоичныеДанные.ОткрытьПотокДляЧтения();
		ДокументPDFПросмотрОкно = Новый ДокументPDF;
		ДокументPDFПросмотрОкно.Прочитать(ПотокПросмотраPDF);
        ДокументPDFПросмотрОкно.Показать(ТекущиеДанные.Наименование, ТекущиеДанные.Наименование);
	Исключение
		КодЯзыка = ОбщегоНазначенияКлиент.КодОсновногоЯзыка();
		ИмяСобытия = НСтр("ru = 'Интеграция с 1С:Документооборот.Просмотр PDF'", КодЯзыка);
		ИнформацияОшибки = ИнформацияОбОшибке();
		КомментарийОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОшибки);
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ИмяСобытия, "Ошибка", КомментарийОшибки, "", Истина);
		ПоказатьПредупреждение(, НСтр("ru = 'Не удалось открыть PDF для просмотра. Подробности в журнале регистрации.'"));
	КонецПопытки;


КонецПроцедуры

// При активации PDF-строки автоматически открываем просмотр поверх текущей формы.
&НаКлиенте
Процедура гкс_ПриАктивизацииСтрокиОткрытьPDF()
	...
	Если ПоследняяСтрокаПросмотраPDF = ТекущиеДанные.ID Тогда
		Возврат;
	КонецЕсли;
	ПоследняяСтрокаПросмотраPDF = ТекущиеДанные.ID;
