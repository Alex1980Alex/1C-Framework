...
&НаКлиенте
Процедура ОтборСобственникПриИзменении(Элемент)
...
Процедура КомандаСформироватьПробы(Команда)
	...
	Иначе		
		...
		ДопПараметры.Вставить("Собственник",			ОтборСобственник);
		...
		// Передача флага детализации по пункту погрузки
		Если ПроверитьРеквизитФормы("ДетализацияПоПунктуПогрузки") Тогда
			ДопПараметры.Вставить("ДетализацияПоПунктуПогрузки", ЭтотОбъект.ДетализацияПоПунктуПогрузки);
		КонецЕсли;
...
Процедура КомандаСформироватьЗПП3(Команда)
	...
	ДополнитьЗначенияЗаполненния(ЗначенияЗаполнения);
...
Процедура УстановитьОтборДинамическихСписков()
	...
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(СписокРегистрации, "Собственник", 
		ОтборСобственник, ВидСравненияКомпоновкиДанных.Равно, , ЗначениеЗаполнено(ОтборСобственник));
...
Процедура КомандаЗаполнитьСредневзвешеннымиНаСервере(ЛабораторныйАнализ)
	...
	Если ЗначениеЗаполнено(СредневзвешенныеПоказатели) Тогда
		...
		ДокументАнализаОбъект.ЗаполнитьПоказателиДляИсследования();
		ДокументАнализаОбъект.ЗаполнитьСредневзвешенныеПоказатели(СредневзвешенныеПоказатели);
		
		ДокументАнализаОбъект.ЗаписатьДокументПослеОбработки(Истина);
	КонецЕсли;	
...
Процедура СоздатьПереключателиРежимаГруппировки()
	...
	// Программно добавляем атрибут для чекбокса
	НовыйАтрибут = Новый РеквизитФормы("ДетализацияПоПунктуПогрузки", Новый ОписаниеТипов("Булево"), ,
		"Детализировать по пункту погрузки");
	МассивАтрибутов = Новый Массив;
	МассивАтрибутов.Добавить(НовыйАтрибут);
	ИзменитьРеквизиты(МассивАтрибутов); 

	// Создаем чекбокс "Детализировать по пункту погрузки"
	ЧекбоксДетализации = Элементы.Добавить("ДетализацияПоПунктуПогрузки", Тип("ПолеФормы"), Элементы.ГруппаНастройки);  
	ЧекбоксДетализации.ПутьКДанным = "ДетализацияПоПунктуПогрузки";  
	ЧекбоксДетализации.Вид = ВидПоляФормы.ПолеФлажка;  
	ЧекбоксДетализации.Заголовок = "Детализировать по пункту погрузки";
	ЧекбоксДетализации.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Право;
	
	// Инициализируем значение по умолчанию (включено)
	ЭтотОбъект.ДетализацияПоПунктуПогрузки = Истина;
КонецПроцедуры
...
&НаКлиенте
Процедура ДополнитьЗначенияЗаполненния(ЗначенияЗаполнения)
	
	ТекущиеДанные = Элементы.СписокКомпозитныхПроб.ТекущиеДанные;
	
	Если ТекущиеДанные <> Неопределено Тогда
		ДополнитьПунктомПогрузки(ТекущиеДанные.ДокументКомпозитнаяПроба, ЗначенияЗаполнения);
	КонецЕсли;
	
КонецПроцедуры

&НаСервере 
Процедура ДополнитьПунктомПогрузки(ДокументКомпозитнаяПроба, ЗначенияЗаполнения)
	
	Запрос = Новый Запрос;
	
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	КомпозитныеПробы.ДокументРегистрации КАК ДокументРегистрации,
	|	ЕСТЬNULL(гкс_РегистрацияНаПЛК.ПунктПогрузки, ЗНАЧЕНИЕ(Справочник.гкс_ТочкиМаршрута.ПустаяСсылка)) КАК ПунктПогрузки
	...
	|		ПО КомпозитныеПробы.ДокументРегистрации = гкс_РегистрацияНаПЛК.Ссылка
	|ГДЕ
	|	КомпозитныеПробы.ТочкаМаршрута = &ТочкаМаршрута
	|	И КомпозитныеПробы.ПроизводственныеСуткиРегистрации = &ПроизводственныеСутки
	|	И КомпозитныеПробы.ВидПеревозки = &ВидПеревозки
	|	И КомпозитныеПробы.КомпозитнаяПроба = &ДокументКомпозитнаяПроба
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ВТ_СписокРегистраций.ПунктПогрузки) КАК КоличествоСтрок
	|ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ ПЕРВЫЕ 1
	|	ВТ_СписокРегистраций.ПунктПогрузки КАК ПунктПогрузки
	|ИЗ
	|	ВТ_СписокРегистраций КАК ВТ_СписокРегистраций";
	
	Запрос.УстановитьПараметр("ТочкаМаршрута", Объект.ТочкаМаршрута);
	Запрос.УстановитьПараметр("ПроизводственныеСутки", Сутки);
	Запрос.УстановитьПараметр("ВидПеревозки", Объект.ВидПеревозки);
	Запрос.УстановитьПараметр("ДокументКомпозитнаяПроба", ДокументКомпозитнаяПроба);
	
	Результаты = Запрос.ВыполнитьПакет();
	
	ВыборкаКоличествоСтрок = Результаты[1].Выбрать();
	ВыборкаКоличествоСтрок.Следующий();
	
	КоличествоСтрок = ВыборкаКоличествоСтрок.КоличествоСтрок;
	
	КоличествоНужныхСтрок = 1;
	
	Если КоличествоСтрок = КоличествоНужныхСтрок Тогда
		
		ВыборкаПунктПогрузки = Результаты[2].Выбрать();
		ВыборкаПунктПогрузки.Следующий();
		ЗначенияЗаполнения.Вставить("ПунктПогрузки", ВыборкаПунктПогрузки.ПунктПогрузки);
