...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	ИначеЕсли УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПФ_MXL_АктЗабраковкиТовара_ru") Тогда
		...
		ТабличныйДокументАктЗабраковкиТовара = СформироватьПечатнуюФормуАктЗабраковкиТовара(СтруктураТипов, 
			ПараметрыПечати);
...
Процедура ЗаполнитьОсобыеОтметки(ОбластьПодвал, ПараметрыПоказателей)
	...
	Для Индекс = 0 По ПоказателиОсобыеОтметки.Количество() - 1 Цикл
		...
		Если ВыборкаПоказатели.НайтиСледующий(СтруктураПоиска) Тогда 
			...
			СтрокаОсобыеОтметки 
				= СтрШаблон("%1%2 %3%4, ", СтрокаОсобыеОтметки, Показатель, 
					Формат(ВыборкаПоказатели.Значение, "ЧДЦ=2; ЧН=0.00;"), СтрокаПоказатель.ЕдиницаИзмерения);
			Продолжить;	
		КонецЕсли;
		 СтрокаОсобыеОтметки 
...
#Область ПечатнаяФормаЯрлыкПробы

// Сформировать печатную форму ярлык пробы.
// 
// Параметры:
//  СтруктураТипов - Соответствие - Структура типов
//  ПараметрыПечати - Структура - Параметры печати:
// * ТекстПредупреждения - Строка - 
// * КлючОбъекта - Строка - 
// * СинонимЛабАнализа - Строка - 
// 
// Возвращаемое значение:
//  ТабличныйДокумент - Сформировать печатную форму ярлык пробы
Функция СформироватьПечатнуюФормуЯрлыкПробы(СтруктураТипов, ПараметрыПечати) Экспорт
	
	ТабличныйДокумент = Новый ТабличныйДокумент;
	ТабличныйДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ЯрлыкПробы";
	...
	Для Каждого СтруктураОбъектов Из СтруктураТипов Цикл
		...
		Если СтруктураОбъектов.Ключ <> ПараметрыПечати.КлючОбъекта Тогда 
			...
			ТекстСообщения = 
			СтрШаблон(ПараметрыПечати.ТекстПредупреждения, "Ярлык пробы", ПараметрыПечати.СинонимЛабАнализа);
		...
		Для Каждого ДокументОснование Из СтруктураОбъектов.Значение Цикл
			...
			ДанныеДляПечати = ПолучитьДанныеДляПечатиЯрлыкПробы(ДокументОснование);
			Если НЕ ДанныеДляПечати = Неопределено Тогда
				ЗаполнитьТабличныйДокументЯрлыкПробы(ТабличныйДокумент, ДанныеДляПечати, ПараметрыПечати);
			КонецЕсли;
...
Функция ПолучитьДанныеДляПечатиЯрлыкПробы(ДокументОснование)
	
	Запрос = Новый Запрос;
	Запрос.Текст = ТекстЗапросаЯрлыкПробы();
	Запрос.УстановитьПараметр("ДокументОснование", ДокументОснование);
	...
	Результат = Новый Структура();
	Результат.Вставить("Шапка", РезультатЗапроса);
...
Процедура ЗаполнитьТабличныйДокументЯрлыкПробы(ТабличныйДокумент, ДанныеДляПечати, ПараметрыПечати)
	...
	Макет = ПолучитьМакет("ПФ_MXL_ЯрлыкПробы");
	...
	ОбластьШапка.Параметры.ДатаОтбора  = Формат(ВыборкаПоШапке.Дата, "Л=ru_RU; ДФ=""d MMMM yyyy 'г.'""");
	...
	Если ВыборкаПоШапке.ГодУрожая > 0 Тогда
		Сырье = СтрШаблон("%1 %2", Сырье, Формат(ВыборкаПоШапке.ГодУрожая, "ЧГ="));
	КонецЕсли;
...
Функция ТекстЗапросаЯрлыкПробы()
	
	Возврат	
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ДокументЛабораторныйАнализ.Ссылка КАК Ссылка,
	|	ДокументЛабораторныйАнализ.Дата КАК Дата,
	|	ДокументЛабораторныйАнализ.НомерПробы КАК НомерПробы,
	|	ЕСТЬNULL(СправочникНоменклатура.Наименование, """") КАК Номенклатура,
	|	ДокументЛабораторныйАнализ.ГодУрожая КАК ГодУрожая,
	|	ДокументЛабораторныйАнализ.Примечание КАК Комментарий,
	...
	|	ЕСТЬNULL(СправочникКонтрагенты.НаименованиеПолное, """") КАК Поставщик,
	|	ЕСТЬNULL(СправочникТранспортныеСредства.НомерТранспортногоСредства, """") КАК НомерТС,
	|	ЕСТЬNULL(РегистрацияНаПЛК.ВесНетто, 0) КАК МассаПартии
	|ИЗ
	|	Документ.гкс_ЛабораторныйАнализ КАК ДокументЛабораторныйАнализ
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК СправочникНоменклатура
	|		ПО ДокументЛабораторныйАнализ.Номенклатура = СправочникНоменклатура.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Организации КАК Организации
	|		ПО ДокументЛабораторныйАнализ.Организация = Организации.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.гкс_РегистрацияНаПЛК КАК РегистрацияНаПЛК
	|		ПО ДокументЛабораторныйАнализ.гкс_ДокументРегистрации = РегистрацияНаПЛК.Ссылка
	...
	|		ПО РегистрацияНаПЛК.ТранспортноеСредство = СправочникТранспортныеСредства.Ссылка
	|ГДЕ
	|	ДокументЛабораторныйАнализ.Ссылка = &ДокументОснование";
