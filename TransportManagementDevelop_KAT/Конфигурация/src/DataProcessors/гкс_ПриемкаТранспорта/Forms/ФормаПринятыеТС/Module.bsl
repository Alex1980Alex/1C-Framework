...
Процедура ПриОткрытии(Отказ)
	...
	Если Не ЕстьПравоНаПросмотрРеестр Тогда
		ТекстСообщения = НСтр("ru = 'У пользователя нет доступа к редактированию Реестр накладных ЗПП-3'");
...
Процедура ДополнительныйСписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		ИначеЕсли Поле.Имя = "ДополнительныйСписокНомерПробы" Тогда

		Если ЗначениеЗаполнено(ТекущиеДанные.ДокументФНП) Тогда
			гкс_ПриемкаНаПЛККлиент.ОткрытьДокумент(ЭтотОбъект, ТекущиеДанные.ДокументФНП, 
...
Процедура КомандаСформироватьЗПП3(Команда)
	...
	ЗначенияЗаполнения = Новый Структура("КомпозитнаяПроба, Анализ", ТекущиеДанные.ДокументФНП,
		ТекущиеДанные.ЛабораторныйАнализ);
		
	ЗначенияЗаполнения.Вставить("ПунктПогрузки", ТекущиеДанные.ПунктПогрузки);
...
Процедура ИнициализироватьРеквизитыПравПользователя()
	...
	ЕстьПравоНаРедактированиеРеестр = ЕстьПравоНаРедактирование("гкс_РеестрНакладныхЗПП3");	
	ЕстьПравоНаПросмотрРеестр = ЕстьПравоНаПросмотр("гкс_РеестрНакладныхЗПП3");	
...
Процедура УстановитьОтборДинамическихСписковДополнительныйСписок()
	...
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(ДополнительныйСписок, "Собственник", 
...
&НаСервере
Функция ЕстьПравоНаПросмотр(ИмяОбъекта)
	Возврат ПравоДоступа("Просмотр", Метаданные.Документы[ИмяОбъекта]);
КонецФункции                     
