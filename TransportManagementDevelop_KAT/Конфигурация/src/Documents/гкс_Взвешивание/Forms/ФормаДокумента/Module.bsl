...
Процедура ВвестиВесВручную(Команда)
	...
	Если Объект.Проведен Тогда
		ТекстСообщения = НСтр("ru = 'ВНИМАНИЕ! Изменения будут произведены только в базе Управление транспортом,
		|в базе МФМ необходимо изменить вес вручную в документе Движение запасов'");
...
Процедура УстановитьВидимостьДоступностьКомандСервисаВзвешивания()
	...
	Если ЗначениеЗаполнено(Объект.Весы) Тогда
		...
		УдалитьОчередьОтветаВидимость = АвтоУдалениеОчередиОтвета = Ложь;
	КонецЕсли;
...
Процедура ПолучитьВесНаСервере(Ошибки)
	...
	Если ЗначениеЗаполнено(Объект.Весы) Тогда
		...
		Попытка
			РезультатыВзвешивания = гкс_ОбменДаннымиСВесами.РезультатыВзвешивания(УникальныйИдентификатор, Объект.Весы);
		Исключение
			РезультатыВзвешивания = Новый Массив;
		...
		Если РезультатыВзвешивания.Количество() > 0 Тогда
			
			ДанныеСВесов = РезультатыВзвешивания[0];
			Объект.Вес 				= ДанныеСВесов.Вес;
			Объект.ВесНеСтабилен 	= ДанныеСВесов.ВесНеСтабилен;
