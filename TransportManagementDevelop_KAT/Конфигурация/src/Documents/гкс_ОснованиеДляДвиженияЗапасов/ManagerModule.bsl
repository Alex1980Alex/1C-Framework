...
// Регистрирует документ-основание движения запасов к обмену RMQ по документу регистрации.
//
// Параметры:
// ДокументРегистрации - ДокументСсылка.гкс_РегистрацияНаПЛК - регистрация, по которой ищется основание
//
Процедура ЗарегистрироватьВОбменRMQ(ДокументРегистрации) Экспорт

	Ссылка = ПолучитьПоРегистрации(ДокументРегистрации);
	Если Не ЗначениеЗаполнено(Ссылка) Тогда
		Возврат;
	КонецЕсли;

	Объект = Ссылка.ПолучитьОбъект();
	гкс_ОчередьСообщенийRMQ.ИнициализироватьДополнительныеСвойстваОбъекта(Объект);
	гкс_ОчередьСообщенийRMQ.СоздатьСообщенияПоСобытиюОбъекта("ЗаписьОбъекта", Объект);
