...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	ДанныеЗаполнения.Вставить("ДокументРегистрации", ТекущийОбъект.Ссылка);
	ДанныеЗаполнения.Вставить("ТранспортныйДокумент", ТранспортныйДокумент);
	ДанныеЗаполнения.Вставить("ТранспортноеСредство", ТекущийОбъект.ТранспортноеСредство);
	ДанныеЗаполнения.Вставить("ЗаявкаНаПеревозку", ЗаявкаНаПеревозку);
	
	гкс_ПриемкаТранспортаВызовСервера.УстановитьСвязьРегистрацииИТранспортногоДокумента(ДанныеЗаполнения);			
...
Процедура НоменклатураПриИзмененииНаСервере()
	...
	УстановитьВидимостьУдостоверенияКачества();
КонецПроцедуры
...
&НаКлиенте
Процедура СписокВнешниеПробыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ПоказатьЗначение( , ТекущиеДанные.ВнешняяПроба);
...
&НаКлиенте
Процедура ДекорацияУдостоверениеКачестваНажатие(Элемент)
	
	ОповещениеОЗакрытии = Новый ОписаниеОповещения("ПослеЗакрытияФормыУдостоверения", ЭтотОбъект);
	гкс_ПриемкаТранспортаСлужебныйКлиент.ОткрытьФормуУдостоверения(ЭтотОбъект, ОповещениеОЗакрытии);
	
КонецПроцедуры

&НаКлиенте
Процедура ПослеЗакрытияФормыУдостоверения(Результат, ДополнительныеПараметры) Экспорт
	
	ОбновитьПредставлениеУдостоверенияКачества();
...
&НаСервере
Функция ИспользоватьСтандартнуюОбработкуВыбораСДИЗВДокументеРегистрацияПЛК()
	Возврат ПолучитьФункциональнуюОпцию("гкс_ИспользоватьСтандартнуюОбработкуВыбораСДИЗВДокументеРегистрацияПЛК");
КонецФункции
...
Процедура ИнициализироватьРеквизитыФормы()
	...
	ИнициализироватьГруппуТС();
	
	Если ЭтоНовыйДокумент() Тогда
		ИнициализироватьЧасовойПоясПЛК();
	Иначе
		Если ОтражатьКомментарийОтклоненияПогрузки(Объект.ТочкаМаршрута, 
			Объект.ВидПеревозки, Объект.ТипРегистрации) Тогда
			
			КомментарийОтклонениеПогрузки = КомментарийОтклоненияПогрузки(Объект.Ссылка);
		КонецЕсли;		
	КонецЕсли;
...
&НаСервере
Процедура ИнициализироватьГруппуТС()
	
	Если ПолучитьФункциональнуюОпцию("гкс_ИспользоватьУсиленныйКонтрольКачества") Тогда
		ГруппаТС = Справочники.гкс_ГруппыТС.ГруппаРегистрации(Объект.Ссылка);		
	КонецЕсли;
...
Процедура УстановитьВидимостьДоступностьЭлементовФормы()
	...
	УстановитьВидимостьЗаявкиНаПеревозку();
...
&НаСервере
Процедура УстановитьВидимостьЗаявкиНаПеревозку()
		
	Элементы.ЗаявкаНаПеревозку.Видимость = ЭтоПриемка;
	Элементы.ГруппаВнешниеПробы.Видимость = ЭтоПриемка;
...
Процедура СДИЗУстановитьПараметрыВыбораНаСервере()
	...
	Если Не ИспользоватьСтандартнуюОбработкуВыбораСДИЗВДокументеРегистрацияПЛК() Тогда
...
Функция АвтоЗаполнениеТраснпортногоДокумента()
	...
	Если Объект.ТипРегистрации = Перечисления.гкс_ТипРегистрации.Отгрузка Тогда
		Возврат Ложь;
	КонецЕсли;
...
Процедура ЗаполнитьРеквизитыДокументаПоставщикаПоУмолчанию()
	...
	РеквизитыТД = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
		ТранспортныйДокумент, "НомерДокумента, Номер, ДатаДокумента, Дата, ПунктПогрузки");
	...
	Если ЗначениеЗаполнено(РеквизитыТД.ПунктПогрузки) И Не ЗначениеЗаполнено(Объект.ПунктПогрузки) Тогда
		Объект.ПунктПогрузки = РеквизитыТД.ПунктПогрузки;
	КонецЕсли;
...
&НаСервере
Процедура ЗаполнитьЗаявкуНаПеревозку()
	
	Если Объект.ТипРегистрации = Перечисления.гкс_ТипРегистрации.Приемка Тогда
		ЗаявкаНаПеревозку = гкс_ПриемкаТранспорта.
			ЗаявкаНаПеревозкуПоТранспортномуДокументу(ТранспортныйДокумент, Объект.ТранспортноеСредство);						
	Иначе	
		ЗаявкаНаПеревозку = "";
	КонецЕсли;	
...
Процедура УстановитьОтборыСпискаВнешниеПробы()
	...
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(СписокВнешниеПробы,
		"ВидПеревозки", Объект.ВидПеревозки, ВидСравненияКомпоновкиДанных.Равно);	
	
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(СписокВнешниеПробы,
		"Номенклатура", Объект.Номенклатура, ВидСравненияКомпоновкиДанных.Равно);
	
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(СписокВнешниеПробы,
		"ТранспортноеСредство", Объект.ТранспортноеСредство, ВидСравненияКомпоновкиДанных.Равно);
...
Процедура ЗаполнитьЗаписьВОчередь(ОбновитьРеквизитыДокументаПоставщика = Истина)
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) 
		И ЭтоАвтоперевозкаНаОснованииВъездаНаКПП() Тогда
		
		ИнформацияОПредварительнойЗаписи = гкс_ПриемкаТранспортаВызовСервера.ИнформацияОПредварительнойЗаписи(
			Объект.Ссылка);
...
Процедура УстановитьВидимостьОтТипаРегистрации()
	...
	Элементы.КомментарийОтклонениеПогрузки.Видимость = 
		ОтражатьКомментарийОтклоненияПогрузки(Объект.ТочкаМаршрута, Объект.ВидПеревозки, Объект.ТипРегистрации);
...
Процедура УстановитьВидимостьОтВидаПеревозки()
	...
	Элементы.ЗаписьВОчередь.Видимость 		= ЭтоАвтоперевозкаНаОснованииВъездаНаКПП();
...
&НаСервере
Процедура УстановитьВидимостьУдостоверенияКачества()
	
	ЭлементДекорация = Элементы.Найти("ДекорацияУдостоверениеКачества");
	Если ЭлементДекорация = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Видимость = гкс_ВходнойКонтрольКачества.ТребуетсяУдостоверениеКачества(
		Объект.Номенклатура, Объект.ВидПеревозки, Объект.ТипРегистрации);
	
	ЭлементДекорация.Видимость = Видимость;
...
&НаСервере
Процедура ОбновитьПредставлениеУдостоверенияКачества()
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		ЭлементДекорация.Заголовок = НСтр("ru = 'Внести данные удостоверения качества'");
		Возврат;
	КонецЕсли;
	
	Данные = гкс_ВходнойКонтрольКачества.ДанныеУдостоверенияКачества(Объект.Ссылка);
	Если Данные = Неопределено Или Не ЗначениеЗаполнено(Данные.ДатаУдостоверения) Тогда
		ЭлементДекорация.Заголовок = НСтр("ru = 'Внести данные удостоверения качества'");
	Иначе
		ПредставлениеУК = СтрШаблон(
			НСтр("ru = '№ %1 от %2'"),
			Формат(Данные.НомерУдостоверения, "ЧЦ=5; ЧВН="),
			Формат(Данные.ДатаУдостоверения, "ДФ=dd.MM.yyyy"));
		ЭлементДекорация.Заголовок = СтрШаблон(
			НСтр("ru = 'Удостоверение качества: %1'"), ПредставлениеУК);
	КонецЕсли;
...
&НаСервере
Функция ЭтоАвтоперевозкаНаОснованииВъездаНаКПП()
	Возврат ЭтоАвтоПеревозка(Объект.ВидПеревозки)
		И ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.гкс_ВъездНаКПП");
КонецФункции

&НаСервереБезКонтекста
Функция ОтражатьКомментарийОтклоненияПогрузки(ТочкаМаршрута, ВидПеревозки, ТипРегистрации)
	ЕстьНормативПогрузки = РегистрыСведений.гкс_НастройкиПриемкаПЛКПоТочкамМаршрута.НормативОтгрузки(ТочкаМаршрута) <> 0;
	ЭтоАвтоПеревозка = ЭтоАвтоПеревозка(ВидПеревозки);
	Возврат
		ТипРегистрации = Перечисления.гкс_ТипРегистрации.Отгрузка
		И ЭтоАвтоПеревозка
		И ЕстьНормативПогрузки; 
КонецФункции 

&НаСервереБезКонтекста
Функция КомментарийОтклоненияПогрузки(ДокументРегистрации) 
	ДанныеРегистрации =
		РегистрыСведений.гкс_ВремяНахожденияТСНаПЛК.ДанныеПоВремениНахожденияНаПЛК(ДокументРегистрации);
	Возврат 
		ДанныеРегистрации.Комментарий;
КонецФункции
