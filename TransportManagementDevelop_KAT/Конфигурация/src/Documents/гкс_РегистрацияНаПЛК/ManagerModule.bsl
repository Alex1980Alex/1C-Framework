...
// Приводит изменение реквизитов документа новыми значениями.
//
// Параметры:
//  Документ - ДокументСсылка.гкс_РегистрацияНаПЛК - ссылка на изменяемый документ
//	ЗначенияЗаполнения - Структура - структура с ключами соответствующим именам реквизитов
//
Процедура ИзменитьРеквизитыДокумента(Документ, ЗначенияЗаполнения) Экспорт
	...
	Исключение
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Не удалось записать %1. %2'"), 
			ДокументОбъект, ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		
		ОбщегоНазначения.СообщитьПользователю(ТекстОшибки, ДокументОбъект); 
			
		ЗаписьЖурналаРегистрации("РегистрацияНаПЛК.Запись", 
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка, ДопПараметры)
	...
	Запрос.УстановитьПараметр("КомментарийПоУмолчанию",
		НСтр("ru = 'Автоматически при создании регистрации на ПЛК'"));
...
Функция ТекстЗапросаТаблицаСтатусыДопускаВагоновКВскрытию(ТекстыЗапроса, Регистры)
	...
	ИспользоватьЗапрет = ?(ПолучитьФункциональнуюОпцию("гкс_ИспользоватьАвтоустановкуЗапретаВскрытия"),
		"ИСТИНА", "ЛОЖЬ");
	...
	|
	|ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(Перечисление.гкс_ДопускиКВскрытиюВагонов.Ветеринарный)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	|	Регистрация.Ссылка КАК ДокументРегистрации,
	|	Регистрация.Дата КАК Период,
	|	Регистрация.Контрагент КАК Поставщик,
	|	ЕСТЬNULL(ТочкиМаршрута.КодСтанцииЖД, ЗНАЧЕНИЕ(Справочник.гкс_КлассификаторСтанцийЖД.ПустаяСсылка)) КАК ЖДСтанция
	...
	|		ПО Регистрация.ПунктПогрузки = ТочкиМаршрута.Ссылка
	|ГДЕ
	|	Регистрация.Ссылка = &Ссылка
	|;
	|
	...
	|	ВЫБОР
	|		КОГДА Настройки.Номенклатура ЕСТЬ NULL
	|			ТОГДА ВЫБОР
	|				КОГДА ВТ_НачальныеДопуски.ДопускКВскрытию = ЗНАЧЕНИЕ(Перечисление.гкс_ДопускиКВскрытиюВагонов.Претензионный)
	|					ТОГДА ИСТИНА
	|				КОГДА ЕСТЬNULL(Настройки.ДопускКВскрытию,
	|					ЗНАЧЕНИЕ(Перечисление.гкс_ДопускиКВскрытиюВагонов.ПустаяСсылка)) = ЗНАЧЕНИЕ(Перечисление.гкс_ДопускиКВскрытиюВагонов.Фитосанитарный)
	|					ТОГДА ИСТИНА
	|				КОГДА ЕСТЬNULL(Настройки.ДопускКВскрытию,
	|					ЗНАЧЕНИЕ(Перечисление.гкс_ДопускиКВскрытиюВагонов.ПустаяСсылка)) = ЗНАЧЕНИЕ(Перечисление.гкс_ДопускиКВскрытиюВагонов.Фумигационный)
	|					ТОГДА ИСТИНА
	|				КОГДА ЕСТЬNULL(Настройки.ДопускКВскрытию,
	|					ЗНАЧЕНИЕ(Перечисление.гкс_ДопускиКВскрытиюВагонов.ПустаяСсылка)) = ЗНАЧЕНИЕ(Перечисление.гкс_ДопускиКВскрытиюВагонов.Ветеринарный)
	|					ТОГДА ИСТИНА
	|				ИНАЧЕ ЛОЖЬ
	|			КОНЕЦ
	|		ИНАЧЕ ИСТИНА
	...
	|	&КомментарийПоУмолчанию КАК Комментарий,
	|	&Пользователь КАК Ответственный
	|ИЗ
	|	ВТ_НачальныеДопуски КАК ВТ_НачальныеДопуски
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.гкс_НастройкиЗапретаВскрытияПоНоменклатуре КАК Настройки
	|		ПО ИСТИНА = ИСТИНА
	|		И Настройки.Номенклатура = ВТ_НачальныеДопуски.Номенклатура
	|		И Настройки.ДопускКВскрытию = ВТ_НачальныеДопуски.ДопускКВскрытию
	|		И Настройки.Активен
	|		И Настройки.ДатаНачала <= ВТ_НачальныеДопуски.Период
	|		И ВТ_НачальныеДопуски.Период <= ВЫБОР
	|			КОГДА Настройки.ДатаОкончания = ДАТАВРЕМЯ(1, 1, 1)
	|				ТОГДА ДАТАВРЕМЯ(3999, 12, 31)
	|			ИНАЧЕ Настройки.ДатаОкончания
	|		КОНЕЦ
	|		И ВЫБОР
	|			КОГДА Настройки.ТранспортноеСредство = ЗНАЧЕНИЕ(Справочник.ТранспортныеСредства.ПустаяСсылка)
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ Настройки.ТранспортноеСредство = ВТ_НачальныеДопуски.Вагон
	|		КОНЕЦ
	|		И ВЫБОР
	|			КОГДА Настройки.Поставщик = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ Настройки.Поставщик = ВТ_НачальныеДопуски.Поставщик
	|		КОНЕЦ
	|		И ВЫБОР
	|			КОГДА Настройки.ЖДСтанция = ЗНАЧЕНИЕ(Справочник.гкс_КлассификаторСтанцийЖД.ПустаяСсылка)
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ Настройки.ЖДСтанция = ВТ_НачальныеДопуски.ЖДСтанция
	|		КОНЕЦ";

	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ИСТИНА = ИСТИНА", ИспользоватьЗапрет);
