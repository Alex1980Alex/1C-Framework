...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура") Тогда
		...
		Если ДанныеЗаполнения.Свойство("ДокументОснование") Тогда
			...
			ИначеЕсли ТипОснования = Тип("ДокументСсылка.гкс_ЗаписьВОчередьПриемкиПЛК") Тогда
				
				ЗаполнитьДокументНаОснованииЗаписиВОчередь(ДанныеЗаполнения.ДокументОснование);
				
			ИначеЕсли ТипОснованияПрибытиеВагонаИлиСудна(ТипОснования)
				И ДанныеЗаполнения.Свойство("ТранспортныйДокумент") Тогда
					
				ЗаполнитьДокументНаОснованииПрибытияВагонаПрибытияСудна(ДанныеЗаполнения);
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	ЗаполнитьЕдиницуИзмерения();
	ЗаполнитьТипДокументаОснования();
...
Процедура ПриЗаписи(Отказ)
	...
	ДобавитьЗаписьРегистрацииПоЭлектроннойОчереди();
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если гкс_ВходнойКонтрольКачества.ТребуетсяУдостоверениеКачества(Номенклатура, ВидПеревозки, ТипРегистрации) Тогда
		
		Если Не ЗначениеЗаполнено(Ссылка) Тогда
			ОбщегоНазначения.СообщитьПользователю(
				НСтр("ru = 'Для контролируемой номенклатуры (приёмка ЖД) после записи документа
				             |необходимо внести данные удостоверения качества и безопасности.'"),,,,Отказ);
		ИначеЕсли Не гкс_ВходнойКонтрольКачества.УдостоверениеКачестваЗаполнено(Ссылка) Тогда
			ТекстОшибкиУдостоверение = НСтр("ru = 'Не заполнено удостоверение качества и безопасности
				                                  |(номер, дата удостоверения, 
				                                  |дата выработки, срок хранения, срок годности).'");
			ОбщегоНазначения.СообщитьПользователю(ТекстОшибкиУдостоверение,,,,Отказ);
		КонецЕсли;
...
Процедура ЗаполнитьДокументНаОснованииЗаписиВОчередь(Знач ДокументОснование)

	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Ссылка", ДокументОснование);
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ ПЕРВЫЕ 1
	|	ЗаписьВОчередьПриемкиПЛК.Номенклатура КАК Номенклатура,
	|	ЗаписьВОчередьПриемкиПЛК.УсловиеПроезда КАК УсловиеПроезда,
	|	ВЫБОР
	|		КОГДА ЗаписьВОчередьПриемкиПЛК.УсловиеПроезда = ЗНАЧЕНИЕ(Справочник.гкс_УсловияПроезда.FCA)
	|			ТОГДА ЗаписьВОчередьПриемкиПЛК.Контрагент
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|	КОНЕЦ КАК Контрагент,
	|	ВЫБОР
	|		КОГДА ЗаписьВОчередьПриемкиПЛК.УсловиеПроезда = ЗНАЧЕНИЕ(Справочник.гкс_УсловияПроезда.DAP)
	|			ТОГДА ЗаписьВОчередьПриемкиПЛК.Контрагент
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|	КОНЕЦ КАК ОтправительПолучатель,
	|	ЗаписьВОчередьПриемкиПЛК.НомерТранспортногоСредства КАК НомерТранспортногоСредства
	|ИЗ
	|	Документ.гкс_ЗаписьВОчередьПриемкиПЛК КАК ЗаписьВОчередьПриемкиПЛК
	|ГДЕ
	|	ЗаписьВОчередьПриемкиПЛК.Ссылка = &Ссылка";
	РезультатЗапроса = Запрос.Выполнить();
	ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
	ВыборкаДетальныеЗаписи.Следующий();
	ЗаполнитьЗначенияСвойств(ЭтотОбъект, ВыборкаДетальныеЗаписи);
	КоличествоСимволовНомераТранспорта = гкс_ПриемкаТранспорта.КоличествоСимволовДляПоискаПоНомеру();
	ТранспортноеСредство = Справочники.ТранспортныеСредства.ТранспортноеСредствоПоЧастиНомера(
		ВыборкаДетальныеЗаписи.НомерТранспортногоСредства, КоличествоСимволовНомераТранспорта);
...
Процедура ЗаполнитьДокументНаОснованииПрибытияВагонаПрибытияСудна(ДанныеЗаполнения)
	...
	Если ДанныеЗаполнения.Свойство("НомерВагона") Тогда
		ТранспортноеСредство = Справочники.ТранспортныеСредства
			.ТранспортноеСредствоПоНомеру(ДанныеЗаполнения.НомерВагона);
	КонецЕсли;
	
	Если Не ДанныеЗаполнения.Свойство("ВесНетто") Тогда
...
Процедура ЗаполнитьТипДокументаОснования()
	
	Если ДокументОснование = Неопределено Тогда
		ТипДокументаОснования = Неопределено;
	Иначе 
		ТипДокументаОснования = ОбщегоНазначения.ИдентификаторОбъектаМетаданных(ТипЗнч(ДокументОснование));
	КонецЕсли;
...
Процедура ДобавитьЗаписьРегистрацииПоЭлектроннойОчереди()
	
	Если Не ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.гкс_ЗаписьВОчередьПриемкиПЛК") Тогда
		Возврат;
	КонецЕсли;
	
	УстановитьПривилегированныйРежим(Истина);
	ОтклонениеВМинутах = гкс_ЭлектроннаяОчередьСервер.ОтклонениеОтЗаписиВОчередь(ДокументОснование, Дата); 
	СтруктураЗаписи = Новый Структура;
	СтруктураЗаписи.Вставить("ЗаписьВОчередь"		, ДокументОснование);
	СтруктураЗаписи.Вставить("РегистрацияНаПЛК"		, Ссылка);
	СтруктураЗаписи.Вставить("ОтклонениеВМинутах"	, ОтклонениеВМинутах); 
	РегистрыСведений.гкс_РегистрацииПоЭлектроннойОчереди.ДобавитьЗапись(СтруктураЗаписи, Истина);
	
КонецПроцедуры

Функция ТипОснованияПрибытиеВагонаИлиСудна(ТипОснования)
	Возврат ТипОснования = Тип("ДокументСсылка.гкс_СРПВПрибытиеНаПуть")
				Или ТипОснования = Тип("ДокументСсылка.гкс_ПрибытиеСуднаНаПЛК");
КонецФункции
