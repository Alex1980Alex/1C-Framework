...
Открывает HTML-дашборд в браузере, автоматически запуская API-сервер если он не работает.
