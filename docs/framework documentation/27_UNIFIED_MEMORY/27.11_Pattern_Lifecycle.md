...
> **Операционные наблюдения (не дефект схемы):** (1) **TEI лежит** (`ModuleNotFoundError: shared.semantic_search`) → dense-плечи всегда пусты, surfacing работает lexical-only — стоит починить TEI для полного hybrid. (2) execution-cache hit-rate = **0%** (217/217 miss) — агрессивная epoch-инвалидация + уникальные промпты. (3) `fact-trace`-готовность: 12 `pattern_id` в ingestion ∩ 5 в lifecycle = **0** пересечений — харвестенные паттерны (UUID5(content_hash)) ещё не были reinforced; механизм сквозного трейса готов, но production-факт «harvest→reinforce» в окне пока не возник.
