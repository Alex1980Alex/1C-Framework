...
flowchart TD
  ...
  ARCH -->|yes| W["archived → hard-exclude (drop)"]
  ARCH -->|no| K["keep"]
  K --> FLT["client-side filter: effective ≥ min_confidence"]
...
```mermaid
flowchart TD
  P["UserPromptSubmit: prompt"] --> CK{"execution cache<br/>key = hash(top-8 salient tokens) + confidence-epoch"}
  CK -->|hit skip ~2s| INJ["inject cached top-k<br/>+ replay reinforcement"]
  CK -->|miss| ARMS["build 4 arms<br/>(§26 Q1: experience/conversation dropped 2026-06-03)"]
  ...
  ARMS --> A5["skill_learning — skill-learning JSONL (saved + pending)"]
  ...
  RRF --> TOPK["top-k → inject via stdout"]
...
stateDiagram-v2
  ...
  note right of ARCHIVED
    surfacing: archived hard-exclude (drop)
    apply_pattern оживляет (revive-on-recurrence)
...
================================================================================
  ...
  (3) READ -+----------------------------------------------------------------------+
   ...
   |        | surfacing        : memory-first-hook -> 4 arms -> RRF(k=60) -> inject |
   |        +----------------------------------+-----------------------------------+
  ...
  surfacing его hard-exclude (drop); любой apply_pattern оживляет (revive-on-recurrence).
...
### 9.1 Сверка схемы с реальными логами (2026-06-05, §27 observability)

Схема выше **сквозно проверена** против живых логов (`confidence-lifecycle.log`, `memory-first-surfacing.log`, `memory-ingestion.log`, `memory-maintenance-runs.jsonl`) через `scripts/memory_observability_report.py` + код `memory-first-hook.py`/`server.py`:

| Стадия схемы | Доказательство в логах | Вердикт |
|---|---|---|
| ① CREATE — старт 0.70 (Beta(7,3) prior) | `apply` event: `old_confidence=0.70` (старт с prior) | ✅ |
| ④ UPDATE — `(7+succ)/(10+succ+fail)` | `apply` 0.70→**0.7273** (=8/11, 1-й success); `reinforce` succ=4/fail=1 → **0.7333** (=11/15) | ✅ **точное совпадение** |
| ③ READ surfacing → RRF k=60 → inject | 217 invocations, injected 108 / no-results 109; `rrf_merge` отрабатывает | ✅ |
| graceful degradation (TEI down → lexical-only) | `tei=down` в **100%** (214/214), но injected 108× через `pattern_lexical` (ALWAYS-ON) | ✅ подтверждена |
| epoch-инвалидация (§6) | `confidence-epoch.txt` = `1780350350.58` (time.time-формат) | ✅ |
| ⑤ MAINTENANCE forget (invalidate-not-delete) | cadence runs=1, archive=**0** (bounded-growth ещё не сработал — 25 паттернов < порога) | ✅ (контрпримеров нет) |
| event-mix §22 | `save`·`apply`·`decay_sweep`·`delete`·`reinforce`·`session` — все стадии присутствуют | ✅ |

**Исправлено по результатам сверки:** §3.4 и §9 указывали «**5 arms**» с плечом `experience / conversation` — это **stale** (collections `experience_embeddings`/`conversation_memory` удалены §26 Q1, 2026-06-03). Логи подтверждают cutover: последняя запись с этими плечами `2026-06-03T01:15`, далее — `{skill, pattern_dense, pattern_lexical}`; код `SEMANTIC_COLLECTIONS = [skill_library, learned_patterns]`. Позже (roadmap 260611 P2.2) добавлено 4-е плечо `skill_learning` (JSONL saved+pending) — **актуально 4 arms** (`SURFACE_RRF_WEIGHTS`).

> **Операционные наблюдения:** (1) ✅ **TEI-import починен (2026-06-05):** причина `tei=down` была НЕ сервер, а namespace-collision — `src/shared/` (реальный пакет с `__init__.py`, без `semantic_search.py`) затенял собственный `.claude/hooks/shared/` хука при `sys.path.insert(0, src_path)` → `ModuleNotFoundError: shared.semantic_search`; это ломало и dense-плечи, и `shared.pattern_reinforce`. Фикс — `append` вместо `insert(0)` в `memory-first-hook.py` (hooks-dir остаётся на `sys.path[0]`). После фикса `tei=ok`, плечи `{skill:5, pattern_dense:5, pattern_lexical:3}`, qdrant=10 — полный hybrid восстановлен. (2) execution-cache hit-rate = **0%** (уникальные промпты + агрессивная epoch-инвалидация) — by design. (3) `fact-trace` разблокирован фиксом (1): тот же collision глушил `record_surfaced` (reinforcement не записывался) → теперь surfaced→reinforce→`pattern_id`-тред в lifecycle произойдёт по мере всплытия харвестенных паттернов. (4) **ForgetGate функционален** (проверено: оценил все 25 паттернов, 0 кандидатов — все ~0.70, не idle>180d, нет fail-floor): архив не сработал **by design**, не баг; для реального архива при появлении кандидатов гонять cadence с `--apply` / `MEMORY_MAINTENANCE_APPLY=1`.
