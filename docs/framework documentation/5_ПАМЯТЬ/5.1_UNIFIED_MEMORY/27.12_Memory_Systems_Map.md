...
flowchart TB
  ...
  subgraph SKILLSYS["Skill-система (260612: вход в память, НЕ через оркестратор)"]
    CAT[".claude/skills/ каталог<br/>(SKILL.md, источник истины)"]
    SKILLLIB["skill_library (Qdrant 4096d)<br/>= зеркало каталога"]
    CAT -->|"skills-harvester (Stop, инкремент)<br/>reindex_skill_library (каденс, mirror+prune)"| SKILLLIB
  end
  ...
  HOOK -.-> AI
  HOOK -.-> VEC
  HOOK -.->|"плечо skill (RRF 0.5,<br/>score-floor 0.55)"| SKILLLIB
  STOP --> AI
  MCP --> ROUTER
  MCP --> SEARCH
  ROUTER --> AI & VEC & SKILL
...
| **pdf-docs** | `docs` | Qdrant | `pdf_documents` + `wiki_pages_v1` (alias → `*_mrl_1024`, 1024d) | документация (RAG, immutable со стороны памяти — ADR P0.4 [260612](../../../roadmap/260612_ROADMAP_PDF_DOCS_FULL_VERIFICATION.md)) | ❌ |
| **skill_library** | — (вне UnifiedID) | Qdrant | `localhost:6333`, коллекция `skill_library` (4096d cosine, 85 pts) | **зеркало каталога `.claude/skills/`** (frontmatter + `content_hash`); писатели — `skills-harvester` (Stop, инкремент) и `scripts/reindex_skill_library.py` (каденс, mirror+prune призраков); читатель — surfacing-плечо `skill` в `memory-first-hook` (score-floor 0.55). [Roadmap 260612 Skill System](../../../roadmap/260612_ROADMAP_SKILL_SYSTEM_FULL_VERIFICATION.md) | ❌ |
...
> Второй путь чтения — **surfacing** (`memory-first-hook`, §3.4 [27.11](27.11_Pattern_Lifecycle.md)): свой облегчённый RRF (k=60, плечи skill/pattern_dense/pattern_lexical/skill_learning) + confidence-gating + execution-cache. Он бьёт по store'ам напрямую (не через оркестратор) ради hot-path-латентности (<3s). **Плечо `skill` (260612 S4-фикс):** абсолютный cosine-floor `SKILL_SURFACE_MIN_SCORE=0.55` (env `MEMORY_SURFACE_SKILL_MIN_SCORE`, 0 = откат) — без него плечо отдавало топ-5 на ЛЮБОЙ промпт (generic-скиллы на 0.45-0.49 вытесняли реальные паттерны из баннера); калибровка живьём: релевантные хиты 0.56-0.62, нерелевантные 0.45-0.49; отсев виден в trace-логе как `gate.skill_below_floor`.
>
...
**LinkType (8, после ADR-L1 260612 — `based_on`/`graph_node` ретированы):** авто `mirrors` · `derives_from` · `session_context` · `promoted_to`; ручные `supports` · `contradicts` · `extends` · `superseded_by`.
...
> 📚 **Каждый блок схемы разобран в отдельной главе-детализации** (уровень «инженер с открытым кодом»):
> [27.12.1 ВХОД (запись)](27.12.1_Блок_Вход_Запись.md) ·
> [27.12.2 КООРДИНАЦИЯ](27.12.2_Блок_Координация.md) ·
> [27.12.3 ХРАНИЛИЩА](27.12.3_Блок_Хранилища.md) ·
> [27.12.4 ВЫХОД (чтение)](27.12.4_Блок_Выход_Чтение.md) ·
> [27.12.5 ОТДЕЛЬНО (.md-слой + Event Store)](27.12.5_Блок_Отдельно.md) ·
> [27.12.6 GOVERNANCE](27.12.6_Блок_Governance.md) ·
> [27.12.7 ПОТОКИ](27.12.7_Блок_Потоки.md)
...
================================================================================
  ...
  └───────────────┘         │  │  →RRF→dedup→   │  │      │ (surfacing, hot) │
   ...
   │ messages     │ │ patterns   │ │ saved/   │ │          │ │ 8 link-types │
...
- `LinkRegistry` — граф связей между сущностями (8 типов после ADR-L1 260612: `based_on`/`graph_node` ретированы; вход только unified-ID; отдельная SQLite `link_registry.db`).
...
- `pdf-docs` (DOCS) — проиндексированная документация для RAG (`pdf_documents`+`wiki_pages_v1`). Qdrant. Immutable со стороны памяти: governance (TTL/versioning/_get_entity) отвечает `unsupported_source` осознанно (ADR P0.4 260612). **Доступ — только через `pdf-vector-graph` MCP + читателей R2-R4; docs-плечо в `unified_search` — non-goal (ADR D2 260612: production-RAG закрывает все сценарии, 0 спроса на federated-доступ за всю историю, плечо = лишняя hot-path латентность).**
- `LinkRegistry` — не «контент», а **связи** между записями всех store'ов. SQLite. **Фактическое состояние 2026-06-12: 32 ребра, живых типов 3 из 10 заявленных (`mirrors` 24 / `derives_from` 6 / `supports` 2); полная верификация блока — [roadmap 260612 LinkRegistry](../../../roadmap/260612_ROADMAP_LINK_REGISTRY_FULL_VERIFICATION.md) (findings L1-L7: пустая типология, 3 raw-UUID ребра, history без delete, stats-рассинхрон, голодающий каскад).**
...
- **Синхронизация с колонками (ADR-011, 2026-06-12)**: репликация — non-goal; три легитимных канала — (1) read-time merge (md-плечо `memory-first-hook` + recall), (2) manual promotion через HITL write-gate (входящий конвейер — SessionStart-баннер `memory-curation-candidates-on-start.py`: зрелые паттерны eff_conf≥0.85/apps≥5, не покрытые каноном; записи с citations), (3) односторонняя event-эмиссия orchestrator-mediated мутаций (восстановление store'ов из событий — non-goal). Конфликт: канон = курируемый слой + текущий код.
...
```text
  ...
  (3) READ ─ на следующий промпт memory-first-hook (surfacing, hot-path):
          4 arms {skill, pattern_dense, pattern_lexical, skill_learning} → RRF(k=60) → inject
...
**Рельс 2 — `LinkRegistry` (общий граф, 8 link-типов после ADR-L1; что каждый поток со связями делает):**
...
| `skill_library` (Qdrant) | `skills-harvester` (Stop) / `reindex_skill_library` (каденс) | — (payload напрямую, point_id = uuid5(skill_name)) | `content_hash` (payload, = sha256(SKILL.md)[:16]) |
...
- **Главы-детализации блоков §10:** [27.12.1 ВХОД](27.12.1_Блок_Вход_Запись.md) · [27.12.2 КООРДИНАЦИЯ](27.12.2_Блок_Координация.md) · [27.12.3 ХРАНИЛИЩА](27.12.3_Блок_Хранилища.md) · [27.12.4 ВЫХОД](27.12.4_Блок_Выход_Чтение.md) · [27.12.5 ОТДЕЛЬНО](27.12.5_Блок_Отдельно.md) · [27.12.6 GOVERNANCE](27.12.6_Блок_Governance.md) · [27.12.7 ПОТОКИ](27.12.7_Блок_Потоки.md)
