...
| `/implement-1c-task <ANALYSIS-REPORT + src>` | [implement-1c-task.md](../../.claude/commands/implement-1c-task.md) | Реализация задачи 1С по готовому `ANALYSIS-REPORT.md` (8-этапный pipeline). Через EDT-MCP + 1c-mcp-crud + bsl-debug-server. Обязательные циклы: `validate_query` → `execute_query` ДО записи; `get_project_errors` ПОСЛЕ записи; `update_database` ПЕРЕД Этапом 6 (live-вызов изменённой функции). **Требования к среде:** EDT IDE с MCP-плагином на `:8765`, 1С HTTP-сервис `MCP_Сервер` опубликован, `.mcp.json:1c-mcp-crud` с валидным паролем — см. [16.6 EDT-MCP setup](16_ПОДКЛЮЧЕНИЕ_1С/16.6_EDT_MCP_setup.md). Smoke-test: `python scripts/smoke_test_implement_1c_task.py`. **Известные ограничения 1c-mcp-crud (v2.5):** `get_metadata` для регистров возвращает пустой `attributes:[]` (workaround — `execute_query`); `execute_query` падает на сериализации композитных ссылочных типов (workaround — `ПРЕДСТАВЛЕНИЕ()`). |
