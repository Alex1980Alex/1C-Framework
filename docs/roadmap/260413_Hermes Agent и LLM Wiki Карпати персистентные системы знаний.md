...
- Проверка метаданных через `get_metadata` (1c-mcp-crud)
...
**Почему неверно:** OpenSpec требует BSL-контекст, номер задачи, 1c-mcp-crud проверки. Для wiki-промоций (RAG паттерны, embeddings, Python code) это абсурдное требование.
