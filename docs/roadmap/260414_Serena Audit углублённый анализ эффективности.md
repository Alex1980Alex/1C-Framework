...
- **R2.5 Fallback orchestration:** если A timeout/error → автопереключение на B. **Артефакт:** обновление Orchestrator. **DoD:** интеграционный тест с remove multilspy → B работает.
  - **Статус:** 🔜 **NEXT**. Потребует `Orchestrator(backends, classifier, routing_matrix)` класс, использующий `RoutingMatrix.route_for(kind)` для выбора primary/fallback. Вся инфраструктура готова (MultilspyBackend + AstGrepBackend реализуют один Protocol).
