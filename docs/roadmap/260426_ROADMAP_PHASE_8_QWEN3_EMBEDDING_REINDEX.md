...
| 1d | **Backend wiring**: добавлен `Qwen3TEIQueryService` (mirror API через TEI HTTP) + γ-fallback в `Qwen3EmbeddingService` (Ollama → TEI при ConnectError). Production end-to-end smoke прошёл: Ollama unavailable → auto-fallback → TEI 200 OK → 4096d vec → bsl_code_v4_late top-3 scores 0.45 (идентично direct-TEI). Path α (`ollama pull qwen3-embedding`) остаётся опциональным upgrade когда Ollama daemon будет установлен | ✅ **DONE 2026-04-30** | 30 мин | После 1a/1b/1c |
