...
- [ ] **5c.7 Cost tracking baseline** ⏳ DEFERRED — требует ≥7 дней production traffic. Зафиксировать baseline в `docs/architecture/cost-baselines.md` (top-10 expensive queries, average tokens per RAG call). **Файл ещё не создан** (verified 2026-05-15). Pre-requisite: §5c.5 spans (DONE) — теперь traces появятся в Langfuse Cloud при production использовании.
