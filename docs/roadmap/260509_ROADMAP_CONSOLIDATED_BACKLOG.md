...
- [~] **5c.4 Wire memory operations** — PARTIAL 2026-05-15. Architectural decision: hooks не используют LangChain, поэтому `build_langfuse_callback()` (LangChain callback handler) **не подходит**. Создан новый helper `emit_observation()` в `langfuse_setup.py` — прямой Langfuse SDK API (pattern из `doneyli/claude-code-langfuse-template`, research cached в `architecture-research/cache/langfuse-standalone-spans-2026.md`). Wired в `session-memory-save.py` (proof-of-concept Stop hook). Smoke test PASS — exception-free execution. Остаются: `memory-first-hook.py` + `memory-sync.py` (same pattern, low risk follow-up).
