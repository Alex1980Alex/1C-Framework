...
- **2026-05-16, `src/api/auth/` UN-EXCLUDED.** 9 errors closed (8 `no-untyped-def` in [`rbac.py`](../../src/api/auth/rbac.py) decorators + RBACManager `__init__`, 1 in [`jwt_handler.py`](../../src/api/auth/jwt_handler.py) `get_jwt_handler(**kwargs)`). Added `Callable[[DecoratedAsync], DecoratedAsync]` type alias for the decorator pattern. `src/api/auth/` removed from `--exclude` in both `.pre-commit-config.yaml` and `.github/workflows/ci.yml mypy-baseline` job. mypy on `src/api/auth/`: `Success: no issues found in 4 source files`. Re-synced baseline (still 1795 errors total — auth/ contributed 0 after fix; net scope expansion captured 1 new edge case elsewhere). Only `src/pdf_framework/quick.py` remains in exclude.

- **2026-05-16, SessionStart banner.** [`.claude/hooks/session-mypy-banner.py`](../../.claude/hooks/session-mypy-banner.py) registered in `settings.json` SessionStart after `submodule-status-check`. Reads `mypy-baseline.txt` line count + best-effort `git diff origin/main -- mypy-baseline.txt --numstat` for delta. Emits `[mypy] baseline: N known errors (±M vs main)`. Pure-informational, doesn't run mypy itself (5s timeout). Research originally flagged this as low-ROI ([cache](../../.claude/skills/architecture-research/cache/mypy-baseline-regression-gate-2026.md)) — added at user request as supplementary signal to the CI gate.
