...
**Deliverables:**
- **D0.1 — оживить `memory-ingestion.log`:** вшить `ingest_metrics.record_ingest(store, action, content_hash, …)` в точки записи: `pattern_harvest.ingest_items` (created/dup/skip/error), skills-harvester, `reflection`, `cross_store_sync` (apply), `route_and_save` per-store; + `record_store_size` в P4-cadence. Закрывает топ-гэп #1.
- **D0.2 — ForgetGate script-path симметрия:** `memory_maintenance.run_forget` на архивации зовёт `lifecycle_log.log_event("decay_sweep"/"forget", …)` + `epoch.bump()` (зеркало MCP `handle_decay_confidence`). Закрывает топ-гэп #2 + баг stale-кеша.
- **D0.3 — оживить audit write-path:** `AuditService.log()` в `route_and_save` / delete / `create_link` / version-rollback / forget (operation/entity/source/ts, metadata-only). Закрывает топ-гэп #3.

**Acceptance:**
- [ ] После прогона харвестера/reflection — `memory-ingestion.log` непустой; `aggregate_ingest_events` в дашборде даёт ненулевые ingest/dup.
- [ ] Forget через P4-cadence виден в `confidence-lifecycle.log` (`decay_sweep`/`forget`) и **бампает epoch** (проверка: surfacing-кеш инвалидируется).
- [ ] `data/services/audit.jsonl` непустой после `route_and_save`; `memory_audit_log` возвращает записи.
- [ ] Всё fail-soft + opt-out; metadata-only (без тел).

### P1 — Новые per-process event-логи для слепых процессов (по образцу surfacing.log)

**Deliverables:**
- **D1.1 — routing-лог:** `Router.classify`/`route` пишет решение (targets, per-target confidence, method, reasoning-tag) в `memory-routing.log`.
- **D1.2 — federated-read трейс:** `unified_search` пишет per-query JSONL (arms-hits, score per source, RRF, dedup, rerank, latency, outcome) — зеркало surfacing-хука для MCP-чтения.
- **D1.3 — propagation трейс:** `propagate_update`/`PropagationEngine` логирует node-reach, decay (time×dist), confidence-дельты соседей.
- **D1.4 — CircuitBreaker trip-лог:** open/half-open/close переходы как персистентные события.

**Acceptance:**
- [ ] Каждый процесс эмитит структурный JSONL на вызов; fail-soft; opt-out env; atomic-rotation.
- [ ] По логам восстановимы: routing-accuracy, read hit-rate/латентность по источникам, охват каскада, инциденты breaker'а.
