...
- ~~PropagationEngine (контур 3.2) — «simulate success» без `update_handlers`: BFS/лог реальны, мутаций нет.~~ **✅ Закрыто P2.3 (2026-06-10):** wired реальные handlers (vector-memory Beta-нудж + memory-ai importance), `_apply_update` честный (no-handler→False).
...
| **P2.2** ✅ | Судьба стабов B1 | **DONE 2026-06-10 — восстановлены (L5-план):** `save_to_wiki_log` (UTF-8 append + trim), `try_promote_patterns` (делегирует канонический `export_graph_to_wiki promote-patterns`), `_emit_langfuse_span` (graceful no-op без Langfuse). Восстановлено из git `7bc57e463`; смоук PASS |
| **P2.3** ✅ | ADR: PropagationEngine vs `_cascade_confidence` | **DONE 2026-06-10 — wired `update_handlers`:** `propagate_update` теперь реально мутирует (vector-memory Beta succ/fail + memory-ai importance), `_apply_update` честный (handler→bool, no-handler→False, конец «simulate success»). Оба механизма сосуществуют: `_cascade_confidence` — синхронный каскад на горячем пути `apply_pattern`, PropagationEngine — явная depth-N multi-store пропагация через MCP. 5 unit + 9 engine + 18 orchestrator зелёные |
...
| 2026-06-10 | **P2.2 + P2.3 DONE (ADR-треки, после `/mcp reconnect`-верификации P1).** **P2.3** (wire `update_handlers`, закрывает A8/контур 3.2): `PropagationEngine._apply_update` сделан честным — handler→`bool(result)`, no-handler→`False` (конец «simulate success»: больше нет фантомных `entities_updated`); `MemoryOrchestrator._build_propagation_handlers()` подключает реальные handlers (vector-memory: Beta `succ/fail`-нудж по знаку delta → `derive_confidence` → `set_payload` на тёплом `_get_qdrant`; memory-ai: `importance ± delta` clamp `[0,1]` в SQLite, `MEMORY_AI_DB_PATH`-override для изоляции тестов); `propagate_update` сворачивает `success` в знак delta. Module-level `propagate_update` получил optional `update_handlers`. Сосуществование с `_cascade_confidence` задокументировано (горячий apply-путь vs явная MCP-пропагация). **P2.2** (восстановление стабов B1, L5-план): `save_to_wiki_log`/`try_promote_patterns`/`_emit_langfuse_span` восстановлены из git `7bc57e463` (fix ctx-ключа `files_changed`); promote делегирует канонический `export_graph_to_wiki promote-patterns` (= §26 P4 cadence job), Langfuse graceful-degrade. **Тесты:** новый [`test_propagation_honest.py`](../../tests/unit/test_propagation_honest.py) (6 unit: no-phantom / handler-False-not-counted / handler-True-counted / memory-ai mutate+clamp+missing) + 2 BFS-теста в `test_p1_infrastructure.py` получили явный dummy success-handler; **5 unit + 9 engine + 18 orchestrator integration + memory-unit кластер (26) PASS**, smoke 3 функций PASS, ruff+compileall чисто. Доки 27.12.1 §5 / 27.12.5 / 27.12.7 §3.2 + A8/B1/P-таблица синхронизированы. **Остаётся P2.1** (maintenance-apply, ~2 нед), **P2.4–P2.6** (док-фиксы B2–B4/B8, надёжность B9, мелочи B6/B7/B10/B11) |
...
| 2026-06-10 | **P2.2/P2.3 post-review remediation (6 фиксов по итогам code-review реализации).** (1) **HIGH:** lazy-init `PropagationEngine` в оркестраторе получил `PropagationConfig(enable_background_processing=False, enable_event_deduplication=False)` — с дефолтным config production-вызов `propagate_update` уходил в background-очередь и возвращал `entities_updated=[]`/`reason="queued_for_background_processing"` (честный результат P2.3 прятался за очередью), а event-dedup молча глотал повторную пропагацию того же entity до переполнения `_recent_events` (1000). (2) **HIGH:** vector-handler бампит §24 epoch после `set_payload` (инвариант «каждый писатель confidence инвалидирует surfacing-кэш»; без бампа memory-first-hook отдавал stale confidence до TTL 900s). (3) `try_promote_patterns` → detached `Popen` (паттерн post-indexing-analyzer, лог `.claude/cache/session-promote.log`): Stop-бюджет не платит за Qdrant-scroll (замер: 1.6s типично, detach 0.004s). (4) `_emit_langfuse_span` — pre-gate `_langfuse_configured()` (env/`.env` probe до импорта `src.*`; гейт через `get_settings()` стоил ~2s даже при выключенном Langfuse; здесь Langfuse реально включён — span = настоящий network-flush ~2.3s, emit последний в `execute()`); + sys.path `append` с guard вместо `insert(0)`. (5) timeout `session-memory-save` 5→15s в `settings.json`. (6) `ai_memory/server.py` `DB_PATH` уважает `MEMORY_AI_DB_PATH` (унификация с handler'ом). Мёртвая ветка `if "Promoted" in stdout: pass` удалена. Mojibake-аудит `docs/wiki/log.md`: 370 строк, 0 повреждённых (страх из анализа не подтвердился — round-trip detect). **+2 теста-пина** в `test_propagation_honest.py` (7 total): `test_orchestrator_propagate_is_sync_and_repeatable` (ловил бы #1), `test_vector_handler_bumps_epoch` (ловил бы #2). 46 тестов (honest 7 + p1_infrastructure 39) PASS, ruff+compileall+settings.json чисто. ⚠ MCP-side (`memory_orchestrator.py`, `ai_memory/server.py`) → `/mcp reconnect`; hook/settings подхватятся со следующей сессии |
