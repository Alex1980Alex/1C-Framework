...
| 2026-06-11 | **F16 + cold-start исправлены** | **F16**: `_apply_version_to_store` (MEMORY_AI-ветка) разворачивает `metadata.importance` из CREATE-снапшотов route_and_save (top-level по-прежнему приоритетен — UPDATE-снапшоты propagation не задеты); тест `test_rollback_create_snapshot_unwraps_metadata_importance`. **Cold-start**: `vector_memory/server.py` — S1-warmup (daemon-поток `_warmup_qdrant` при старте сервера пре-оплачивает импорт `qdrant_client` до первого tool-вызова; fail-soft, opt-out `MEMORY_VECTOR_NO_WARMUP=1`) + double-checked lock в `_get_qdrant`; publish-after-ensure (review N1) — упавший init оставляет глобал None, следующий вызов ретраит весь lazy-путь; `_ensure_collection(client)` принимает клиент параметром. Тесты: fail-soft + одиночная инициализация под 8 потоками. Code-verify PASS (субагент `a4ac25884`). ⚠ MCP-side → активно после следующего `/mcp reconnect`. **N2 follow-up (не чинилось)**: симметричный гэп vector-ветки — rollback-to-CREATE vector-паттерна восстановит только `content`, не `metadata.confidence/name/description`; ответ честный (`fields`), конфликт с Beta-posterior — заносить как F-item, если rollback vector-паттернов станет live-путём |
