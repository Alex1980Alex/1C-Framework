...
| Дата | Событие | Детали |
|------|---------|--------|
| 2026-06-12 | **P0-P3 DONE + ADR-L1, acceptance-окно открыто (день 1/14: 6/6 PASS)** | **P0**: дамп 32 рёбер (`link_registry_pre_p0_dump.json`) → миграция [`scripts/link_registry_p0_migrate.py`](../../scripts/link_registry_p0_migrate.py): 2 ребра разрезолвлены по живым store'ам (4 конца: 2×memory_ai → `episodic:`, 2×learned_patterns → `semantic:`), 1 неразрешимое удалено с журналом → **31 ребро, 0 raw-ID**; `create_link`/`create_links_batch` валидируют unified-ID (ValueError на сырой UUID); `delete_links_for_entity` журналирует delete + чистит stats соседей; `_update_stats` purge-on-zero + `rebuild_stats()` (73→45 строк, рассинхрон излечен) + job `rebuild_link_stats` в maintenance-каденсе (apply-only). **ADR-L1**: типология 10→8 — `based_on` (non-goal по ADR-D2/D4 pdf-docs) и `graph_node` (0 писателей/читателей/рёбер) ретированы из enum+CHECK(новые БД)+весов propagation+каскад-списка+скилла+карты 27.12; `supports/contradicts/extends/superseded_by` = ручной словарь. **W4 починен** (корень raw-рёбер L2: `route_and_save` линковал сырые store-id; теперь unified через `_TARGET_TO_SOURCE`+`from_original`, `created_by=route-and-save`, честный `cross_links_created`); **W5 перекласифицирован**: проведён ещё F7 (260610), не спал — ребро появится на первом промоуте. **Live-цепочки**: A1 PASS (sync ×2: created 0/existing 22 — идемпотентен), A3-neg PASS (честный ValueError), B1 PASS (BFS от мигрированного semantic-конца ходит derives_from→episodic + depth-2 mirrors — миграция оживила легаси-рёбра), B2 наблюдён живьём ранее (mirrors в `linked_entities` unified_search), B4 PASS (31/45 согласованы), C3-TTL наблюдён живьём (unparseable_id, утренняя D7-сессия); B3 live — deferred до первого организменного supports-ребра (unit-покрытие: 5 тестов test_apply_cascade), A2 — существующие 6 derives_from рёбер = доказательство цепочки. **P3**: sink `memory-links.log` в `known_sinks` + `link_create`/`link_delete` из CRUD (fail-soft `_trace`) + `cascade_empty` при пустом каскаде (L5 закрыт). **P4**: [`scripts/link_registry_acceptance.py`](../../scripts/link_registry_acceptance.py) (4-й потребитель acceptance_common), день 1/14 — все 6 критериев PASS. **Тесты**: `tests/unit/test_link_registry_p0.py` (6) + патчи 5 файлов под ADR-L1/валидацию — 11 unit + 126 integration PASS. Наблюдения: (1) pre-existing провал `test_wiki_keywords_exist` (router, вне scope — падает и на чистом дереве); (2) валидатор `roadmap_progress_log.py links` флажил dash-форму wikilink'ов при underscore-именах memory-файлов — системный mismatch (валидатор считал каноном имя файла, контракт памяти — frontmatter-слаг; 100% репортов ложные) — **закрыт в этой же сессии**: `memory_slug_index()` (file-stem + frontmatter `name:`, нормализация dash-lower), live-прогон `links` → OK, регресс `tests/unit/test_roadmap_links_resolver.py` (4). ⚠ Правки orchestrator/vector_memory — MCP-side: нужен `/mcp reconnect` |
