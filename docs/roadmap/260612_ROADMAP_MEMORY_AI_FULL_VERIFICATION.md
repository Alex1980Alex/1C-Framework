...
**Acceptance вердикт: PASS** (окно 2026-06-12 → 2026-06-26, зафиксировано 2026-07-17 после разбора двух нулей — см. две строки выше).

⚠ **Оговорка к вердикту, честно:** PASS получен ПОСЛЕ починки 5 реальных дефектов и ЗАМЕНЫ критерия 5. Прямое сравнение с «6 критериев» из строки 2026-06-12 некорректно — их теперь 7, и один измеряет другое. Все семь фальсифицируемы (пин: `test_memory_ai_acceptance.py::test_each_criterion_can_fail`). Открытые долги (ревью, не блокеры): `read_episodes` не фильтрует `archived_at` — при `min_cluster=2` латентный конфликт с forget-gate ожил (архивные эпизоды могут дистиллироваться в свежий паттерн); `MEMORY_MAINTENANCE_APPLY_<JOB>` покрывает только `SUBPROCESS_JOBS` (5 инлайн-джобов — молчаливый no-op, зафиксировано в docstring); `_progress` в SessionStart-хуке мёртв при закрытом окне; fallback-импорт не снимает полуинициализированный модуль из `sys.modules` при провале exec.
