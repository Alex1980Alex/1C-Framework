...
| 2026-07-17 | **Разбор двух нулей** (`dup_nonzero`, `reflection>=1`) | Окно закрылось FAIL: 4/6 зелёных, два — ровно 0. Проверялись ОБЕ гипотезы («код мёртв» / «критерий врёт»); у каждого нуля оказался свой корень, и ни один не был «кодом мёртв». **dup: прибор не подключён.** Дедуп в `session-memory-save` срабатывал (ровно 1 summary/день — это его работа), но путь раннего выхода эмитил только no-op-span и НЕ звал `_record_ingest` → в логе 2 события `memory_ai` при 155 строках в БД (у `learned_patterns` для контраста 5365 dup). **reflection: четверо ворот.** (1) `min_cluster=3` при макс. реальном кластере **2** (52 эпизода) — недостижим; (2) `importance_theta=None` → OR-ветка триггера мертва; (3) каденс всегда dry-run (`MEMORY_MAINTENANCE_APPLY` не задан нигде) — **reflect отработал 35 раз с rc=0 и написал ноль**; (4) найдено живым прогоном: `ingest_items` на dup-ветке делал `continue` ДО `on_created` → DERIVES_FROM терялся навсегда для точек, записанных другим харвестером первым, и ни один повторный прогон их не бэкфиллил. Фиксы: эмит `dup` с хешем **инкумбента** (ctx дрейфует между Stop'ами → свежий хеш осиротил бы fact-trace) + честный `reason` (два ключа дедупа: session_id / дата); `DEFAULT_MIN_CLUSTER`=2; дефолты CLI из констант модуля (argparse-копия и была тем дрейфом — каденс идёт через CLI, правка модуля была no-op); новый колбэк `on_present` в `ingest_items` (провенанс не зависит от того, кто записал точку первым; идемпотентен — `create_link` бросает на дубль, `make_link_fn` глотает); `harvester="reflection"`; per-job `_job_apply` + `MEMORY_MAINTENANCE_APPLY_REFLECT=1` (общий флаг раздал бы запись сразу 5 джобам, включая production `wiki_pages_v1`/`skill_library`). Попутно: сломанный с рождения fallback-импорт в `reflection.py` (`# pragma: no cover`, не регистрировал модуль в `sys.modules` до `exec_module` → `@dataclass` падал AttributeError). 11 саботаж-проверок, 2373 unit |
| 2026-07-17 | **Критерий 5 заменён** (⚠ читать при сравнении с историей) | `reflection>=1` (рёбра, созданные В ОКНЕ) признан **неудовлетворимым по построению**: reflection идемпотентен, отработал 2026-06-11 — за день до открытия окна; корректный идемпотентный джоб на почти статичном корпусе обязан давать 0 новых рёбер. Он не «проспал» баг — он честно горел красным, но не отличал «сломано» от «нечего делать». Замена: **`reflection_reachable`** (`clusters_triggered>0` через реальный `reflect(dry_run=True)` — ловит мёртвый триггер: до фикса дал бы 0 → FAIL) + **`reflection_wrote`** (≥1 ingest-событие `harvester=reflection`; `ingest_items` эмитит статистику только при `not dry_run` → событие = доказательство записи; ловит возврат в dry-run, т.к. флаг живёт в gitignored `settings.local.json`). Вариант «рёбер всего ≥1» отвергнут — был бы зелёным все 5 недель при мёртвом триггере. Адверсариальное ревью (PARTIAL → фиксы) сняло первую редакцию замены: она не наблюдала путь записи вообще; там же пойманы вакуумный тест `test_fresh_session_still_saves`, непокрытая строка проводки `_job_apply`, и отчётность каденса, врущая «dry-run» при записи в прод. Файл `memory_ai_acceptance.py` получил первые тесты (`tests/unit/test_memory_ai_acceptance.py`, 7 критериев × фальсифицируемость) |


| 2026-07-17 | **Ревью раунд 2 → FAIL → фиксы** | Второй ревьюер снял первую редакцию замены: `reflection_reachable`+`reflection_linked` **не наблюдали путь записи вообще** — оба остались бы зелёными при reflect в вечном dry-run (флаг в gitignored `settings.local.json`; клон/другая машина/удалённая строка). `reflection_linked` → **`reflection_wrote`** (ingest-события `harvester=reflection`, окно 30д). Там же снят мой ложный тезис «старый критерий проспал баг» — он честно горел красным, просто не отличал «сломано» от «нечего делать». **Окно у критерия 6 удовлетворимо**, вопреки логике критерия 5: apply-прогон над собранным корпусом уходит в `skipped_dup` и эмитит `dup` на хеш → каждый прогон даёт ≥1 событие; аргумент идемпотентности сюда не переносится (перенёс механически — поймано ревью). **Системная находка:** CI собирает строго по маркерам, а `tests/integration/test_memory_p5_session_save.py` маркера не имел → слом контракта `already_saved` оставил 5 красных тестов при зелёном CI («2373 unit passed» врали). Маркер проставлен; **ещё 27 файлов в `tests/integration/` без маркера в CI не гоняются — открытый долг**. Прочие фиксы: баннер каденса писал «dry-run» пока reflect мутировал прод (`_dry_run_mode`); вакуумный тест (`save_to_sqlite` застаблен → `assert "dup" not in []` тривиален); непокрытая строка проводки `apply = _job_apply(name, apply)`; `read_episodes` не фильтровал `archived_at` (конфликт с forget-gate, **активированный** правкой `min_cluster=2`); fallback-импорт не снимал полуинициализированный модуль при провале exec. 2376 unit + 28 integration + саботаж 14/14 |

**Acceptance вердикт: PASS** (окно 2026-06-12 → 2026-06-26, зафиксировано 2026-07-17 после разбора двух нулей — см. строки выше).

⚠ **Оговорка к вердикту, честно:** PASS получен ПОСЛЕ починки 6 реальных дефектов и ЗАМЕНЫ критерия 5. Прямое сравнение с «6 критериев» из строки 2026-06-12 некорректно — их теперь 7, и один измеряет другое. Все семь фальсифицируемы (пин: `test_memory_ai_acceptance.py::test_each_criterion_can_fail`), каждый фикс проверен саботажем. Открытые долги (не блокеры): **27 файлов `tests/integration/` без маркера не гоняются в CI** (системный пробел, обнаружен по ходу); `MEMORY_MAINTENANCE_APPLY_<JOB>` покрывает только `SUBPROCESS_JOBS` — 5 инлайн-джобов молчаливый no-op (fail-closed + docstring); `_progress` в SessionStart-хуке мёртв при закрытом окне; `reflection_wrote` не заметит регресс раньше чем через 30д и слепнет при ротации ingest-лога на 2МБ.
