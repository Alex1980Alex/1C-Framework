...
> LinkRegistry). Объект: СИСТЕМА_СКИЛЛОВ ([глава 11](../framework%20documentation/9_НАВЫКИ/9.1_СИСТЕМА_СКИЛЛОВ/11.1_Обзор.md))
...
> [27.12](../framework%20documentation/5_ПАМЯТЬ/5.1_UNIFIED_MEMORY/27.12_Memory_Systems_Map.md)).
...
| 2026-06-12 | **P3 DONE — стык с Unified Memory задокументирован** | [27.12 Memory Systems Map](../framework%20documentation/5_ПАМЯТЬ/5.1_UNIFIED_MEMORY/27.12_Memory_Systems_Map.md): skill-система добавлена как вход (subgraph в диаграмме §1: каталог → harvesters → `skill_library` → surfacing-плечо), строка в физической карте store'ов (§2), строка в write-contract таблице (§11.2), score-floor в surfacing-примечании (§5). Fact-trace: жизнь скилла тредится по `content_hash` (= sha256(SKILL.md)[:16]) через `memory-ingestion.log` → `memory_observability_query.py --view fact-trace` |
...
| 2026-06-13 | **Follow-up [260613](260613_ROADMAP_SKILL_SYSTEM_VERIFICATION_FOLLOWUP.md) P0–P3 реализован → критерий 3 честно FAIL** | Acceptance переключён с in-sample `skill_metrics.f1` (0.7595, padded) на pooled honest `action_f1`. Живой прогон: **action_f1=0.7361 < 0.75 → критерий 3 = FAIL** (ровно «оспорено» из строки выше, теперь подтверждено числом). `all_pass=false`. Также landed: word-boundary матч имени скилла (F3), honest CI advisory (F2), exit-1 структурный чек критерия 7 (F6), A.1 phantom-guard (F7), `library_content_fresh` критерий (F4), дефолт health-path (F8). Дотяжка роутера до honest 0.75 — follow-up в 260613. Acceptance 7/7 из строки «day-1» выше относилось к padded-числу и **отозвано**. |
| 2026-07-18 | **Acceptance вердикт: PASS** | Окно закрыто (`skill_system_acceptance.py --final`): ghosts=0 & drift=0, library_content_fresh, ingest>0, router_f1≥0.75 (0.7708 gate-representative), floor отрезал 2302 skill-хита (arm fired 759/1119), skill-health-report возраст 0.8d, honest-failure guards (ci_honest_fail+phantom_guard) — все 9 критериев PASS |
| 2026-06-13 | **Критерий 3 РАЗРЕШЁН честно через [260613 A→D](260613_ROADMAP_SKILL_ROUTER_HONEST_EVAL.md)** | Сквозная работа honest-eval закрыла спор: GT очищен от leakage + стратиф. split (A), репрезентативный гейт (A7: 22 ре-верифицированы), честная метрика pooled action_f1 + train/test/CV инфра (B), дотяжка удалением pure-FP optional-скиллов (C: gate 0.7361→**0.7708≥0.75**, held-out test 0.83, CV 0.779 — генерализуется, не overfit, code-verify PASS). **Acceptance теперь all_pass=True ЧЕСТНО (9/9, gate_representative=True, router_f1 0.7708).** CI-гейт осознанно advisory (D3: GT мал/эволюционирует; авторитет — локальный харнесс). Формальный вердикт окна — 2026-06-27. Критерий 3 («качество доказано числом») выполнен честно-измеренным числом на репрезентативном leakage-free гейте. |
