...
| 2026-06-13 | **Фаза D DONE — CI-гейт решён (advisory, осознанно), acceptance all_pass=True; роадмап функц. завершён** | **D3 (CI-гейт):** honest gate semantic-independent — с `SKILL_ROUTER_NO_SEMANTIC=1` (CI-эквивалент без TEI/Qdrant) число **ИДЕНТИЧНО 0.7708** (silence даже выше: 0.9375) → CI==local, риск «CI≠local» снят. **Решение: оставить advisory ОСОЗНАННО** (не «пока не дотянут» — дотянут): GT мал (95) и эволюционирует (A7-рост test ещё pending) → blocking фолс-трипал бы на ЛЕГИТИМНОМ росте GT; авторитетный honest-гейт = локальный acceptance-харнесс (`skill_system_acceptance.py`, all_pass=True, полное окружение). [`ci.yml`](../../.github/workflows/ci.yml) комментарий обновлён на резолюцию; флип на blocking = одна строка когда GT стабилизируется. **D4 (acceptance):** all_pass=True ЧЕСТНО (9/9, gate_representative=True, router_f1 0.7708); формальный вердикт PENDING до закрытия окна 2026-06-27 (ежедневный баннер фиксирует авто). **D5/D6:** методология в CLAUDE.md + §18; паттерн `honest-eval-gate-pooled-not-traintest` captured (skill-learning); архив роадмапа — после окна. **Роадмап 260613 (A→B→C→D) функционально завершён; формальная acceptance — по окну.** |
