...
### V.6 Дизамбигуация чат-входа + создание новой ТЗ-папки (правило пользователя 2026-06-15)
Когда задача пришла **сообщением в чате (A2)** и тип не задан:
1. **Авто-детект родителя:** сначала ищу — не доработка/багфикс ли это существующего ТЗ (по `configuration/*/docs/`, `ANALYSIS-REPORT`, именам).
2. **ASK, НЕ guess:** если **не нашёл ИЛИ пользователь явно не сказал «это доработка/исправление по ТЗ X»** → **ОБЯЗАТЕЛЬНО уточнить**: «новая отдельная задача или доработка/багфикс существующего ТЗ?». Не угадывать тип.
3. **Standalone (пользователь подтвердил «отдельная задача»)** → создать **новую ТЗ-папку** и вести по дорожной карте (4 этапа):
   - **Расположение (provisional, 2026-06-15):** `configuration/260304_GKSTCPLK-2182 Доработать создание Направление на разгрузку для заблокированных ТС/docs/<YYMMDD_slug>/` — подпапка под `docs/` существующего parent-сабмодуля (рядом с 2176/2177/2178). ⚠ **Временно** — «потом решим, возможно перенесём в другую папку» (**TODO**: вынести standalone-задачи в отдельный корень/структуру). Артефакты трекаются git'ом submodule 260304.
   - **Имя/slug:** авто **`YYMMDD_<краткое-имя>`** из описания (без JIRA-тикета) [решение].
   - **Артефакты:** чат-сообщение → спека `<slug>.md` + (скриншоты, если приложены) + `ANALYSIS-REPORT.md`, далее по 4 этапам.
   - **Объём:** полная папка для **ЛЮБОЙ** отдельной задачи, **включая trivial** [решение] (полная трассируемость; этапы внутри trivial-задачи — компактны per ADR-018, но папка+артефакты создаются всегда).
4. **Доработка/багфикс (T2/T3)** → НЕ новая standalone-папка: подпапка к родителю `configuration/<parent>/docs/<slug>/`, линк `parent_task`, загрузка prior `ANALYSIS-REPORT`/реализации.

Уточняет **G21** (классификация = ASK при неоднозначности, не guess) и **G22** (standalone vs дельта развязаны явно). **Поведенческое правило применяется СРАЗУ** (не ждёт автоматизации Phase 2; автоматизация — часть input-ingestion V.3). Расположение — provisional, под ревизию.
...
| 2026-06-15 | Реализация | **B′ РЕАЛИЗОВАН: F-1 + F-1.5 + F-2 + F-3 (foundation-first, каждый — свой `pipeline/`, approve, регресс).** Через ту же 4-этапную дисциплину (dogfooding). **F-1** (`6fc65a977`): `pipeline_1c_bridge.ensure_pipeline_1c` + проводка в analyze/implement-preflight → 1С-слэш авто-заводит pipeline-state (G3 закрыт); unit 5/5 + live. **F-1.5** (`0b941782d`): PostToolUse `pipeline-1c-advance` + `advance_for_artifact` → ANALYSIS-REPORT→этапы 1,2; IMPLEMENTATION-PROGRESS→3 (guard по title-метке). **F-2** (`5b8ffeccd`): `pipeline-gate` += `/implement-1c-task` + `gate_1c_implement` → хард-блок до approve дизайна (G4); live block/allow. **F-3** (`eeb42d745`): нота «4-этапная парадигма» в analyze/implement SKILL.md (G5/G2). Итого G3/G4/G5/G2 закрыты для слэш-маршрута; методика 1С не тронута (0 правок логики). Остаток: этап-4 advance (тест-артефакт), input-ingestion (V.6/G20-23), TOOL-USAGE-отчёт (W). | `pipeline_1c_bridge.py` + `pipeline-1c-advance.py` + `pipeline-gate.py` + 2 SKILL.md + `test_pipeline_1c_bridge.py` (10 тестов) + `settings.json` + CLAUDE.md |

| 2026-06-15 | Реализация | **AUTO-режим `/run-1c-task` (доработка пользователя, umbrella `impl-1c-run-task`).** Запрос: для 1С-задачи добавить полный прогон без паузы на ревью первого этапа — и для чат-задачи, и для готовой ТЗ-папки. Решения (AskUserQuestion): **одна команда** `/run-1c-task <вход>`, объём **analyze→implement→test**. Реализация: `resolve_task_input` (детект входа folder/jira/chat) в `pipeline_1c_bridge.py` + команда `.claude/commands/run-1c-task.md` + skill-оркестратор `.claude/skills/run-1c-task/SKILL.md` (analyze-1c-task-v2 → **авто-approve** [`pipeline_state approve`, без паузы] → implement-1c-task → va-bdd-testing → W; хард-правило «AUTO ≠ игнор блокеров»). Гейтованный поток `/analyze`+`/implement` не затронут; методики 1С делегируются, не дублируются. Тесты: +4 unit `resolve_task_input` (18 passed bridge, 51 collision-immune), live resolve 3 кейса, skill discoverable. | `pipeline_1c_bridge.py` (`resolve_task_input`) + `run-1c-task.md` + `run-1c-task/SKILL.md` + `test_pipeline_1c_bridge.py` (+4) + CLAUDE.md + `pipeline/impl-1c-run-task/` |
| 2026-06-15 | Реализация | **B′ хвост реализован: F-1.6 + W + input-ingestion** (`8455baea9`, umbrella `impl-1c-bprime-tail`). **F-1.6:** `advance_test_done` — `.run-state.json` все `chain.status==passed` → этап 4 done (live: partial→pending, all→done). **W (раздел W):** `scripts/tool_usage_report.py` (stdlib, `--run-id`/`--session`→отчёт по `hook-invocations.jsonl` + append `data/tool-effectiveness.jsonl` [gitignored]; `--rollup` cross-task) — live на реальном логе сессии (Edit 977/Bash 648/…). **input-ingestion (V.6/G20–23):** `classify_1c_task` (T1/T2/T3) + UPS-хук `onec-task-input` → протокол V.6 (ASK/folder/prior-load) при 1С-задаче из чата (live: «исправь ошибку гкс_…»→T2-протокол). 17 unit + 47 регресс passed. **B′ полностью завершён** (G3/G4/G5/G2 + этап-4 advance + W + V.6-дизамбигуация). | `pipeline_1c_bridge.py` + `pipeline-1c-advance.py` + `onec-task-input.py` + `tool_usage_report.py` + 2 теста + `settings.json` |

| 2026-06-15 | Реализация | **Классификатор сложности + маршрутизация 1С-задачи из чата (доработка пользователя, umbrella `impl-1c-task-router`).** Запрос: при чат-задаче сам определять 1С/не-1С (сомнение→спросить) + по оценке трудозатрат (простая/средняя/сложная) выбирать поток. Решение (AskUserQuestion): **простая→AUTO `/run-1c-task`, средняя→спросить, сложная→гейт**; не-1С/сомнение→спросить. Реализация: `estimate_effort` (эвристика по keyword-сигналам, config `_EFFORT_CFG` тюнится; **light=downgrade-only при pos==0**, не counterweight) + `route_1c_task` (classify+estimate→flow none/ask_1c/auto/ask_flow/gated, `_1C_STRONG` для уверенности) в `pipeline_1c_bridge.py`; `onec-task-input.py` сурфейсит маршрут+оценку+хвост «эвристика<правило». **Durable-фикс `code-skill-enforcer`:** `_is_test_file`→tests/ exempt от content-уровней A/A.1 (Level A.1 форсил learning-loop на 1С-токенах в тест-фикстурах). **Live-smoke поймал баг эвристики** («Комментарий» занижал medium→simple) → light=downgrade-only + регресс. Тесты: +8 unit (28 passed, 61 collision-immune), in-process route 5 потоков верны, code-verify PASS (ReDoS/injection/банды/over-exempt — все PASS). Граница: classify_1c_task recall (без точки/JIRA/гкс_ может не поймать 1С) — унаследованное ограничение, покрыто поведенческим правилом «сомнение→спросить». | `pipeline_1c_bridge.py` (`estimate_effort`/`route_1c_task`/`_EFFORT_CFG`/`_1C_STRONG`) + `onec-task-input.py` + `code-skill-enforcer.py` + `test_pipeline_1c_bridge.py` (+8) + CLAUDE.md + `pipeline/impl-1c-task-router/` |

| 2026-06-15 | Реализация | **Recall-расширение 1С-детектора (доработка пользователя, umbrella `impl-1c-detector-recall`).** Запрос: расширить детектор чтобы хук ловил больше формулировок; решение «это 1С» за Claude + сомнение→спросить; задачи 1С в `C:\1С-Framework\configuration`. **Доп. анализ (по требованию):** заземление на ~35 реальных задач `configuration/260304…/docs/` → живой словарь глаголов/терминов (АРМ, РС/РН, печатн, обмен, движени, предопредел, профил, …) вместо догадок. Реализация: двухуровневый `classify_1c_task` — `_1C_DEFINITIVE` (гкс_/configuration → 1С без глагола) + `(_1C_SIGNAL ∪ _1C_STRONG CamelCase) AND _TASK_VERB`; `_TASK_VERB`/`_1C_SIGNAL` расширены (+T3-«не учтено» учт/учес/учл по рек. ревьюера), `\b`-границы против substring-FP. **Инвариант:** weak-матч (term+verb без сильного маркера) → `ask_1c`, не авто-маршрут (recall↑ безопасно). CamelCase в `re.U` без `re.I` (критично). Тесты: +6 (34 passed, 67 collision-immune), code-verify PASS (эмпирически: re.U-флаг, ReDoS-тайминги, \b-кириллица, over-recall→ask инвариант). Граница: детектор = подсказка, не замена суждению; generic-термины дадут ложные ask на не-1С (приемлемо). Память `feedback-1c-detection-judgment-mine`. | `pipeline_1c_bridge.py` (`_TASK_VERB`/`_1C_SIGNAL`/`_1C_DEFINITIVE`/`_1C_STRONG`/`classify_1c_task`) + `test_pipeline_1c_bridge.py` (+6) + CLAUDE.md + память + `pipeline/impl-1c-detector-recall/` |

| 2026-06-15 | Реализация | **Закрытие пробелов 43.5 (ТОП-3 + остаток, umbrella `impl-43-gaps-fixes`, `b11c95827`).** Глубокий адверсариальный анализ 43.5 (переоценка 3 заявленных + 7 скрытых H1–H7). **H5 (реальный баг):** единый gate `onec-task-completion-stop` детектил 1С-задачу только по `updated_at≥session_start` → задача в 2 сессии проскакивала; детект расширен `_onec_pipeline_updated` ИЛИ `_incomplete_onec_pipeline` ПРИ `config_edit` сессии. **H2:** `_write_loops_report`→`pipeline/<slug>/LOOPS.md` (✓/✗ петли + opt-out + указатель W). **Call-graph:** `git_post_commit_reindex` на `.bsl`-diff спавнит `build_call_graph` (детачед, троттл ≤1/6ч через spawn-sentinel; full re-parse без `--paths`); `graph_embeddings` — manual. **H7 (latent):** `advance_for_artifact` требует ≥200 непробельных символов (пустой/stub-артефакт не продвигает этапы). **H6:** `run-1c-task` этап 2.4 preflight полноты ТЗ перед авто-approve. **H1:** tool-effectiveness задокументирован отчётным (не обучающий контур). H4 by-design. 45/45 unit + code-verify PASS (порог H7 эмпирически против 102 реальных артефактов). | `b11c95827` + `onec-task-completion-stop.py` + `pipeline_1c_bridge.py` + `git_post_commit_reindex.py` + `run-1c-task/SKILL.md` + 43.3/43.5 + `pipeline/impl-43-gaps-fixes/` |

| 2026-06-15 | Реализация | **Закалка тонких мест N1–N12 + единый task-completion gate (umbrella `impl-thin-spots-hardening`).** Адверсариальный анализ 43.5 → 12 тонких мест. **Исправлено:** N4 единый предикат `is_1c_task_title()` (вместо 6 копий литерала — рассинхрон bridge/хуки); N3 `TRANSCRIPT_TAIL_BYTES=8 МБ` (ранний recall/research не обрезается tail-окном); N6 capture по `.md` сужен до `/.claude/.../memory/*.md`; N5 AUTO `approve --by auto` (метка `approved_by=auto`); N1+N11 превью block-сообщения `pipeline-protocol-stop` + under-steps в `run-1c-task` SKILL (тройной Stop-каскад за проход); N10 opt-out → `outcome="allow-optout"` в `tool_usage_report`. **Консолидация:** `onec-task-completion-stop` заменил `memory-protocol-stop`+`research-protocol-stop` (один чеклист ПАЙПЛАЙН/RECALL/CAPTURE/RESEARCH/SKILL, блок один раз). **By-design:** N7 research-relevance (любой WebSearch; проверка→false-block), N9 AUTO авто-approve (суть режима+N5-метка), N12 advance по имени файла (best-effort), N2/N8 graceful. Регресс-тесты. (Перенесено из 43.5 при сжатии главы 2026-06-16.) | `onec-task-completion-stop.py` + `pipeline-protocol-stop.py` + `pipeline_1c_bridge.py` + `run-1c-task/SKILL.md` + `pipeline/impl-thin-spots-hardening/` |
