...
| **P1.7 Age-bound для `_incomplete_onec_pipeline`** (2.E.5) | Порог давности state (например, `updated_at` не старше 14 дн.) либо требование совпадения slug с сессионным сигналом | unit: брошенный месяц назад пайплайн не навешивает гейт | 1 ч |
