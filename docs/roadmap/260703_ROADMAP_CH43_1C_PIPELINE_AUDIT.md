...
6. **~~Пред-существующая тест-инфра — коллизия `shared`~~ → ОПРОВЕРГНУТО (2026-07-04):** в ходе P0/P1/P2 я ошибочно записал, что полный сбор `pytest tests/unit/` падает на семействе хук-`shared`-тестов из-за затенения `.claude/hooks/shared` пакетом `src/shared`, и что это «держит CI красным, а новые гейты декоративны». **Проверка чистой инвокацией (`pytest tests/ -m unit`, как в CI) это НЕ подтвердила:** сбор — 0 ошибок коллекции (2234 собрано), а всё gate/hook-семейство (`test_gate_parity`/`test_gate_parity_harness`/`test_gate_policies`/`test_sonar_rescan_state`/`test_llm_health`/`test_pipeline_state_colocation`/`test_pipeline_1c_bridge`/`test_bsl_lint_format`) — **597 passed, 0 failed** в контексте полного `tests/`. Коллизия воспроизводилась ЛОКАЛЬНО только при `pytest tests/unit/ -k …` с загрязнённым за длинную сессию кэшем `sys.modules` — **артефакт локального тулинга, НЕ блокер CI**. Следствие: новые CI-гейты (P3.4 parity-harness, P3.5 doc-drift линтер, P4.3 регресс-детектора) **работают по-настоящему**, а не декоративны; выделенный «Gate parity (isolated)» шаг — избыточная страховка, тесты прошли бы и в основном прогоне. Это живой пример переоценки из п.4 (я применил её к себе). Если CI красный — по независимым пред-существующим причинам, вне scope 260703.
...
| 2026-07-04 | **Поправка (интеллектуальная честность): коллизия `shared` НЕ блокирует CI** | Ранее (в ходе P0/P1/P2) записал «системный долг: полный сбор падает на shared-коллизии → CI красный → новые гейты декоративны». Проверка чистой инвокацией `pytest tests/ -m unit` (как в CI) **опровергла**: 0 ошибок коллекции (2234 собрано), gate/hook-семейство **597 passed / 0 failed** в контексте полного `tests/`. Коллизия — локальный артефакт `tests/unit/ -k` + загрязнённого кэша длинной сессии, НЕ CI-блокер. Новые CI-гейты (parity-harness/doc-drift/регресс-детектора) работают реально. §5 п.6 исправлен. Живой пример переоценки scope (п.4) — на себе. | §5 п.6 + §18 |
| 2026-07-04 | **P4 (2/3 реализовано, P4.2 осознанно отложен)** | **P4.1** `bsl_lint.py --sarif` → SARIF 2.1.0 (`build_sarif`), схема ИДЕНТИЧНА `sonar_issues_pull.build_sarif` (единый findings-контракт ADR-034 R4 — оба BSL-линтера дают один формат для будущего findings-роутера; severity bsl-ls→SARIF level); +3 unit (схема/уровни/empty/совпадение формы с sonar) [SARIF 2.1.0]. **P4.3** `eval_1c_detector.py --min-f1`/`--min-route-acc` → exit 1 при регрессе + CI advisory-шаг «1С-detector regression» (полы 0.90, текущее is_1c F1=0.974/route-acc=0.972 на 248-GT); CV-в-CI отклонён (regex-детектор детерминирован → k-fold метрика та же ± шум, осмысленно лишь для SetFit-модели, которой нет в CI), рост GT 248→400 отложен (задача разметки). **P4.2** child-workflow оркестрация `/fix-sonar-task` — **отложен по критерию самого roadmap** («когда объём Sonar-задач вырастет») + YAGNI: per-cluster child-паттерн (строго последовательно, свой state, R1/R2) УЖЕ задокументирован в `fix-sonar-task` SKILL Шаг 3; автоматизация в Workflow-скрипт преждевременна до роста объёма. Коммит `9f02878a2`. | `bsl_lint.py` + `eval_1c_detector.py` + `ci.yml` + `test_bsl_lint_format.py` |
| 2026-07-04 | **P3 РЕАЛИЗОВАН (6/6, паттерны лидеров, [ADR-044](../../.claude/skills/architecture-research/adr/044-gate-pipeline-robustness-leader-patterns.md))** | **P3.4** parity-harness `test_gate_parity_harness.py` (256-комбинационная композиция `evaluate_gates == AND живых хуков`) + CI-шаг «Gate parity (isolated)» → рассинхрон валит CI, повтор G-1 невозможен [OPA]. **P3.5** `scripts/lint_ch43_sync.py` — doc-drift линтер (5 инвариантов, факты из кода ast+regex) + CI advisory-джоб → автоматизирует 4-слойную ручную сверку сессии [own+code-anchored]. **P3.1** `_fix_recipe` — block-сообщение = copy-paste команды только для ✗-петель (sonar fail-файлы прокинуты) + фикс WebSearch-дрейфа в pipeline-protocol block [koto {{gate_output}}]. **P3.2** `pipeline_state.bump_attempt`/`set_needs_human` + CLI + run-1c-task bounded-петля (env `ONEC_MAX_FIX_ITERATIONS`=4, needs-human p0/p1/p2) [agentico/Graybark]. **P3.3** критики дизайна: 2-3 параллельных субагента ANALYSIS-REPORT перед авто-approve, blocker→гейтованный поток [agentico]. **P3.6** compound-learning: capture error-fix при эскалации/FAIL [Graybark]. 369 таргетных тестов (P3.4 harness 298 + P3.2 +3 + P3.1 +2), линтер CLEAN, compile OK. Коммиты 9e9e8e644…21c7426c3. Пайплайн `ch43-p3-leader-patterns`. | 6 файлов + 2 новых скрипта/теста + ADR-044 |
| 2026-07-04 | **P2 verify (адверсариальный ре-аудит) → закрыт** | **Механический слой:** fences 27/27, ноль `.py#L`, ноль битых относительных ссылок (полный линк-чек), grep-матрица плохих паттернов чиста; 1 пропуск найден и исправлен (43.2 preflight 4→5 режимов, `e5944673f`). **Адверсариальный ре-аудитор** (свежие глаза без эталон-фактов, read-only против кода): подтвердил ядро синхронизации (тиры/веса/Sonar-статусы/6 числовых примеров пересчитаны/~60 MCP-тулов существуют/tier-метки согласованы) и вынес **RESIDUAL**: 4 дрейфа + 2 внесённые ошибки + 4 противоречия. Все исправлены (`24af588e6` + auto-save), ключевое: C4-пример «ПолучитьЭмбеддинг без langchain → без veto» был ложен — маркер «эмбеддинг» сидит в САМОМ идентификаторе (`_NON_1C_CONTEXT`, live-прогон) → контраст заменён на `ПолучитьОстатки`→ask_flow; формула confident получила guard `is_1c` (strong требует глагол); фазовый маппинг 43.5→1-3/4-5; §6-box +SONAR+оркестратор; **корень дрейфа «WebSearch GitHub» в `run-1c-task/SKILL.md` шаг 1.5** → канонические скрипты; off-by-one 43.5.6; RESEARCH в 43.4 +P0.4-маркеры; age-bound H5 нота. Каждый фикс предварительно сверен с кодом/live-прогоном. | `e5944673f` + `24af588e6` |
| 2026-07-03 | **P2 РЕАЛИЗОВАН (синхронизация 27 файлов главы)** | 5 параллельных субагентов-редакторов (по файл-группам) синхронизировали главу 43 с кодом после P0/P1, каждому дан проверенный эталон + мандат сверять с кодом. **P2.1/P2.2** (43.5/43.5.4): 6-flow, actionless→ask_action, confidence-тиры, детектор-уровни, 248-GT, Д-1 denylist, SetFit=ON+7 env-ручек, примеры-якоря пересобраны живым прогоном модели. **P2.3** (43.3/43.6): 6-flow, SONAR-петля, G4+OpenSpec, effort develop/ttype, детектор-каскад, preflight 5, onec-state-first-guard, correlationid P1.2. **P2.4** (43.1/43.2): фазы 1-3/4-5, фантомы 3R→bsl-symbol-editing, YAxUnit-трек, /fix-sonar-task. **P2.5** (43.7/43.8): changed-lines+baseline_degenerate+tz-фикс, 18 тестов, новая секция «оркестратор↔живые хуки» (инцидент G-1, parity). **P2.6** (43.4/43.9.x): внешний анализ→ecosystem_scan/onec_search, serena vendored-only, ast-grep канал, codepilot1c ревизия, SonarQube MCP. **P2.7**: 17 устаревших .py#L-якорей сняты (strip-скрипт) + якорная политика в чеклист 43.6. **P2.8**: процедура «grep ADR + бампни Состояние» в инвариант 43.6. **Верификация (мой гейт над делегированием):** 3 субагента сами поймали и исправили ошибки (P2.6 фабрикация compare_semantic→реальные имена; P2.1/P2.5 честные ноты про 230-vs-248 GT); я поймал и исправил 2 внесённых субагентами дрейфа — P2.6 придумал «T3-T4»-таксономию (код: Тир-2/ADR-035 + T1/ADR-036 для find_callers) и 43.5 остаточный фантом+WebSearch. Все факты сверены с кодом (0 непроверенных). Коммиты b2db15c9e…b76ce3e21 (8 инкрементальных). Пайплайн `ch43-p2-doc-sync`. | 27 файлов главы 43 + strip_anchors.py |
| 2026-07-03 | **P1 runtime-verify** | **Живой прогон V1-V7 каждого фикса (9/9 PASS):** V1 P1.5 — реальный артефакт папки GKSTCPLK-2637 резолвится в её зарегистрированного владельца `gkstcplk-2182` (не CURRENT), чужой путь → None-fallback; V2 P1.7 — `_incomplete` вернул свежий slug (07-02, внутри age-bound); V3 P1.3 — при реальном CURRENT=не-1С advisory эмитится; V4 P1.1 — порог прочитан из meta.json (0.5 калиброванный; путь делегации отдельно доказан unit-моком 0.83); V5 P1.6 — живой хук на allow пишет allow-запись с `sonar_rescan` (decisions → scratch через `GATE_DECISIONS_LOG`, ноль загрязнения); V6 P1.2 — BaseHook-лог реального хук-процесса несёт `run_id`+`correlationid=run_id` (set_run→subprocess→tail→clear_run); V7 P1.8 — degraded-запись пишется. Ревьюер-субагент P1: **PASS** по 8/8 инвариантам; его LOW-находка (первый- vs глубочайший-предок) исправлена `72f460ca8` (+1 unit). Синтетика вычищена (advisory −1, invocations −3). | `scratchpad/verify_p1.py` (сессионный) |
| 2026-07-03 | **P1 РЕАЛИЗОВАН (7/9)** | **P1.5** `_owning_1c_slug` — владелец артефакта по state_dir-предку / любому JIRA пути (nested-безопасно) до CURRENT-fallback (класс «чужой задачи»). **P1.7** `_incomplete_onec_pipeline` age-bound (env `ONEC_INCOMPLETE_MAX_AGE_DAYS`=14; скип только заведомо-старых [dt<cutoff], missing-метка сохранена) + tie-break на свежайший. **P1.3** `gate_1c_implement` advisory-поле при no-op → `pipeline-gate` эмитит systemMessage (G4 не молчит). **P1.1** `_setfit_threshold_safe` — калиброванный порог из `onec_setfit_gate.threshold()` (meta.json), env только override. **P1.2** `BaseHook.run()` пробрасывает `get_run_id` → нативные тулы коррелируются (`tool_usage_report --run-id`). **P1.6** onec-хук логирует allow реальной 1С-задачи (симметрия decision-log). **P1.8** `_log_degraded` в build_context (load/onec-eval) + sonar-except → fail-open виден в аудите. **Деферал (осознанный, simplicity):** P1.9 (light/pos) — правка `pos` флипнула бы foldered-косметику medium→simple (формальная ТЗ → AUTO без ревью); current pos==0-guard интенционален → документируем, не меняем. P1.4 (advance_test_done раннера) — косметика display этапа, Variant B трогает Stop-flow (риск ради nicety) → отложено. Тесты: 181 таргетный + 18/18 runtime pre-flight (P0 цел), ruff чисто. | `onec-task-completion-stop.py` + `pipeline_1c_bridge.py` + `gate_policies.py` + `pipeline-gate.py` + `base/protocol.py` + 12 unit |
