...
- **P3.B Сервер ✅ реализовано 2026-07-06:** провижининг 3 проектов через `run-sonar-analysis.ps1` (SONAR_SPLIT_PROJECTS=1): все **EXECUTION SUCCESS + CE SUCCESS** (utp-ib 640k ncloc/48 BLOCKER/2105 bsl, utp-svetly 638k/48/2093, utp-mfm 830k/123/2991). **A9 heap OK** (1 конфиг на скан ≪ mono-3-конфига). Baseline пропинан per-project (P2.2, SPECIFIC_ANALYSIS на первый анализ, branch=main). **Live-smoke резолюции:** component-ключи формата `<proj>:src/...` → `_match_component(comp_rel="src/…")` резолвит True для всех 3 (split-verify рабоч). Per-config `.scannerwork`/CE-handoff отработал (Finding-1 fix). SCM blame — оживёт при следующем скане (projectBaseDir=git work-tree конфига).
- **P3.C Переключение ✅ реализовано 2026-07-06 (локальный флип):** `SONAR_SPLIT_PROJECTS=1` в `.env` (gitignored) — читают И `run-sonar-analysis.ps1`, И гейт-verify (`sonar_rescan_verify.split_enabled` через `_dotenv`→`os.environ`, подтверждено live: `split_enabled()` из `.env`=True). Гейт теперь split-mode. **Реверс = убрать строку из `.env`** (mono-проект `upravlenie-transportom-plk` сохранён как rollback-цель — НЕ архивирован). **Осталось (отдельные решения):** repo-wide дефолт (код `split_enabled`/ps1) + CI-цикл `ci-1c.yml` (риск CI-скана без 3 проектов — сейчас CI не читает локальный `.env`, остаётся mono); docs (гл.43.9.9 + fix-sonar-task/implement); архивация mono ≥30 дней. **Операционка:** полный split-скан (без `-Project`) = 3 конфига последовательно (~6 мин); точечный = `run-sonar-analysis.ps1 -Project <key>`.
...
### 2026-07-06 — P3.B провижининг + P3.C флип: split АКТИВЕН (роадмап 260706 закрыт)

- **P3.B:** провижининг 3 проектов (SONAR_SPLIT_PROJECTS=1) — все EXECUTION SUCCESS + CE SUCCESS (utp-ib/utp-svetly/utp-mfm; ncloc 640k/638k/830k; A9 heap OK на 1 конфиг). Baseline пропинан per-project (P2.2, первый анализ, branch=main). Live-smoke: component-ключи `<proj>:src/…` → `_match_component` резолвит True ×3. По ходу — PS 5.1-гоча: `@($json|ConvertFrom-Json)` оборачивает JSON-массив в `[Object[3]]` → `$proj.root`=Object[]; фикс = убрать `@()` (ConvertFrom-Json сам отдаёт Object[]).
- **P3.C (локальный флип):** `SONAR_SPLIT_PROJECTS=1` в `.env` — читают ps1 И гейт-verify (`split_enabled` через `_dotenv`→`os.environ`, live=True). Гейт split-mode; verify-санити (нет изменённых→pass) чист. Реверс = убрать строку; mono-проект сохранён (rollback). Repo-wide дефолт (код+CI) + docs + архивация mono — отдельные решения.
- **Роадмап 260706 ПОЛНОСТЬЮ закрыт: P0 ✅ P1 ✅ P2 ✅ P3 (A0+registry+A7+A.wiring+B+C) ✅.**
