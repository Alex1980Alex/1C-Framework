...
**Ре-аудит 260712 (3 адверсариальных ревьюера по кластерам W-fix / W4′ / B1+C1+W5′ + операционные проверки):** W-fix PASS, B1/C1/W5′ PASS, W4′ PARTIAL. **Волна fix-260712** закрыла найденные стыки (мета-урок §6.2 аудита 260710 снова подтверждён — дефекты живут на СТЫКАХ фич): M-5-класс в A5/C4 arm (`skipped_collisions` — чужой logpoint/BP больше не затирается и не сносится teardown'ом), A5 collect teardown → `finally` (весь путь), `_drain_snapshot_tasks` перед replay/diff-чтением (M-4-класс), метрики `_stop_events`/`_bp_by_location` → innermost `stack[-1]` (Ф-2-sweep завершён), XML-escape eval-выражений. +8 unit → **592 passed**. Честные поправки DONE-нот: C2 en-fqn/дубль-warning не реализованы (см. §8.2). Backlog (не блокеры): en-fqn resolve, `artifacts.py` в watch-list, M-9 conftest, наблюдаемость watch-halt в session_summary (F-4), `_watch_last` per-target (F-5). Детали — [260710 аудит, секция «Ре-аудит 2026-07-12»](260710_AUDIT_260708_AUTONOMOUS_1C_DEBUGGING.md).
