...
> **Статус:** RESEARCH + ROADMAP + **W1 ✅** (2026-07-08, A0/A1/C0/B2/W1.0, live-verified) + **W2 ✅ кроме B1** (2026-07-09/10: B3 event-wait, B5.a/b/c/d переносимость+уборка+Python-CI+P0-креды) + **W3 A2/A3/B4 ✅** (2026-07-10: root-cause / trace-variable / heartbeat-auto-reattach). **425 unit passed, code-verify PASS по каждому пункту.** Осталось (оба нужен live-ИБ): **B1** (persistent JOB-контекст, ADR-049) + **C1** (setExpression, live-зонд RDBG). Не начато (nice-to-have, не блокеры): W4 (A4/C2/A6), W5 (A5/C3/C4/C6), C7; defer C5.
...
**Легенда статуса (обновлено 2026-07-10):** ✅ реализовано · 🟡 частично · ⬜ не начато · ⏸ отложено (live-ИБ / vendor). Сделано: **W1** (A0/A1/C0/B2), **W2** (B3/B5.a-d) кроме B1, **W3** A2/A3/B4. Детали каждого — в статус-блоках вверху карты.
...
| 🟡 **W2** (надёжность) | B1 + **B3 ✅** + **B5 ✅** (a/b/c/d; d = b1+: Python CI, P0 креды, jar untracked) | ~11ч | Интерактивный JOB-step (B1 pending — ЕДИНСТВЕННЫЙ остаток W2); **sub-second trap ✅** (B3); **переносимость на SVETLY/MFM ✅** (B5.a/b); **root declutter + живой CI ✅** (B5.c/d) |
| 🟡 **W3** (диагностика) | **A2 ✅** + **A3 ✅** + C1 + **B4 ✅** | ~11ч | **Root-cause diagnosis-record ✅** (A2); **trace-variable ✅** (A3); runtime hypothesis-test (C1 — единственный остаток W3, нужен live-зонд RDBG); **auto-reattach ✅** (B4) |
