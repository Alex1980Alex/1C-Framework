...
|---|---|---|---|---|---|
| **B1** | Persistent JOB-контекст (P-1) | JOB-halt эфемерен: rphost фонового задания живёт **<100ms** (spawn→строка→quit до attach+BP) | Спавнить **долгоживущий** debug-worker: `гкс_ОтладкаВыполненияКода.ВыполнитьКодСУдержанием` — после исполнения держит контекст через ожидание сигнала (флаг в РС/врем.файле), пока агент читает/шагает; освобождение по `debug_step(Continue)`. Убирает гонку на корне (CDP-модель paused-on-entry). Заодно оформить существующий `debug_capture_mode` (sticky re-arm) как дефолт-политику в начале записи сессии | ~5ч | medium (BSL-модуль в БД + деплой) |
...
1. **B1 требует BSL-модуль в БД** (`ВыполнитьКодСУдержанием`) — деплоить в `ИБTransportManagementDevelop` + `SVETLY` + `MFM`, или только dev-ИБ?
