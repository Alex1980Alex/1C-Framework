...
> ✅ **DONE 2026-07-11 (сабмодуль `eb3d425`):** C2.1 `uuid_index._reverse_fqn_entries` (обратная карта fqn→(object_id,module_type), зеркалит `get_source_info`) + `find_by_fqn` (lazy, thread-safe) + C2.2 `_first_executable_line` (через C3-классификатор) + C2.3 tool `debug_set_function_breakpoint` (rpartition→find_by_fqn→resolve→find_method_range→first_executable→делегирует `debug_set_breakpoint`, B2-offset) + C2.5 11 unit + C2.6 live (реальный EDT 3506 объектов/7782 fqn: `ПеревестиТекст` header 11→first_exec 13, `ПараметрыАдминистрированияКластера` 35→37). code-verify PASS (Rec2 гонка `_reverse` под локом - пофикшен). **Отложено:** C2.4 auto_calibrate fallback (B2-offset уже покрывает большинство сдвигов; calibrate - отдельный тул). **⚠ Поправка ре-аудита 260712 → ✅ ЗАКРЫТО 2026-07-12:** en-fqn resolve реализован (`uuid_index._KIND_FQN_EN` + `_reverse_fqn_entries` строит И RU, И EN ключи через `setdefault` — `Document.X.ObjectModule`/`CommonModule.Y`/`Document.Z.Form.W` теперь резолвятся; RU-путь аддитивно не тронут) и warning на дубль имени метода реализован (`autonomy.count_method_definitions` + общий `_method_head_re`; `debug_set_function_breakpoint` отдаёт `warning`+`definitions_found`, BP на первом). +11 unit (`test_reaudit_260712_tails.py`). **Follow-up (пре-существующий, вне C2):** `MODULE_PROPERTY_FILES` (`uuid_index.py:36`) мапит RecordSetModule-pid `9f36fd70`→`ManagerModule.bsl` вместо `RecordSetModule.bsl` (противоречит метке в `MODULE_PROPERTY_IDS:63`) - для регистров с логикой в наборе записей резолв читает не тот файл (обычно вырождается в `method_not_found`; общий баг resolve() с C3 line-BP, не дефект C2).
...
**Ре-аудит 260712 (3 адверсариальных ревьюера по кластерам W-fix / W4′ / B1+C1+W5′ + операционные проверки):** W-fix PASS, B1/C1/W5′ PASS, W4′ PARTIAL. **Волна fix-260712** закрыла найденные стыки (мета-урок §6.2 аудита 260710 снова подтверждён — дефекты живут на СТЫКАХ фич): M-5-класс в A5/C4 arm (`skipped_collisions` — чужой logpoint/BP больше не затирается и не сносится teardown'ом), A5 collect teardown → `finally` (весь путь), `_drain_snapshot_tasks` перед replay/diff-чтением (M-4-класс), метрики `_stop_events`/`_bp_by_location` → innermost `stack[-1]` (Ф-2-sweep завершён), XML-escape eval-выражений. +8 unit → **592 passed**. **Хвосты закрыты волной tails-260712 (+11 unit → 602 passed, code-verify PASS):** C2 en-fqn resolve + дубль-warning (§8.2), F-4 наблюдаемость watch-halt в session_summary (`_watch_halt_count` + `_stop_events` reason="watchpoint"), F-5 `_watch_last` per-`(target,name)`, M-9 conftest autouse `.active.json`→tmp (защита живой HMR-сессии), `artifacts.py` в watch-list. Остаётся ТОЛЬКО C-1 (ротация пароля/history-rewrite публичного форка — outward-facing, за пользователем) + push сабмодуля (публикация). Детали — [260710 аудит, секция «Ре-аудит 2026-07-12»](260710_AUDIT_260708_AUTONOMOUS_1C_DEBUGGING.md).
