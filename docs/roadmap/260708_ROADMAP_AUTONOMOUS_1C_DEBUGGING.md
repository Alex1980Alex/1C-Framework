...
| 🟡 **B5** | Переносимость + housekeeping (техдолг код-агента) | `uuid_index.py:66-69` **hardcoded** `C:\1С-Framework\ИБTransportManagementDevelop\…` → блокер на др. ИБ (SVETLY/MFM); UUID-cache invalidation по mtime **корня** src (coarse — вложенный `.mdo` не инвалидирует); ~30 `test_rdbg*` + `.log` в корне; неиспользуемые Java-артефакты (jar 12МБ + vsix 11МБ + `src/`) | **(a) ✅** config-path через env `BSL_DEBUG_CONFIG_SRC_MAP` per-alias + per-alias `UUIDIndex`-реестр (сабмодуль `91e89a8`); **(b) ✅** рекурсивный fingerprint `{max_mtime,count,total_size}` для cache-invalidation; **(c) ✅** 32 gitignored dev-scratch (`test_rdbg*`/`test_full*`/`test_quick*`) перенесены в `archive/` (в `.gitignore`), product-код не тронут (сабмодуль `d5b16f6`); **(d) ⏸ ТРЕБУЕТ РЕШЕНИЯ** — «Java-артефакты» = **tracked** upstream yukon39 (`src/main/java/…`, 40+ файлов, roadmap §2.5 цитирует как RDBG-reference) + `bsl-debug-server-1.1-SNAPSHOT.jar` 12МБ + **сабмодульный CI `.github/workflows/build.yml` = `./gradlew check sonarqube`** (единственный CI сабмодуля, Python-тесты гоняются вне его). Удаление = de-Java рефактор + замена CI на Python (pytest+ruff), НЕ «уборка»; убирает цитируемый reference; git-history блобы остаются. Опции: (b1) заменить CI на Python, `src/` оставить как reference; (b2) полный de-Java (rm src/gradle/jar + Python CI); (b3) defer. Не выполнять без явного решения | ~3ч | low (c) / **medium (d) — CI+identity** |
...
| 🟡 **W2** (надёжность) | B1 + **B3 ✅** + B5 (**B5.a/b/c ✅**, B5.d ⏸ de-Java) | ~11ч | Интерактивный JOB-step (B1 pending); **sub-second trap ✅** (B3); **переносимость на SVETLY/MFM ✅** (B5.a/b); **root declutter ✅** (B5.c) |
