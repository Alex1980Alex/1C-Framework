...
### 2026-07-19 — рек.3 follow-up: методика+доки YAxUnit сверены с исходником (галлюцинированный API исправлен)

Мандат: проанализировать реализацию рек.3, обогатить скиллы работы с тестами данными GitHub/интернета,
проверить документацию и дополнить при неполноте.

- **Код гейта рек.3 — корректен, НЕ тронут**: `onec-task-completion-stop` (advisory `•`, единственный маршрут
  `mcp-onec-test-runner`) + applicable-сигнал `onec_change_scope.edits_exported_method` + метрика
  `onec_toolgate_validation.yaxunit_rate_on_applicable`. Слабое место — не в гейте, а в **методичке, которую
  агент читает, когда решает написать смоук**.
- **Диагноз (главное)**: 3 файла (skill `yaxunit-unit-testing`, гл. 17.6, `/write-1c-unit-tests`) несли
  **галлюцинированный YAxUnit-API** — `Мокито.Создать/Обучить/Параметры`, `ОжидаетВыброс`,
  `ДобавитьСерверныйТестСКонтекстом`, `СодержитЗаписьСПолями`, `УстановитьЗначениеПоля().Сохранить()`,
  `ЮТТесты.ДобавитьМодуль`, «авто-дискавери без регистрации». Тест по такой методике падал бы «Метод не найден».
- **Research** (3 параллельных агента, под caveat-контролем): офиц. доки bia-technologies + GitHub-экосистема +
  Infostart-практики. Ключевое: `mcp-onec-test-runner`=`alkoleft/METR`; `Coverage41C`→`genericCoverage.xml`→Sonar
  (ADOPT для нас); «smoke» = ТРИ разных понятия (`ЮТДымовыеТесты` functional whole-config ≠ рек.3-смоук unit
  затронутого модуля ≠ syntax-check компиляции); безопасный режим ломает и Мокито (офиц. `run.md`).
- **Фикс**: КАЖДЫЙ API-символ сверен с вендоренным `src/bsl/exts/YAXUNIT/` (авторитет для развёрнутой версии).
  Skill переписан (519→367 body, полный справочник вынесен в `references/api-reference.md`); frontmatter
  восстановлен (регресс NODESC исправлен). Гл. 17.6 §3/§4/§5 исправлены + §9 (рек.3/безопасный-режим/CI-раннеры/
  покрытие с URL). `/write-1c-unit-tests` хирургически. 0 остаточных галлюцинаций (grep), `lint_skills` clean,
  память `reference-yaxunit-api-ground-truth`. Пайплайн `pipeline/enrich-yaxunit-test-docs/` (4 этапа).
- ⚠ Прогон живого YAxUnit-теста по новой методике — за отдельной 1С-задачей на живой ИБ (среда пользователя).
...
codepilot `qa_*` (6 тулов BDD-через-MCP), `auto-documenter autotestplan`, `bsl-debugger`.

**Применяется (live, окно лога ~14д):** ВСЕ перечисленные — **0 вызовов**; slash тест-команды —
0 запусков; `.run-state.json` — ни одного файла в папках задач; UnitTests-расширение — 1 реальный
тест-модуль (`гкс_РассылкиИОповещенияТест`) + пример. Этап 4 done — **9/28** 1С-пайплайнов
(19 без тестирования; ⚠ открытый вопрос — чем закрыты 9 при отсутствии `.run-state` артефактов).
...
прежнему политика пользователя, но **раннер заземлён реальным сценарием** (2026-07-19): не только
смоук — многошаговый UI-сценарий на **реальных данных** (`01_open_labanaliz.feature`: открыть список
`гкс_ЛабораторныйАнализ` → перейти к строке таблицы «Список» → открыть существующую запись → проверить
ФормаДокумента → закрыть) прошёл `junit tests=1 failures=0 errors=0 time=19.7с` через `qa_run` binary-путь.
Доказано: раннер драйвит реальную навигацию списка + открытие записи, не только запуск клиента. ⚠ ПОЛНАЯ
ARM-цепочка со сменой состояний (06_arm_workflow) — на транспортной конфе (не настроена под qa_run здесь;
MFM TestDB пуст → rich-lifecycle нечем наполнить); её прогон = отдельная setup-задача (транспортный
EDT-workspace + qa-config + TestDB, + известные инфра-блокеры расписания из старого run-state). (3) ✅ **РЕАЛИЗОВАНО (шаг 1,
2026-07-19)**: контракт `advance_test_done` расширен — этап 4 доказуем ЛИБО `.run-state.json` (BDD),
ЛИБО секцией live-данных ADR-050 в IMPLEMENTATION-PROGRESS с чистым PASS (`_live_data_test_passed`,
консервативно; caller зовёт оба хелпера). Закрыл дыру группы C («done» без тест-артефакта): теперь
«done» всегда имеет конкретное доказательство. Разбор 18 задач (не 9 — 14д-окно vs вся история):
15/18 честно протестированы live-данными, 3 — только пометка без файла-следа; новый контракт делает
след обязательным. 11 unit + smoke + code-verify. ⚠ **Среда:** dev-лицензия ограничивает число
...
- ✅ **Хардинг детектора (2026-07-19, commit a3c78593b, `pipeline/fix-yaxunit-advisory-hardening/`)** —
  адверсариальный аудит рек.3 + сверка с грамматикой 1c-syntax/bsl-parser (BSLLexer.g4): (1) async-
  ключевое слово — **`Асинх`/`Async`**, прежний префикс «Асинхронная» словом грамматики не является →
  async-методы не матчились заголовком ВООБЩЕ (FN, «async-фикс» code-verify был с неверным словом);
  (2) ключевые слова двуязычны — добавлены `Procedure|Function|Export|EndProcedure` (кириллица-only =
  FN на англоязычных модулях); (3) строковые литералы вырезаются до поиска `Экспорт` (FP на
  `П = "Экспорт"` в сигнатуре); (4) **session-scoped applicable**: живой замер — 159 чужих WIP `.bsl`
  в сабмодулях → `impact_applicable`/`yaxunit_applicable` вырождались в «всегда True»; сбор
  `sig["bsl_paths"]` из транскрипта (Write/Edit + MCP-арги на `.bsl`) сужает скан, пустое пересечение →
  полный скан (fallback, ошибка только в сторону True); (5) перф-кап `_MAX_FILES_SCAN=50` (2 git-вызова/
  файл под таймаутом Stop-хука). 5 новых unit (саботаж на pre-fix `4e7f4eb5e` красный), 384 passed,
  code-verify PASS. Точность на живом модуле (907 методов): 12 многострочных спанов — все легитимны, FP 0.
