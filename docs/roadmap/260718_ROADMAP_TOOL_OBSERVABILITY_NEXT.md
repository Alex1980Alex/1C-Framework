...
**Проверка.** 22 unit ([`test_tool_health_client_timeout.py`](../../tests/unit/test_tool_health_client_timeout.py))
+ саботаж 5/5 (вычет выключен → 7 красных; парный Pre не забирает свою запись → 1; сопоставление не
расходуется → 1; `CLIENT_CAP_MS=0` → 1; счётчик возвращён в провалы → 2) + полный unit-suite
2725 passed / 2 skipped + ruff/compileall чистые.
