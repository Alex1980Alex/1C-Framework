...
| P1.1.2 | Создать `Neo4jGraphStore(BaseGraphStore)` | `src/pdf_framework/graph_store/providers/neo4j_store.py` | ~250 | Unit test |
...
| # | Подзадача | Файл | Строк | Тест |
|---|-----------|------|-------|------|
| P1.5.1 | Unit test: CRUD operations (mock driver) | `tests/unit/graph_store/test_neo4j.py` | ~80 | Passes |
| P1.5.2 | Unit test: Cypher query generation | Тот же файл | ~40 | Queries correct |
| P1.5.3 | Integration test: real Neo4j (Docker) | `tests/integration/test_neo4j.py` | ~60 | @pytest.mark.slow |
...
| P2.3.3 | Документация: docker compose -f gpu запуск | `docs/guides/deployment.md` | +10 | — |
...
| # | Подзадача | Файл | Строк | Тест |
|---|-----------|------|-------|------|
| P2.4.1 | `docker compose up -d` — все сервисы запускаются | — | — | All healthy |
...
**Acceptance:** `docker compose up -d` запускает все 7+ сервисов. Health checks зелёные.
...
| P3.1.7 | Unit test: all PII types detected | `tests/unit/guardrails/test_pii.py` | ~60 | 100% detection |
...
| P3.2.8 | Unit test: known injection patterns | `tests/unit/guardrails/test_injection.py` | ~60 | All caught |
...
| P3.4.5 | Integration test: injection blocked | `tests/integration/test_guardrails.py` | ~40 | 403/400 returned |
...
| P4.3.5 | Unit test: classifier routing | `tests/unit/agents/test_routing.py` | ~50 | 3 complexity levels |
...
- [ ] Neo4j provider: all BaseGraphStore methods implemented
- [ ] Migration script: NetworkX → Neo4j (3166 entities, 3528 edges)
- [ ] Docker compose: all services start + healthy
- [ ] `nginx.conf`, `prometheus.yml`, `init-db.sql` created
- [ ] PII detector: 7+ PII types, redact/block modes
- [ ] Injection defense: 10+ patterns, scoring, block mode
- [ ] GuardrailsMiddleware registered in app.py
- [ ] Model routing: Haiku/Sonnet/Opus by complexity
- [ ] Cost budget: per-query + daily limits
