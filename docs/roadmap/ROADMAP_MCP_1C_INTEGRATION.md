...
### Фаза 3: Расширенный доступ к данным 🔄 IN PROGRESS (2026-03-14)
...
| Qdrant            | Фаза 3.5 | Уже работает (localhost:6333)                        |
  ...
  Как работает bsl-debug-server                                                                                                                                                                                                                                                                                                 
  ┌─────────────────┐      DAP Protocol      ┌──────────────────┐      TCP      ┌─────────────────┐                                                             
  │    VS Code      │  ◄──────────────────►  │ bsl-debug-server │  ◄─────────►  │ 1С:Предприятие  │
  │  + vsc-bsl-dap  │     JSON-RPC           │    (Java)        │   port 1560   │  (debug agent)  │
  └─────────────────┘                        └──────────────────┘               └─────────────────┘
          │
          ├── Точки останова ставятся ЗДЕСЬ
          ├── Срабатывают ЗДЕСЬ (UI)
          ├── Переменные показываются ЗДЕСЬ
          └── Step control ЗДЕСЬ

  Workflow:

  1. Запускаете 1С с параметром /Debug -TCP -port 1560
  2. Запускаете bsl-debug-server (Java)
  3. Открываете VS Code с расширением vsc-bsl-dap
  4. Ставите breakpoint в VS Code на BSL файле
  5. Запускаете отладку (F5)
  6. 1С попадает на breakpoint → VS Code показывает строку + переменные

  Что видите в VS Code:
  ...
  ┌─ Call Stack ────────────────────┐
  │ ОбработкаПроведения() : 142     │  ← текущая строка
  │ Записать() : 89                 │
  │ ОбработкаКоманды() : 15         │
  └─────────────────────────────────┘
