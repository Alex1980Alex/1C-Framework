...
class LazyMCPTester:
    ...
    def test_initialize(self):
        ...
        if response and "result" in response:
            log_ok("Initialize OK")
            log_ok(f"  Server: {response['result'].get('serverInfo', {}).get('name', 'unknown')}")
            log_ok(
                f"  Version: {response['result'].get('serverInfo', {}).get('version', 'unknown')}"
            )
    ...
    def test_recommend_tools(self):
        ...
        for task, expected_category in test_cases:
            response = self.send_request(
                "tools/call", {"name": "recommend_tools", "arguments": {"task_description": task}}
            )
                ...
                if result_content:
                    ...
                    try:
                        ...
                        if found_category:
                            ...
                            log_warn(
                                f"  '{task[:40]}...' → {categories} (expected: {expected_category})"
                            )
                            self.results["warnings"] += 1
                    except json.JSONDecodeError:
                        log_fail("  Invalid JSON response")
                        self.results["failed"] += 1
            else:
    ...
    def test_get_tools_category(self):
        ...
        # Test root level
        response = self.send_request(
            "tools/call", {"name": "get_tools_in_category", "arguments": {"path": "/"}}
        )
    ...
    def test_execute_tool(self):
        ...
        if response and "result" in response:
            ...
            if result_content:
                ...
                try:
                    ...
                    if "error" not in result:
                        log_ok("execute_tool → ripgrep loaded successfully")
            ...
            log_warn("execute_tool timeout or no response (may need longer timeout)")
            self.results["warnings"] += 1
