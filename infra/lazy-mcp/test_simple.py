...
def main():
    ...
    resp = send("tools/call", {"name": "get_tools_in_category", "arguments": {"path": "/"}}, 3)
    if resp:
        if "result" in resp:
            print("get_tools_in_category: OK")
    ...
    resp = send(
        "tools/call",
        {"name": "recommend_tools", "arguments": {"task_description": "search files"}},
        4,
    )
    if resp:
        if "result" in resp:
            print("recommend_tools: OK")
            content = resp["result"].get("content", [])
            if content:
    ...
    resp = send(
        ...
        {
            "name": "execute_tool",
            "arguments": {"tool_path": "/file-operations/ripgrep/list-file-types", "arguments": {}},
        },
        5,
    )
    if resp:
