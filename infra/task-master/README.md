...
# List all tasks in JSON format (NEW in v0.26.0+)
task-master list --json

# List with filters and JSON output
task-master list --status=pending --json
task-master list --tag=feature --json
...
Task Master includes built-in Cursor editor integration with keyboard shortcuts for seamless workflow:
...
The new `--json` flag enables seamless integration with other development tools:

```bash
# Pipe to external tools
task-master list --json | jq '.stats'

# Save for processing
task-master list --status=pending --json > pending-tasks.json

# Integration with scripts
npx task-master list --json | python process-tasks.py
```

[Full keybindings documentation](.cursor/README.md)
