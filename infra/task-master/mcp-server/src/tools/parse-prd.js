...
export function registerParsePRDTool(server) {
	server.addTool({
		name: 'parse_prd',
		description: `Parse a Product Requirements Document (PRD) text file to automatically generate initial tasks. Supports enhanced parsing with intelligent analysis, context preservation, and in-flight task expansion. Reinitializing the project is not necessary to run this tool. It is recommended to run parse-prd after initializing the project and creating/importing a prd.txt file in the project root's ${TASKMASTER_DOCS_DIR} directory.`,
		...
		parameters: z.object({
			...
			enhanced: z
				.boolean()
				.optional()
				.describe('Use enhanced PRD parsing with intelligent analysis and context preservation'),
			adaptiveCount: z
				.boolean()
				.optional()
				.describe('Automatically determine optimal task count based on PRD complexity'),
			expandTasks: z
				.boolean()
				.optional()
				.describe('Enable in-flight task expansion pipeline for immediate subtask generation'),
			preserveDetail: z
				.boolean()
				.optional()
				.describe('Preserve maximum detail from PRD in task generation (may increase task count)'),
			minComplexityThreshold: z
				.string()
				.optional()
				.describe('Minimum complexity threshold for auto-expansion (1-5, default: 2)'),
			maxSubtasksPerExpansion: z
				.string()
				.optional()
				.describe('Maximum subtasks per expansion (3-10, default: 6)')
		}),
