...
} from './task-manager/generate-test.js';
...
} from './cursor-keybindings.js';

import {
	addContextOptions,
	parseContextOptions,
	getContextOptionsHelp
} from './context/context-options-helper.js';

import {
	enhancePromptWithContext,
	createContextSummary,
	validateContextOptions
} from './context/ai-context-integration.js';
...
function registerCommands(programInstance) {
	...
	// parse-prd command
	addContextOptions(programInstance
		.command('parse-prd')
		...
		.option(
			'--enhanced',
			'Use enhanced PRD parsing with intelligent analysis and context preservation'
		)
		.option(
			'--adaptive-count',
			'Automatically determine optimal task count based on PRD complexity'
		)
		.option(
			'--expand-tasks',
			'Enable in-flight task expansion pipeline for immediate subtask generation'
		)
		.option(
			'--preserve-detail',
			'Preserve maximum detail from PRD in task generation (may increase task count)'
		)
		.option(
			'--min-complexity <number>',
			'Minimum complexity threshold for auto-expansion (1-5)',
			'2'
		)
		.option(
			'--max-subtasks <number>',
			'Maximum subtasks per expansion (3-10)',
			'6'
		))
		.action(async (file, options) => {
			...
			try {
				...
				console.log(
					boxen(
						`${chalk.white.bold('Parse PRD Help')}\n\n${chalk.cyan('Usage:')}\n  task-master parse-prd <prd-file.txt> [options]\n\n${chalk.cyan('Basic Options:')}\n  -i, --input <file>       Path to the PRD file (alternative to positional argument)\n  -o, --output <file>      Output file path (default: .taskmaster/tasks/tasks.json)\n  -n, --num-tasks <number> Number of tasks to generate (default: 10)\n  -f, --force              Skip confirmation when overwriting existing tasks\n  --append                 Append new tasks to existing tasks.json instead of overwriting\n  -r, --research           Use Perplexity AI for research-backed task generation\n\n${chalk.cyan('Enhanced PRD Options:')}\n  --enhanced               Use enhanced PRD parsing with intelligent analysis\n  --adaptive-count         Automatically determine optimal task count from PRD complexity\n  --expand-tasks           Enable in-flight task expansion for immediate subtask generation\n  --preserve-detail        Preserve maximum detail from PRD (may increase task count)\n  --min-complexity <n>     Minimum complexity threshold for auto-expansion (1-5, default: 2)\n  --max-subtasks <n>       Maximum subtasks per expansion (3-10, default: 6)\n\n${chalk.cyan('Examples:')}\n  ${chalk.gray('# Basic usage')}\n  task-master parse-prd requirements.txt --num-tasks 15\n  task-master parse-prd requirements.txt --research\n  \n  ${chalk.gray('# Enhanced parsing')}\n  task-master parse-prd requirements.txt --enhanced --adaptive-count\n  task-master parse-prd requirements.txt --enhanced --expand-tasks\n  task-master parse-prd requirements.txt --enhanced --preserve-detail --max-subtasks 8\n\n${chalk.yellow('Note: This command will:')}\n  1. Look for a PRD file at ${TASKMASTER_DOCS_DIR}/PRD.md by default\n  2. Use the file specified by --input or positional argument if provided\n  3. Generate tasks from the PRD and either:\n     - Overwrite any existing tasks.json file (default)\n     - Append to existing tasks.json if --append is used\n  4. With --enhanced: Use intelligent PRD analysis for better task generation\n  5. With --expand-tasks: Automatically expand complex tasks into subtasks`,
			...
			// Enhanced PRD options
			const enhanced = options.enhanced || false;
			const adaptiveCount = options.adaptiveCount || false;
			const expandTasks = options.expandTasks || false;
			const preserveDetail = options.preserveDetail || false;
			const minComplexity = parseInt(options.minComplexity, 10) || 2;
			const maxSubtasks = parseInt(options.maxSubtasks, 10) || 6;
			...
			try {
				...
				await parsePRD(taskMaster.getPrdPath(), outputPath, numTasks, {
					...
					minComplexityThreshold: minComplexity,
					maxSubtasksPerExpansion: maxSubtasks,
					// Context options
	...
	programInstance
		...
		.description('Generate Jest test files for specific tasks using AI')
		.option(
			'-i, --id <id>',
			'Task ID to generate tests for (required)'
		)
		.option(
			'--ids <ids>',
			'Comma-separated list of task IDs to generate tests for'
		)
		.option(
			'-f, --file <file>',
			'Path to the tasks file',
		...
		.option(
			'--prefix <prefix>',
			'File prefix for generated test files',
			'task_'
		)
		.option(
			'--overwrite',
		...
		.option(
			'--no-validate',
			'Skip validation of generated test content'
		)
		.option(
			'--research',
			'Use research model for enhanced test generation'
		)
		.option(
			'--continue-on-error',
			'Continue generating tests even if some fail'
		)
		.option('--tag <tag>', 'Specify tag context for task operations')
		.action(async (options) => {
			try {
				// Check if ID or IDs are provided
				if (!options.id && !options.ids) {
					console.error(chalk.red('Error: Either --id or --ids must be specified'));
					console.error(chalk.yellow('Example: task-master generate-test --id=1'));
					console.error(chalk.yellow('Example: task-master generate-test --ids=1,2,3'));
					process.exit(1);
				}
				...
				// Determine which tasks to generate tests for
				const taskIds = options.ids
					? options.ids.split(',').map(id => id.trim())
					: [options.id];

				console.log(chalk.blue(`Generating tests for task(s): ${taskIds.join(', ')}`));
				console.log(chalk.blue(`Using tasks file: ${tasksPath}`));
				...
				if (taskIds.length === 1) {
					...
					results = await generateTestForTask(taskIds[0], generateOptions);

					if (results.success) {
						console.log(chalk.green(`✓ Successfully generated test file: ${results.filename}`));
						console.log(chalk.gray(`  Lines generated: ${results.linesGenerated}`));
						console.log(chalk.gray(`  Output path: ${results.outputPath}`));
					} else {
						console.error(chalk.red(`✗ Failed to generate test for task ${taskIds[0]}: ${results.error}`));
						process.exit(1);
					}
				} else {
					// Multiple tasks generation
					results = await generateTestsForTasks(taskIds, generateOptions);
					...
					if (results.failed > 0) {
						...
						failures.forEach(failure => {
							console.log(chalk.red(`  - Task ${failure.taskId}: ${failure.error}`));
						});
	...
	programInstance
		...
		.option('--tag <tag>', 'Specify tag context for task operations')
		.option('--json', 'Output in JSON format instead of formatted text')
		.action(async (options) => {
			// Initialize TaskMaster
			const initOptions = {
				tasksPath: options.file || true,
			...
			const result = await listTasks(
				...
				withSubtasks,
				options.json ? 'json' : (compact ? 'compact' : 'text'),
	...
	addContextOptions(programInstance
		...
		.option(
			'--with-prd-context',
			'Use PRD context for enhanced expansion (if available from task generation)'
		)
		.option(
			'--preserve-context',
			'Preserve and enhance PRD context in generated subtasks'
		)
		.option(
			'--file <file>',
	...
	programInstance
		...
		.action(async (taskId, options) => {
			...
			if (options.json) {
				// JSON output mode
				if (taskIds.length > 1) {
					await displayMultipleTasksJSON(
						taskMaster.getTasksPath(),
						taskIds,
						taskMaster.getComplexityReportPath(),
						statusFilter,
...
Examples:
  ...
  $ task-master models --setup                            # Run interactive setup`
	...
	programInstance
		...
		.description('Install pre-configured Cursor keybindings for Task Master')
		.option('--dry-run', 'Show what would be done without making changes')
		.option('--backup', 'Create backup of existing keybindings (default: true)', true)
		.option('--custom-keys <keys>', 'Custom key combinations (JSON format)')
		.option('--force', 'Overwrite existing Task Master keybindings')
		.action(async (options) => {
			try {
				await installCursorKeybindings(options);
			} catch (error) {
	...
	addContextOptions(programInstance
		...
		.option(
			'--file-prefix <prefix>',
			'Prefix for generated test files',
			'task_'
		)
		.option(
			'-r, --research',
			'Use research capabilities for enhanced test generation'
		)
		.option(
			'--overwrite',
			'Overwrite existing test files'
		...
		.option(
			'--continue-on-error',
			'Continue generating tests even if one fails (for batch mode)'
		)
		.option('--tag <tag>', 'Specify tag context for task operations'))
		.action(async (options) => {
			try {
				...
				if (!options.id && !options.ids) {
					console.error(
						chalk.red('Error: Either --id or --ids must be provided')
					);
					process.exit(1);
				}
				...
				if (options.id && options.ids) {
					console.error(
						chalk.red('Error: Cannot specify both --id and --ids')
					);
					process.exit(1);
				}
				...
				console.log(chalk.blue('🧪 AI-Powered Test Generation'));
				console.log(chalk.gray(`Tasks file: ${tasksPath}`));
				console.log(chalk.gray(`Output directory: ${options.outputDir}`));
				console.log(chalk.gray(`Processing ${taskIds.length} task(s): ${taskIds.join(', ')}`));

				// Single task processing
				if (taskIds.length === 1) {
					const taskId = taskIds[0];

					// Show task preview and statistics
					const tasksData = readJSON(tasksPath);
					const tasks = Array.isArray(tasksData) ? tasksData : tasksData.tasks;
					const taskData = findTaskById(tasks, taskId);
					...
					const stats = getTestGenerationStats(taskData.task);
					...
					console.log(`   Has test strategy: ${stats.hasTestStrategy ? 'Yes' : 'No'}`);
					console.log(`   Dependencies: ${stats.dependencyCount}`);
					console.log(`   Subtasks: ${stats.subtaskCount}\n`);

					// Generate test
					const result = await generateTestForTask(taskId, {
						tasksPath,
						outputDir: options.outputDir,
					...
					if (result.success) {
						...
						console.log(chalk.gray(`   Generated: ${result.filename}`));
						console.log(chalk.gray(`   Location: ${result.outputPath}`));
						console.log(chalk.gray(`   Lines: ${result.linesGenerated}`));
					} else {
					...
					const batchResult = await generateTestsForTasks(taskIds, {
						tasksPath,
						outputDir: options.outputDir,
						filePrefix: options.filePrefix,
						research: options.research,
					...
					console.log(`   Total tasks: ${batchResult.total}`);
					...
					if (batchResult.failed > 0 && !options.continueOnError) {
						process.exit(1);
					}
				}

			} catch (error) {
				console.error(chalk.red(`Error in generate-test command: ${error.message}`));
				if (getDebugFlag()) {
