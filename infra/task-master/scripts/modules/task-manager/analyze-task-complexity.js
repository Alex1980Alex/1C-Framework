 ...
 * @param {Object} [contextOptions] - Context options for AI operations
 */
async function analyzeTaskComplexity(options, context = {}, contextOptions = {}) {
	...
	try {
		...
		try {
			...
			const enhancedPrompt = await enhancePromptWithContext(prompt, contextOptions || {}, {
				taskContext: tasksData.tasks.length > 0 ? `Analyzing ${tasksData.tasks.length} tasks for complexity` : null,
