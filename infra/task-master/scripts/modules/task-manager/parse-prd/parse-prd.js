...
} from './enhanced-parse-prd.js';
import { shouldEnableInFlightExpansion } from './in-flight-expansion-pipeline.js';
...
/**
 * Main entry point - decides between enhanced/standard and streaming/non-streaming
 */
async function parsePRD(prdPath, tasksPath, numTasks, options = {}) {
	// Check if enhanced parsing is requested
	const isEnhancedParsing = options.enhanced ||
		options.adaptiveCount ||
		options.expandTasks ||
		options.preserveDetail ||
		shouldEnableInFlightExpansion(options, null); // null analysis will be handled in enhanced parsing

	// Route to enhanced parsing if requested
