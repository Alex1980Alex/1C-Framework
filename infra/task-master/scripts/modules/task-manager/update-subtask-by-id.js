...
async function updateSubtaskById(
	...
	try {
		...
		try {
			...
			// Enhance userPrompt with context if contextOptions are provided
			const enhancedPrompt = await enhancePromptWithContext(userPrompt, contextOptions || {}, {
				taskContext: parentTask ? `Parent task: ${parentTask.title} (ID: ${parentTask.id})` : null,
				subtaskContext: subtaskToUpdate ? `Current subtask: ${subtaskToUpdate.title} (ID: ${subtaskToUpdate.id})` : null,
