...
/**
 * Test script for generate-test functionality - FIXED version
 */
...
// Access the master tag tasks
const tasks = tasksData.master?.tasks || [];

console.log(chalk.yellow(`Found ${tasks.length} tasks in master tag`));
...
if (task24) {
    ...
    const testContent = generateTestContent(task24);
    ...
    console.log(chalk.yellow('Available task IDs:'), tasks.map(t => t.id).slice(0, 10).join(', '), '...');
}

function generateTestContent(task) {
    return `/**
 * Generated Test File for Task #${task.id}
 * ${task.title}
 * 
...
describe('Task #${task.id}: ${task.title}', () => {
    let tempDir;
    let mockConfig;
