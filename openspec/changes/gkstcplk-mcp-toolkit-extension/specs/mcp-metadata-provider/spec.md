...
### Requirement: get_access_rights — права доступа

Инструмент `get_access_rights` MUST возвращать матрицу прав для объекта.

- Параметр: `metadata_object` (формат `Справочник.ИмяСправочника`)
- Возвращает: список прав, роли и их настройки

#### Scenario: права на справочник

- **WHEN** `metadata_object = "Справочник.Контрагенты"`
- **THEN** возвращается матрица `{role: {read, write, delete, ...}}` для всех ролей с правами
