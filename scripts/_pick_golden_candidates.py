...
flt = models.Filter(
    must=[models.FieldCondition(key="module_path", match=models.MatchText(text="CommonModules"))]
)
all_pts: list = []
offset = None
while True:
    pts, offset = c.scroll(
        "bsl_code_v4_late", limit=256, with_payload=True, scroll_filter=flt, offset=offset
    )
    all_pts.extend(
        [
            p
            for p in pts
            if (p.payload or {}).get("symbol_type") in ("Function", "Procedure")
            and (p.payload or {}).get("content")
        ]
    )
...
def pick(pool, n):
    ...
    for p, sz in pool:
        ...
        if len(nm) < 12:
            continue
        if "_part" in nm or nm.startswith("Тест"):
            continue
        if mp in seen:
            continue
        seen.add(mp)
...
Path("data/_golden_candidates.json").write_text(
