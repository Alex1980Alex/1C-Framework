...
_SKIP = re.compile(
    r"встреча|телемост|запись|обсужден|диалог|^\d{4}-\d\d-\d\d|"
    r"^analysis|^esti|^информаци|^вариант|copy$|^[a-z0-9_]+$",
    re.I,
)
...
def build() -> list[dict]:
    ...
    if CANDIDATES.exists():
        curated = [
            r
            for r in json.loads(CANDIDATES.read_text(encoding="utf-8"))
        ...
        rows.append(
            {
                ...
                "split": split_of(text),
                "regex_is_1c": rx,
                "regex_conf": cl.get("confidence", 0.0),
                "review": is_1c != rx,
            }
        )
...
def merge_into_gt(cands: list[dict]) -> int:
    ...
    lines = (
        ["["]
        + [
            "  " + json.dumps(r, ensure_ascii=False) + ("," if i < len(gt) - 1 else "")
...
def main(argv=None) -> int:
    ...
    print(
        f"review (regex != метка): {len(rev)}  FN={sum(1 for r in rev if r['is_1c'])} "
