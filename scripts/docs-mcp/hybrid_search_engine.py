...
try:
    ...
    log_stderr(
        "Внимание: watchdog не установлен. Автоиндексация недоступна. Установите: pip install watchdog"
    )
...
class HybridSearchEngine:
        ...
        if not self._embedding_model_loaded:
            ...
            if EMBEDDINGS_AVAILABLE:
                try:
                    ...
                    self.logger.info(
                        "Модель эмбеддингов загружена: paraphrase-multilingual-MiniLM-L12-v2"
                    )
                except Exception as e:
    ...
    def index_document(
        self, file_path: str, doc_type: str = "markdown", force: bool = False
    ) -> bool:
    ...
    def delete_by_source(self, source_path: str) -> dict[str, int]:
        ...
        try:
            ...
            log_stderr(
                f"[INFO] Найдено {len(docs_to_delete)} документов для удаления из источника: {source_path}"
            )
            ...
            log_stderr(
                f"[OK] Удалено из индекса: {stats['documents']} документов (FTS синхронизирован), {stats['embeddings']} эмбеддингов"
            )
            return stats
    ...
    def search(
        self, query: str, limit: int = 10, search_type: str = "hybrid"
    ) -> list[SearchResult]:
        """
        ...
        try:
            ...
            for row in cursor.fetchall():
                try:
                    ...
                    self.logger.error(
                        f"Ошибка обработки эмбеддинга {row[0] if row else 'unknown'}: {e}"
                    )
                    continue
            ...
            for row in cursor.fetchall():
                try:
                    ...
                    self.logger.error(
                        f"Ошибка обработки документа {row[0] if row else 'unknown'}: {e}"
                    )
                    continue
    ...
    def _hybrid_search(self, query: str, limit: int) -> list[SearchResult]:
        ...
        try:
            ...
            semantic_results = (
                self._semantic_search(query, limit * 2) if self.embedding_model else []
            )
            ...
            if len(filtered_results) < len(final_results):
                self.logger.debug(
                    f"Отфильтровано {len(final_results) - len(filtered_results)} результатов с низкой релевантностью"
                )
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        return {
            ...
            and (self._embedding_model is not None or not self._embedding_model_loaded),
    ...
    def index_bsl_file_data(
        self, file_path: str, chunk_mode: str = "smart"
        ...
        try:
            ...
            if chunk_mode == "procedures" or chunk_mode == "smart":
                ...
                if chunk_mode == "smart":
                    ...
                    chunks = [
                        c for c in self._parse_bsl_content(content, file_path) if c.get("is_export")
                    ]
                else:
                ...
                for chunk in chunks:
                    ...
                    proc_title = (
                        f"{metadata['object_type']}.{metadata['object_name']}.{chunk['name']}"
                    )
                    if chunk["is_export"]:
                        proc_title += " (Экспорт)"
    ...
    def _analyze_bsl_content(self, content: str) -> dict[str, Any]:
        ...
        analysis["procedures_count"] = len(
            re.findall(r"\b(?:Процедура|Procedure)\s+\w+", content, re.IGNORECASE)
        )
        analysis["functions_count"] = len(
            re.findall(r"\b(?:Функция|Function)\s+\w+", content, re.IGNORECASE)
        )
        ...
        try:
            ...
            if chunk_mode == "full":
                ...
                for chunk in chunks:
                    ...
                    if chunk_analysis["directives"]:
                        proc_summary += (
                            f"**Директивы:** {', '.join(chunk_analysis['directives'])}\n"
                        )
                ...
                for chunk in export_chunks:
                    ...
                    if chunk_analysis["directives"]:
                        api_desc += (
                            f"**Контекст выполнения:** {', '.join(chunk_analysis['directives'])}\n"
                        )
                    if chunk_analysis["dependencies"]:
                        api_desc += (
                            f"**Зависимости:** {', '.join(chunk_analysis['dependencies'][:5])}\n"
                        )
    ...
    def index_javascript_file(
        self, file_path: str, chunk_mode: str = "smart", force: bool = False
    ) -> int:
        """
        ...
        try:
            ...
            log_stderr(
                f"[OK] {doc_type.upper()}: {path.name} → {indexed_count} док. (режим: {chunk_mode})"
            )
            return indexed_count
    ...
    def _build_js_summary(self, content: str, path: Path, analysis: dict[str, Any]) -> str:
        ...
        summary_parts.append(
            f"**Сложность:** {analysis['complexity_level']} ({analysis['complexity']})\n"
        )
    ...
    def index_bsl_project(self, project_path: str, chunk_mode: str = "smart") -> dict[str, int]:
        ...
        log_stderr(
            f"[OK] BSL проект проиндексирован: {stats['files']} файлов, {stats['documents']} документов"
        )
        return stats
    ...
    def index_xml_project(self, project_path: str, xml_types: list[str] = None) -> dict[str, int]:
        ...
        log_stderr(
            f"[OK] XML проект проиндексирован: {stats['files']} файлов, {stats['documents']} документов"
        )
    ...
    def index_directory_incremental(
        self, docs_path: str, doc_type: str = "markdown"
    ) -> dict[str, int]:
        """
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT path, content_hash FROM documents WHERE path LIKE ?", (f"{docs_path}%",)
        )
        ...
        self.logger.info(
            f"✅ Инкрементальная индексация завершена: "
...
class DocsFileWatcher:
    ...
    def __init__(
        self, search_engine: HybridSearchEngine, docs_path: str, debounce_seconds: float = 1.0
    ):
        """
            ...
            for path, event_type in events_to_process:
                try:
                    if event_type == "deleted":
                        ...
                        self.logger.info(
                            f"📝 Файл {'создан' if event_type == 'created' else 'изменён'}: {Path(path).name}"
                        )
    ...
    def start(self):
        ...
        self.observer.schedule(DocsEventHandler(self), str(self.docs_path), recursive=True)
...
def main():
    ...
    # Тестовые запросы
    test_queries = ["Task Master", "BSL анализ качества", "MCP сервер", "интеграция Claude"]
