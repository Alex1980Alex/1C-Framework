...
def _id_to_key(client: QdrantClient, collection: str, ids: list[str]) -> dict[str, tuple[str, str]]:
...
def eval_arm(
    ...
    golden_path: Path,
    k: int = 10,
    source_collection: str = "bsl_code_v4",
) -> dict[str, Any]:
    ...
    for item in golden:
        ...
        retrieved_keys = [
            ((p.payload or {}).get("name", ""), (p.payload or {}).get("module_path", ""))
            for p in points
        ]
...
def main() -> None:
    ...
    for r in results:
        print(
            f"{r['arm']:<14}{r['collection']:<22}{r['n_queries']:<5}"
