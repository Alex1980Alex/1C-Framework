...
if not "%USE_TEMPLATE%"=="" (
    if "%USE_TEMPLATE%"=="reindex" (
        ...
        set "PROMPT=ЗАДАЧА: Автономное изучение конфигурации 1С через AutoResearch-цикл. КОНТЕКСТ: Конфигурация УправлениеТранспортомНаПЛК v2026.1.1.0. Тестовая база: testdb1c_research. MCP: 1c-mcp-crud (get_metadata, execute_query, execute_code, find_references, get_access_rights, get_event_log). MCP: EDT-MCP (list_modules, get_module_structure, get_symbol_info, validate_query, get_problems). Кэш: .claude/skills/1c-config-knowledge/cache/. Дорожная карта: docs/roadmap/ROADMAP_AUTORESEARCH_1C_STUDY.md. АЛГОРИТМ: 1. Прочитай cache/_index.json (если нет — создай). 2. Получи список объектов через get_metadata. 3. Найди первый НЕизученный (приоритет: документы, регистры, справочники). 4. Если всё изучено — RALPH_DONE. 6 ФАЗ: Ф1 МЕТАДАННЫЕ (get_metadata, find_references, execute_query первые 3). Ф2 ИСХОДНЫЙ КОД (list_modules, get_symbol_info ОбработкаПроведения/ПередЗаписью). Ф3 ГИПОТЕЗА (обязательные реквизиты, движения, зависимости, бизнес-смысл). Ф4 ПРОВЕРКА НА БАЗЕ (создать ТЕСТ_AR_*, провести, проверить движения). Ф5 ГРАНИЧНЫЕ СЛУЧАИ (без обязательного поля, отмена проведения). Ф6 CLEANUP + КЭШ (удалить ТЕСТ_AR_*, записать cache/documents/). БЕЗОПАСНОСТЬ: префикс ТЕСТ_AR_, cleanup, только testdb1c_research. КРИТЕРИИ ИТЕРАЦИИ: файл cache создан, _index.json обновлён, тестовые данные удалены. КРИТЕРИИ ЦИКЛА: все 27 документов изучены, document_register_map.md полная карта, business_processes.md цепочки. Когда ВСЕ критерии цикла выполнены: RALPH_DONE"
    ) else if "%USE_TEMPLATE%"=="1c-analysis" (
        echo [TEMPLATE] 1c-analysis
        set "PROMPT=ЗАДАЧА: Итеративный анализ задачи 1С через Analyze-1C-Research (3 агента). КОНТЕКСТ: Прочитай файл %TASK_FILE%. Scorer: python scripts/score-analysis-report.py. Templates: .claude/skills/autoresearch/templates/1c-analysis-*.md. Session: data/analyze-1c-research/{task-id}/. ПРОТОКОЛ: 1. EXECUTOR: фазы 1-4 анализа (требования, объекты, алгоритм, план). Для каждого поля SQL — get_metadata. Для каждой точки — bsl_search на паттерн. Маркеры: ✓ get_metadata, ✓ pattern, ✓ execute_query, [REQ-N]. Коммит [AR-N]. 2. REVIEWER: scorer, verify через MCP (до 3 полей, до 2 SQL), verdict KEEP/IMPROVE/REVERT, reviewer_feedback.json. 3. EXECUTOR: fix ONE gap из feedback. 4. Repeat. СТОП: score >= 85 ИЛИ 3 итерации без улучшения ИЛИ 0 gaps. MCP: bsl-semantic-search, 1c-mcp-crud, bsl-platform-context. Когда ВСЕ критерии выполнены: RALPH_DONE"
    ) else if "%USE_TEMPLATE%"=="autoresearch" (
        echo [TEMPLATE] autoresearch
        set "PROMPT=ЗАДАЧА: Запусти AutoResearch v2 цикл. КОНТЕКСТ: Прочитай data/autoresearch/autoresearch.md для контекста. Если файла нет — спроси пользователя: какую метрику улучшить, scope файлов, verify команду. ПРОТОКОЛ: 1. Если нет autoresearch.md — создай его через /autoresearch:plan. 2. Phase 0: ANALYZE — определи инструменты, метрику, подход. 3. Iterations: Executor (одно изменение, коммит [AR-N]) → Reviewer (verify, verdict) → каждые 5: Comparator (A/B). 4. KEEP если метрика улучшилась, REVERT если ухудшилась. 5. Обновляй autoresearch.md после каждой итерации. СТОП-УСЛОВИЯ: target достигнут, 5 итераций плато, все идеи исчерпаны. Когда ВСЕ критерии выполнены: RALPH_DONE"
    ) else (
        echo [ERROR] Unknown template: %USE_TEMPLATE%
        echo Available: reindex, test-coverage, evaluation, documentation, lint, quality, skill-health, 1c-study, 1c-analysis, autoresearch
        exit /b 1
    )
)
