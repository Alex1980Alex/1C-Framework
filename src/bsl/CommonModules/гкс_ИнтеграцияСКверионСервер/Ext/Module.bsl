...
Процедура ВыгрузитьСообщениеВКверион(ПараметрыВыгрузки, Отказ) Экспорт
	...
	ДанныеСинхронизацииRMQ 		= ДанныеСинхронизацииRMQ(ДокументРегистрацииОбъект);
	Для Каждого ОбъектXDTO Из ДанныеXDTO Цикл
		...
		ДопСвойстваРегистрации.Вставить("ДанныеСинхронизацииRMQ", ДанныеСинхронизацииRMQ);
...
Функция ДанныеСинхронизацииRMQ(Источник)

	УстановитьПривилегированныйРежим(Истина);
	гкс_ОчередьСообщенийRMQ.ИнициализироватьДополнительныеСвойстваОбъекта(Источник);
	ДанныеСинхронизацииRMQ = ОбщегоНазначения.СкопироватьРекурсивно(
		Источник.ДополнительныеСвойства.ДанныеСинхронизацииRMQ);
	ДанныеСинхронизацииRMQ.Вставить(
		"Получатели", ПолучателиСообщенияКверион(ДанныеСинхронизацииRMQ.ТипМетаданных));

	Возврат ДанныеСинхронизацииRMQ;

КонецФункции

Функция ПолучателиСообщенияКверион(ТипМетаданных)

	ДопПараметры 			= Новый Структура;
	ИмяПредопределенного	= "ПолучательДанныхУправлениеДвором";
	ИскомыйПолучатель 		= гкс_ОбщегоНазначенияВызовСервера.ПроизвольныйПредопределенныйОбъект(
		ИмяПредопределенного);
	НовыеПолучатели		= Новый Массив;
	ВсеПолучатели 		= гкс_ОчередьСообщенийRMQВызовСервера.ПолучателиRabbitMQПоТипуМетаданных(
		ТипМетаданных, ДопПараметры);
	Для Каждого Получатель Из ВсеПолучатели Цикл

		Если Получатель.СсылкаНаПолучателя = ИскомыйПолучатель Тогда
			НовыеПолучатели.Добавить(Получатель);
		КонецЕсли;

	КонецЦикла;

	Возврат НовыеПолучатели;
