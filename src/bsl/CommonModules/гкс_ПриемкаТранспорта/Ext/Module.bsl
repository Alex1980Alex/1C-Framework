...
// Определить служебную номенклатуру при пропуске входного контроля качества.
// Функция выполняет поиск служебной номенклатуры в регистре настроек назначения разгрузки
// для случаев, когда лабораторный анализ не проводится и качество определяется по настройкам.
//
// Параметры:
...
//
// Возвращаемое значение:
//  СправочникСсылка.Номенклатура - служебная номенклатура (сегмент качества) или пустая ссылка,
//                                   если настройки не найдены
//
// Используется при пропуске входного контроля качества для автоматического
//                определения силоса без проведения лабораторного анализа
//
Функция ОпределитьСлужебнуюНоменклатуруПриПропуске(ТочкаМаршрута, ВидПеревозки, Номенклатура) Экспорт
...
Процедура СоздатьНаправлениеНаРазгрузкуПриПропускеКонтроля(ДокументРегистрации) Экспорт
	...
	//  ШАГ 2 - Определяем служебную номенклатуру.
	// При пропуске контроля качества служебная номенклатура определяется
	// из регистра настроек (без лабораторного анализа).
	СлужебнаяНоменклатура = ОпределитьСлужебнуюНоменклатуруПриПропуске(
		ДанныеРегистрации.ТочкаМаршрута,
		ДанныеРегистрации.ВидПеревозки,
	...
	Если Не ЗначениеЗаполнено(СлужебнаяНоменклатура) Тогда
		ТекстСообщения = СтрШаблон(
			НСтр("ru = 'Не найдена служебная номенклатура для автоназначения силоса.
			           |Регистрация: %1, Номенклатура: %2'"),
			ДокументРегистрации, ДанныеРегистрации.Номенклатура);
		МетаданныеРегистрации = ДокументРегистрации.Метаданные();
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'ПропускВходногоКонтроля'"),
			УровеньЖурналаРегистрации.Предупреждение,
			МетаданныеРегистрации,
			ДокументРегистрации,
			ТекстСообщения);
		Возврат;
	КонецЕсли;

	// ШАГ 3 - Получаем силос и яму разгрузки из регистра настроек.
	Если ПропуститьИнициализациюМестаРазгрузки Тогда
		ДанныеРазгрузки = Новый Структура("Силос,ЯмаРазгрузки", Неопределено, Неопределено);
	Иначе
		...
		Если ДанныеРазгрузки = Неопределено Тогда
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'GKSTCPLK-1656: Не найдены настройки назначения разгрузки (силос/яма).
...
Функция ПоказателиКачестваЛабАнализа(ЛабораторныйАнализ, ДокументРегистрации, НазначениеИспользованияКачества)
	...
	Запрос.Текст =
		...
		|	И ФактическиеПоказателиКачества.НазначениеИспользованияКачества = &НазначениеИспользованияКачества";

	Результат = Запрос.Выполнить();
	Если Не Результат.Пустой() Тогда
		ТаблицаПоказателей = Результат.Выгрузить();
	Иначе
		ТаблицаПоказателей = ЛабораторныйАнализ.КачественныеПоказатели.Выгрузить();
	КонецЕсли;
...
// Текст запроса лаб анализ для промежуточного композита.
// Используется для поиска существующего ЛабораторногоАнализа при формировании единичной пробы
// с НазначениеИспользованияКачества = ПромежуточныйКомпозит.
// Особенности запроса:
// - Фильтр по гкс_ДокументРегистрации = &ДокументРегистрации
// - Фильтр по гкс_НазначениеИспользованияКачества = ПромежуточныйКомпозит
// - Исключены помеченные на удаление записи
//
// Возвращаемое значение:
//  Строка - Текст запроса для поиска ЛабораторногоАнализа промежуточного композита
Функция ТекстЗапросаЛабАнализДляПромежуточногоКомпозита()
