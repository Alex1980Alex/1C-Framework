...
Процедура ОбновитьПредпросмотр()
	...
	Если ТекущиеДанные <> Неопределено И НРег(ТекущиеДанные.Расширение) = "pdf" Тогда
		...
		Попытка
			ДанныеФайла = РаботаСФайламиСлужебныйВызовСервера.ДанныеФайлаДляОткрытия(
				ТекущиеДанные.Ссылка, Неопределено, УникальныйИдентификатор,, АдресДанныхФайла);
			ЗагрузитьPDFПредпросмотрНаСервере(ДанныеФайла.СсылкаНаДвоичныеДанныеФайла);
		Исключение
		КонецПопытки;
	...
	// Не PDF — штатная логика (картинки)
