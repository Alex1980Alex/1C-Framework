...
Процедура УстановитьВидимостьДоступностьПропуститьКонтрольКачества()
   ...
   Элементы.СписокРегистрацииПропуститьКонтрольКачества.Видимость = 
		ЭтоПриемкаТранспорта 
			И ЭтоЖДПеревозка 
			И Не ПолучитьФункциональнуюОпцию("гкс_ИспользоватьОтключениеВходногоКонтроляКачества");
...
Процедура УстановитьВидимостьЭлементовСпискаРегистрации()
	...
	Если ЗначениеЗаполнено(ОтборТочкаМаршрута) И ЗначениеЗаполнено(ОтборВидПеревозки) Тогда
		...
		// GKSTCPLK-2407: итоговая видимость колонок = (тип перевозки разрешает) И (пользователь не скрыл колонку).
		// Без этого "И" жёсткое присваивание ниже затирает пользовательскую настройку
		// "Видимость = Ложь" из диалога "Настройка элементов формы" (хранится в РС.гкс_ХранилищеНастроек).
		ПользВидимость = ПрочитатьПользВидимостьКолонокСпискаРегистрации();

		Элементы.СписокРегистрацииФумигационный.Видимость 	= ЭтоЖДПеревозка  И ПользВидимостьКолонки(ПользВидимость, "СписокРегистрацииФумигационный");
		Элементы.СписокРегистрацииФитосанитарный.Видимость 	= ЭтоЖДПеревозка  И ПользВидимостьКолонки(ПользВидимость, "СписокРегистрацииФитосанитарный");
		Элементы.СписокРегистрацииПретензионный.Видимость 	= ЭтоЖДПеревозка  И ПользВидимостьКолонки(ПользВидимость, "СписокРегистрацииПретензионный");

		Элементы.СписокРегистрацииВремяЗаписи.Видимость 	= ЭтоАвтоПеревозка И ПользВидимостьКолонки(ПользВидимость, "СписокРегистрацииВремяЗаписи");
		Элементы.СписокРегистрацииВодитель.Видимость 		= ЭтоАвтоПеревозка И ПользВидимостьКолонки(ПользВидимость, "СписокРегистрацииВодитель");

		Элементы.СписокРегистрацииПунктПогрузки.Видимость 	= ЭтоАвтоПеревозка И ПользВидимостьКолонки(ПользВидимость, "СписокРегистрацииПунктПогрузки");
		Элементы.СписокРегистрацииСтанцияЖДНаименованиеПолное.Видимость = ЭтоЖДПеревозка И ПользВидимостьКолонки(ПользВидимость, "СписокРегистрацииСтанцияЖДНаименованиеПолное");
...
// GKSTCPLK-2407: читает пользовательские настройки видимости колонок СписокРегистрации
// из того же хранилища, что использует диалог "Настройка элементов формы"
// (РС.гкс_ХранилищеНастроек, ключ ИмяФормы + логин пользователя + UUID ИБ).
// Возвращает Соответствие (Имя элемента → Булево Видимость). Если пользователь ничего
// не настраивал — возвращает пустое соответствие (см. ПользВидимостьКолонки — дефолт Истина).
&НаСервере
Функция ПрочитатьПользВидимостьКолонокСпискаРегистрации()
	...
	ЗаполнитьКэшПользВидимостиКолонок(ДанныеЭлементов, Результат);

	Возврат Результат;

КонецФункции

// GKSTCPLK-2407: рекурсивно обходит структуру сохранённых настроек элементов формы
// и заполняет плоское Соответствие (Имя элемента → Видимость). Работает с тем же
// форматом данных, что ПрочитатьНастройкиФормы / ЗаполнитьНастройкиФормы выше.
&НаСервере
Процедура ЗаполнитьКэшПользВидимостиКолонок(Узел, Результат)

	Если ТипЗнч(Узел) <> Тип("Соответствие") Тогда
		Возврат;
	КонецЕсли;

	Для Каждого Строка Из Узел Цикл

		Значение = Строка.Значение;

		Если ТипЗнч(Значение) = Тип("Соответствие") Тогда
			// Корневой узел таблицы формы (например, "СписокРегистрации") — спускаемся внутрь.
			ЗаполнитьКэшПользВидимостиКолонок(Значение, Результат);
			Продолжить;
		КонецЕсли;

		Если ТипЗнч(Значение) = Тип("Структура") И Значение.Свойство("Видимость") Тогда
			Результат.Вставить(Строка.Ключ, Значение.Видимость);

			// Группа колонок — внутри может быть вложенное Соответствие "ЭлементыГруппы".
			Если Значение.Свойство("ЭтоГруппаКолонок") И Значение.ЭтоГруппаКолонок
				И Значение.Свойство("ЭлементыГруппы") Тогда
				ЗаполнитьКэшПользВидимостиКолонок(Значение.ЭлементыГруппы, Результат);
			КонецЕсли;
		КонецЕсли;
...
// GKSTCPLK-2407: возвращает пользовательскую настройку видимости колонки.
// Если пользователь ничего не настраивал (ключ отсутствует) — видимость разрешена (Истина),
// чтобы дефолтное поведение совпадало с прежним (до правки).
&НаСервере
Функция ПользВидимостьКолонки(ПользВидимость, ИмяКолонки)

	Значение = ПользВидимость.Получить(ИмяКолонки);
...
//
&НаКлиенте
Процедура Подключаемый_ЛАБлокировкиНажатие(Элемент, СтандартнаяОбработка)
