...
Функция ДокументыРегистрацииФормированияНомераПроб(ФормированиеНомераПробы) Экспорт
	...
	Запрос.Текст =
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ФНП_Регистрации.ДокументРегистрации КАК ДокументРегистрации
		|ИЗ
		|	Документ.гкс_ФормированиеНомераПробы.СписокРегистраций КАК ФНП_Регистрации
		|ГДЕ
		|	ФНП_Регистрации.Ссылка = &Ссылка
		|	И ФНП_Регистрации.ДокументРегистрации <> ЗНАЧЕНИЕ(Документ.гкс_РегистрацияНаПЛК.ПустаяСсылка)";
	...
	Возврат
		Результат.ВыгрузитьКолонку("ДокументРегистрации");
...
Процедура СформироватьЛабораторныйАнализПоНеобходимости(ФормированиеНомераПробы, ТипПробы = Неопределено,
	...
	Если ДополнительныеДанныеЗаполнения <> Неопределено
		И ДополнительныеДанныеЗаполнения.Свойство("ПоказателиДляЗаполнения") Тогда
		ДополнительныеДанныеЗаполнения.Свойство("ПоказателиДляЗаполнения",
			ПоказателиДляЗаполнения);
	КонецЕсли;
	...
	Если ДополнительныеДанныеЗаполнения <> Неопределено
		...
		ДанныеЗаполнения.Вставить("МассивПоказателей",
			ПоказателиДляЗаполнения);
	Иначе
		ДанныеЗаполнения.Вставить("гкс_НазначениеИспользованияКачества",
...
Функция ДанныеДляФормированияЛабораторногоАнализа(ФормированиеНомераПробы, РегистрацияТранспорта)
	...
	Если ПолучитьФункциональнуюОпцию("гкс_ИспользоватьИнтеграциюСWarehouse") Тогда
		...
		РезультатПоиска = Документы.гкс_ЛабораторныйАнализ.
			ПоискДокументаВнешнейПробы(РегистрацияТранспорта, НазначениеИспользованияКачества);

		Если ЗначениеЗаполнено(РезультатПоиска) И ЗначениеЗаполнено(РезультатПоиска.ВнешняяПроба) Тогда

			ДанныеЗаполнения.Вставить("ВнешняяПроба", РезультатПоиска.ВнешняяПроба);
			ДанныеЗаполнения.Вставить("ПоказателиВнешнихПроб", РезультатПоиска.КачественныеПоказатели);

			ДанныеЗаполнения.Вставить("КонтрольнаяСуммаВнешнейПробы",
				ОбщегоНазначения.ЗначениеРеквизитаОбъекта(РезультатПоиска.ВнешняяПроба, "КонтрольнаяСумма"));
		КонецЕсли;
...
Функция ПроверитьФормированияЛабораторногоАнализа(ДокументОснование, ТипПробы, НазначениеИспользованияКачества)
	Возврат Не ЗначениеЗаполнено(ДокументОснование)
		Или ЛабораторныйАнализПоФормированиюПробыУжеСуществует(ДокументОснование, ТипПробы,
			НазначениеИспользованияКачества)
