...
Процедура СформироватьНомерПробыИЗакрытьДокумент(Команда)
	...
	Если ПромежуточныйКомпозитПредыдущейГруппыРегистрацииНеЗавершен() Тогда
		Оповещение = Новый ОписаниеОповещения("ЗакрытьДокументЗавершение", ЭтотОбъект);
		ТекстСообщения = НСтр("ru='Нет завершенного промежуточного композита для предыдущей группы'");
...
&НаКлиенте
Процедура ЗакрытьДокументЗавершение(Результат) Экспорт
...
&НаСервере
Функция ПромежуточныйКомпозитПредыдущейГруппыРегистрацииНеЗавершен()

	Возврат Справочники.гкс_ГруппыТС.ПромежуточныйКомпозитПредыдущейГруппыРегистрацииНеЗавершен(Регистрация);
