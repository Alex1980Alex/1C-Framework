...
def extract_functions(content: str) -> Generator[dict, None, None]:
    ...
    for match in re.finditer(pattern, content, re.VERBOSE | re.IGNORECASE):
        ...
        if export:
            full_code += (
                " Экспорт" if "Функция" in func_type or "Процедура" in func_type else " Export"
            )
        full_code += f"\n{body}\n{end_keyword}"
...
def create_training_example(func: dict, file_path: str = "") -> dict:
    ...
    func_type = (
        "функцию" if "Функция" in func["type"] or "Function" in func["type"] else "процедуру"
    )
...
def process_folder(folder_path: str, output_prefix: str, max_examples: int = None):
    ...
    print(
        f"  С комментариями: {stats['with_comments']} ({100 * stats['with_comments'] / max(1, stats['functions_found']):.1f}%)"
    )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        print(
            "Использование: python extract_dataset.py <папка_с_bsl> <output_prefix> [max_examples]"
        )
        print(
            "Пример: python extract_dataset.py D:\\1C-Enterprise_Framework\\src D:\\1C-Enterprise_Framework\\data\\datasets\\bsl 10000"
        )
        sys.exit(1)
