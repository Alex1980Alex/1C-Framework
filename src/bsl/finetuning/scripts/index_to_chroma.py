...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        print(
            "Пример: python index_to_chroma.py D:\\1C-Enterprise_Framework\\data\\datasets\\bsl_rag.json D:\\1C-Enterprise_Framework\\data\\chroma_db"
        )
        sys.exit(1)

    rag_file = sys.argv[1]
    chroma_path = (
        sys.argv[2] if len(sys.argv) > 2 else "D:\\1C-Enterprise_Framework\\data\\chroma_db"
    )
    ...
    # Тестовый поиск
    test_queries = ["проверка ИНН", "запрос к справочнику", "обработка исключений"]
