...
class OAuth2Service:
    ...
    def generate_authorization_code(
        self, login: str, password: str, redirect_uri: str, code_challenge: str
    ) -> str:
    ...
    def exchange_code_for_tokens(
        ...
        if code_data.redirect_uri != redirect_uri:
            logger.warning(
                f"Несовпадение redirect_uri: ожидался {code_data.redirect_uri}, получен {redirect_uri}"
            )
            return None
        ...
        self.store.save_access_token(
            access_token,
            AccessTokenData(login=code_data.login, password=code_data.password, exp=access_exp),
        )
    ...
    def refresh_tokens(self, refresh_token: str) -> tuple[str, str, int, str] | None:
        ...
        self.store.save_access_token(
            new_access_token,
            AccessTokenData(
                login=refresh_data.login, password=refresh_data.password, exp=access_exp
            ),
        )
        ...
        logger.debug(
            f"Обновлены токены для пользователя {refresh_data.login} (rotation #{refresh_data.rotation_counter + 1})"
        )
