...
class Config(BaseSettings):
    ...
    auth_mode: Literal["none", "oauth2"] = Field(
        default="none", description="Режим авторизации: none или oauth2"
    )
    public_url: str | None = Field(
        default=None,
        description="Публичный URL прокси для OAuth2 (если не задан, формируется из запроса)",
    )
    ...
    oauth2_refresh_ttl: int = Field(
        default=1209600, description="TTL refresh token в секундах (14 дней)"
    )

    class Config:
        env_file = ".env"
        env_prefix = "MCP_"
        extra = "ignore"  # Игнорировать дополнительные MCP_* переменные окружения
