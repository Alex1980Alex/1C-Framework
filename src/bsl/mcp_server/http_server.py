...
class MCPHttpServer:
    ...
    def __init__(self, config: Config) -> None:
        ...
        self.oauth2_store: OAuth2Store | None = None
        self.oauth2_service: OAuth2Service | None = None
        ...
        self.app.add_middleware(
            OAuth2BearerMiddleware, oauth2_service=self.oauth2_service, auth_mode=config.auth_mode
        )
            ...
            try:
                ...
                async with sse_transport.connect_sse(
                    ...
                    await self.mcp_proxy.server.run(
                        streams[0], streams[1], self.mcp_proxy.get_initialization_options()
                    )
            except Exception as e:
    ...
    def _register_routes(self) -> None:
        ...
        async def info() -> Any:
            ...
            return {
                ...
                "transports": {
                    "sse": {"endpoint": "/sse", "messages": "/sse/messages/"},
    ...
    def _register_oauth2_routes(self) -> None:
        ...
        async def well_known_as_metadata(request: Request) -> Any:
            ...
            return {
                ...
                "token_endpoint_auth_methods_supported": [
                    "none"
                ],  # Публичный клиент, без client_secret
        ...
        async def authorize_post(
            ...
            try:
                async with httpx.AsyncClient(timeout=10.0) as client:
                    ...
                    response = await client.get(
                        health_url, auth=httpx.BasicAuth(username, password)
                    )
            ...
            logger.info(
                f"Authorization code выдан для пользователя {username}, redirect: {redirect_uri}"
            )
        ...
        async def token_endpoint(
            ...
            if grant_type == "password":
                ...
                try:
                    async with httpx.AsyncClient(timeout=10.0) as client:
                        health_url = (
                            f"{self.config.onec_url}/hs/{self.config.onec_service_root}/health"
                        )
                        response = await client.get(
                            health_url, auth=httpx.BasicAuth(username, password)
                        )
                    ...
                    return JSONResponse(
                        ...
                        content={
                            "error": "server_error",
                            "error_description": "Unable to validate credentials",
                        },
                    )
