...
Примеры использования:
  ...
  python -m src.bsl.mcp_server
  python -m src.bsl.mcp_server stdio
...
Переменные окружения:
  ...
  MCP_OAUTH2_REFRESH_TTL - TTL refresh token в секундах (по умолчанию: 1209600)
    ...
    parser.add_argument("--env-file", type=str, help="Путь к .env файлу с конфигурацией")
    parser.add_argument("--onec-url", type=str, help="URL базы 1С")
    parser.add_argument("--onec-username", type=str, help="Имя пользователя 1С")
    parser.add_argument("--onec-password", type=str, help="Пароль пользователя 1С")
    parser.add_argument("--onec-service-root", type=str, help="Корневой URL HTTP-сервиса в 1С")
    parser.add_argument(
        "--log-level",
        type=str,
    ...
    parser.add_argument("--host", type=str, help="Хост для HTTP-сервера (только для режима http)")
    parser.add_argument("--port", type=int, help="Порт для HTTP-сервера (только для режима http)")
    ...
    parser.add_argument("--public-url", type=str, help="Публичный URL прокси для OAuth2")
