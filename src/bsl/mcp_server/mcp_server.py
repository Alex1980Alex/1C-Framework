...
current_onec_credentials: contextvars.ContextVar[tuple[str, str] | None] = contextvars.ContextVar(
    "current_onec_credentials", default=None
)
...
class MCPProxy:
    ...
    def __init__(self, config: Config):
        ...
        """
        self.config = config
        self.onec_client: OneCClient | None = None
    ...
    async def _lifespan(self, server: Server) -> AsyncIterator[dict[str, Any]]:
        ...
        logger.debug(
            f"Инициализация MCP сервера '{self.config.server_name}' v{self.config.server_version}"
        )
        ...
        if self.config.auth_mode == "oauth2":
            ...
            if session_creds:
                ...
                logger.debug(
                    "Per-session креденшилы не найдены, использую дефолтные из конфигурации"
                )
        else:
    ...
    def _register_handlers(self):
        ...
        async def handle_call_tool(name: str, arguments: dict[str, Any]) -> list[types.TextContent]:
            ...
            try:
                ...
                return [
                    types.TextContent(type="text", text=f"Ошибка выполнения инструмента: {str(e)}")
                ]
        ...
        async def handle_get_prompt(
            ...
            try:
                ...
                return types.GetPromptResult(
                    description=f"Ошибка получения промпта: {str(e)}", messages=[]
                )
