...
class OneCClient:
    ...
    def __init__(self, base_url: str, username: str, password: str, service_root: str = "mcp"):
        ...
        self.client = httpx.AsyncClient(
            auth=self.auth, timeout=30.0, headers={"Content-Type": "application/json"}
        )
            ...
            try:
                ...
                if response_json.get("status") == "ok":
                    ...
                    raise httpx.HTTPStatusError(
                        f"1C service reported not healthy: {response_json}",
                ...
                raise httpx.HTTPStatusError(
                    f"Invalid JSON response from 1C health check: {e}",
                    request=response.request,
                    response=response,
                )
    ...
    async def call_rpc(self, method: str, params: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            rpc_request = {"jsonrpc": "2.0", "id": 1, "method": method, "params": params or {}}
    ...
    async def call_tool(self, name: str, arguments: dict[str, Any]) -> types.CallToolResult:
        ...
        """
        result = await self.call_rpc("tools/call", {"name": name, "arguments": arguments})
        ...
        if "content" in result:
            for item in result["content"]:
                ...
                if content_type == "text":
                    ...
                    logger.warning(
                        f"Неизвестный тип контента: {content_type}, обрабатываем как текст"
                    )
                    content.append(types.TextContent(type="text", text=str(item.get("text", item))))

        return types.CallToolResult(content=content, isError=result.get("isError", False))
    ...
    async def read_resource(self, uri: str) -> list[ReadResourceContents]:
        ...
        if "contents" in result:
                ...
                if content_type == "text":
                    contents.append(
                        ReadResourceContents(
                            content=item.get("text", ""), mime_type=mime_type or "text/plain"
                        )
                    )
            ...
            contents.append(
                ReadResourceContents(
                    content=json.dumps(result, ensure_ascii=False), mime_type="application/json"
                )
            )
    ...
    async def get_prompt(
        ...
        Returns:
                Результат промпта
        """
        result = await self.call_rpc("prompts/get", {"name": name, "arguments": arguments or {}})
        ...
        if "messages" in result:
            for msg_data in result["messages"]:
                content = types.TextContent(
                    type="text", text=msg_data.get("content", {}).get("text", "")
                )

                message = types.PromptMessage(role=msg_data.get("role", "user"), content=content)
