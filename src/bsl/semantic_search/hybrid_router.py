...
def classify_query(query: str) -> QueryType:
    ...
    topology_kw = [
        ...
        "топ функций",
        "узкие места",
        "больше всего вызовов",
        "больше всего исходящих",
        "ранжируй по",
        "топ-",
        "статистика графа",
    ]
    ...
    if any(k in q for k in topology_kw):
        return "topology"
    if any(k in q for k in callers_kw):
        return "multi_hop_callers"
    if any(k in q for k in impact_kw):
        return "impact_analysis"
    if any(k in q for k in arch_kw):
        return "architectural"
    if any(k in q for k in dead_kw):
        return "dead_code"
    if any(k in q for k in cross_kw):
...
def hybrid_search(
    ...
    *,
    ...
    if st == "vector" and vector_fn:
        res.results = vector_fn(query, k=k)
        res.layers_consulted.append("Layer 1: bsl_code_v4_late")
    elif st == "graph" and graph_fn:
        res.results = graph_fn(query, k=k, query_type=qt)
        res.layers_consulted.append("Layer 3: Neo4j")
        if not res.results and vector_fn:
            res.results = vector_fn(query, k=k)
            res.layers_consulted.append("Layer 1 (graph empty fallback)")
    elif st == "community" and community_fn:
        res.results = community_fn(query, k=k)
        res.layers_consulted.append("Layer 4: communities")
    elif st == "hybrid":
        merged = []
        if vector_fn:
        ...
        if vector_fn:
            res.results = vector_fn(query, k=k)
            res.layers_consulted.append("Layer 1 (fallback)")
