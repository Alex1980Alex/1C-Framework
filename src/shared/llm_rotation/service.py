...
class LLMRotationService:
    ...
    def explain_route(self, model: str | None = None) -> dict[str, Any]:
        ...
        """
        # Та же нормализация, что в complete() (ревью 2026-07-26): иначе диагностика
        # отдавала бы requested_model="" там, где реальный вызов отдаёт None - два
        # представления одного состояния ровно в том выводе, по которому судят о маршруте.
        model = model or None
