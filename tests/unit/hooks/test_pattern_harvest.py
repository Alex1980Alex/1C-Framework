...
def test_confirmed_drafts_and_lessons_create_cubes(dirs: tuple[Path, Path], tmp_path: Path) -> None:
    ...
    _write_lifecycle(
        ...
        "2099-01-01_sess",
        lessons=[
            "Always run post-merge importlib sweep after -X theirs merges to catch regressions"
        ],
...
def test_pending_and_placeholder_drafts_skipped(dirs: tuple[Path, Path]) -> None:
    ...
    client = FakeClient()
    stats = ph.harvest(
        drafts_dir=drafts, lifecycle_dir=life, client=client, embed=_fake_embed, sources=("drafts",)
    )
...
def test_cap_fires_above_n(dirs: tuple[Path, Path]) -> None:
    ...
    for i in range(5):
        _write_draft(
            ...
            f"d{i}",
            "confirmed",
            f"distinct confirmed rule number {i} long enough to pass floor",
        )
    client = FakeClient()
    stats = ph.harvest(
        drafts_dir=drafts,
...
def test_failsoft_client_raises(dirs: tuple[Path, Path]) -> None:
    ...
    stats = ph.harvest(
        drafts_dir=drafts, lifecycle_dir=life, client=Boom(), embed=_fake_embed, sources=("drafts",)
    )
    assert stats["created"] == 0
    assert stats["errors"] == 1
...
def test_skill_learning_source_ingests_confirmed(tmp_path: Path) -> None:
        ...
        [
            {
                "pattern_id": "p1",
                "name": "use httpx",
                "content": "Always use httpx.AsyncClient for async HTTP calls in this framework",
            ...
            },
            {
                "pattern_id": "p2",
                "name": "archived one",
                "content": "this is archived and must be skipped entirely here",
                "archived": True,
            },
    ...
    client = FakeClient()
    stats = ph.harvest(
        skill_learning_file=jf, client=client, embed=_fake_embed, sources=("skill_learning",)
    )
        ...
        [
            {
                "pattern_id": "p1",
                "name": "x",
                "content": "a confirmed skill pattern long enough to pass the floor",
            }
        ],
    )
    client = FakeClient()
...
def test_ingest_items_on_created_callback() -> None:
    ...
    items = [
        ph.HarvestItem(
            content="a brand new consolidated semantic pattern here",
            name="n",
            description="d",
