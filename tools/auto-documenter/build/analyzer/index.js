...
import { analyzeBSLFile } from './bsl-integration.js';
// Re-export BSL integration types and functions
export { analyzeBSLFile, formatBSLAnalysisAsMarkdown, createBSLSummary, shouldDocumentBSLFile, extractBSLKeyInfo } from './bsl-integration.js';
export { BSLTreesitterAnalyzer, BSLElementType, getBSLAnalyzer } from './bsl-treesitter-analyzer.js';
export { Structure1CAnalyzer, structure1CAnalyzer, analyze1CStructure, get1CContextInfo, MetadataObjectType, ModuleType } from './structure-1c-analyzer.js';
/**
...
export class FileAnalyzer {
     ...
     */
    async analyzeFiles(directoryPath, filePaths) {
        const result = {
            analyzedFiles: [],
            excludedFiles: [],
            limited: false,
     ...
     */
    shouldDocument(directoryPath, filePaths, hasSubdirectories = false) {
     ...
     */
    testShouldDocumentEmptyDirWithSubdirs(hasSubdirectories) {
    ...
    async processFile(filePath, result) {
        try {
            ...
            // Analyze BSL files with tree-sitter for enhanced information
            const extension = path.extname(filePath).toLowerCase();
            let analyzedFile;
            if (extension === '.bsl') {
                analyzedFile = await analyzeBSLFile(filePath, content);
            }
            else {
                analyzedFile = {
                    path: filePath,
            ...
            result.analyzedFiles.push(analyzedFile);
            // Update total size
