    ...
    if (result.metrics) {
        for (const [key, value] of Object.entries(result.metrics)) {
            if (typeof value === 'number') {
                ...
                for (const [lang, count] of Object.entries(value)) {
 ...
 */
export function formatSuiteResults(suite) {
    const lines = [];
 ...
 */
export function generateBenchmarkReport(suites, metadata) {
    const lines = [];
    // Header
    lines.push('╔══════════════════════════════════════════════════════════════════════╗');
 ...
 */
export function generateMarkdownReport(suites, metadata) {
    const lines = [];
    lines.push('# Autodocument Benchmark Report');
    lines.push('');
    // Metadata
    ...
    for (const suite of suites) {
        ...
        if (suite.results && suite.results.length > 0) {
            ...
            for (const result of suite.results) {
                const status = result.success ? '✅' : '❌';
                lines.push(`| ${result.name} | ${formatDuration(result.duration)} | ${formatBytes(result.memoryUsed)} | ${result.opsPerSecond.toFixed(2)} | ${status} |`);
            }
            lines.push('');
        }
    }
 ...
 */
export function generateJsonReport(suites, metadata) {
    const report = {
        metadata: {
            ...metadata,
