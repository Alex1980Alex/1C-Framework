...
export class FormExtendedValidator extends FormValidator {
     ...
     */
    async validateFormExtended(formXmlPath, moduleBslPath) {
     ...
     */
    validateDataPaths(formStructure) {
        const issues = [];
     ...
     */
    validateHierarchy(formStructure) {
        const issues = [];
     ...
     */
    checkRequiredHandlers(formStructure) {
        const issues = [];
        // Check if form has OnCreateAtServer (highly recommended)
        const hasOnCreateAtServer = formStructure.formEvents.some(e => e.eventType === 'OnCreateAtServer');
        ...
        const fieldsWithStartChoice = formStructure.controls.filter(c => c.type === 'InputField' && c.events.some(e => e.eventType === 'StartChoice'));
     ...
     */
    async validateCommands(formStructure, moduleBslPath) {
        const totalCommands = formStructure.commands.length;
        let commandsWithHandlers = 0;
        const missingCommandHandlers = [];
        const orphanedCommandHandlers = [];
        if (totalCommands === 0) {
            return {
                totalCommands: 0,
        ...
        const commandActionNames = new Set(formStructure.commands.filter(c => c.action).map(c => c.action));
        for (const handler of allHandlers) {
            // Skip if it's already matched as a command handler
            if (commandActionNames.has(handler.name))
                continue;
     ...
     */
    validateConditionalAppearance(formStructure) {
        const issues = [];
    ...
    generateRecommendations(validation) {
        ...
        const formEventsWithoutHandlers = validation.formStructure.formEvents.filter(e => !e.handlerExists);
     ...
     */
    generateExtendedReport(validation) {
...
//# sourceMappingURL=form-extended-validator.js.map