...
export function hasMetadataXML(directoryPath) {
    ...
    if (fs.existsSync(commonModuleXml))
        return true;
    return false;
}
/**
 ...
 */
export function findMetadataXML(directoryPath) {
    ...
    if (path.basename(directoryPath) === 'Ext') {
        ...
        if (fs.existsSync(xmlFileInParent))
            return xmlFileInParent;
    }
 ...
 */
export async function enrichWithMetadata(directoryPath, analysisResult) {
 ...
 */
export async function analyzeFormEventHandlers(metadataResult) {
 ...
 */
export async function validateFormModules(metadataResult) {
    const validator = new FormExtendedValidator();
    const formValidationResults = new Map();
    if (metadataResult.relatedFiles.formModules) {
        for (const formModule of metadataResult.relatedFiles.formModules) {
            try {
 ...
 */
export function generateFormValidationPrompt(formValidationResults) {
 ...
 */
export function generateMetadataContextPrompt(metadataResult, formEventHandlers, formValidationResults) {
    ...
    if (meta.type === 'Catalog') {
        ...
        if (catalog.attributes && catalog.attributes.length > 0) {
            ...
            catalog.attributes.slice(0, 10).forEach((attr) => {
        ...
        if (doc.attributes && doc.attributes.length > 0) {
            ...
            doc.attributes.slice(0, 10).forEach((attr) => {
                const attrName = attr.synonym?.items[0]?.content || attr.name;
                prompt += `  - ${attrName} (${attr.name})\n`;
            });
        ...
        if (doc.tabularSections && doc.tabularSections.length > 0) {
            ...
            doc.tabularSections.forEach((ts) => {
        ...
        if (dp.attributes && dp.attributes.length > 0) {
            ...
            dp.attributes.slice(0, 10).forEach((attr) => {
                const attrName = attr.synonym?.items[0]?.content || attr.name;
                prompt += `  - ${attrName} (${attr.name})\n`;
            });
        }
    ...
    if (formEventHandlers && formEventHandlers.size > 0) {
        ...
        for (const [formName, analysis] of formEventHandlers.entries()) {
            const meta = metadataResult.metadata;
            const form = meta.forms?.find((f) => f.name === formName);
 ...
 */
export function getMetadataSummary(metadataResult) {
    const meta = metadataResult.metadata;
    ...
    if (meta.type === 'Catalog') {
        ...
        if (cm.global)
            summary += ` (глобальный)`;
        if (cm.server && cm.clientManagedApplication)
            summary += ` (сервер + клиент)`;
        else if (cm.server)
            summary += ` (серверный)`;
        else if (cm.clientManagedApplication)
            summary += ` (клиентский)`;
    }
    return summary;
}
//# sourceMappingURL=metadata-integration.js.map