...
import { getBSLCallGraphAnalyzer } from '../analyzer/bsl-call-graph-analyzer.js';
/**
...
export class DependencyGraphTool extends BaseTool {
    constructor(apiKey, model) {
        ...
        this.name = 'generate_dependency_graph';
        this.description = 'Generates Mermaid dependency graphs for BSL/1C code using hybrid analysis (static extraction + LLM formatting)';
     ...
     */
    findBslFilesRecursively(dirPath) {
        const bslFiles = [];
        try {
    ...
    extractDependencies(code, fileName) {
        ...
        while ((match = moduleCallRegex.exec(code)) !== null) {
            ...
            if (skipPrefixes.some(p => moduleName.toLowerCase() === p.toLowerCase()))
                continue;
        ...
        for (const [ref, count] of metadataRefs) {
            ...
            let type = 'external';
            if (prefix.includes('Справочник') || prefix.includes('Catalog'))
                type = 'catalog';
            else if (prefix.includes('Документ') || prefix.includes('Document'))
                type = 'document';
            else if (prefix.includes('Регистр') || prefix.includes('Register'))
                type = 'register';
            else if (prefix.includes('Обработк') || prefix.includes('DataProcessor'))
                type = 'processing';
            else if (prefix.includes('Отчет') || prefix.includes('Report'))
                type = 'report';
            else if (prefix.includes('Перечислен') || prefix.includes('Enum'))
                type = 'enum';
            else if (prefix.includes('Константа') || prefix.includes('Constant'))
                type = 'constant';
     ...
     */
    async generateMermaidWithLLM(analysis) {
...
\`\`\``;
        try {
            ...
            const response = await this.openRouterClient.generateWithCustomPrompt([analysisFile], systemPrompt);
     ...
     */
    async findBslFilesInDirectory(dirPath) {
        const bslFiles = [];
    ...
    fixMermaidNodeIds(mermaidCode) {
        ...
        const makeId = (label) => label.replace(/[^a-zA-Zа-яА-ЯёЁ0-9]/g, '_').replace(/_+/g, '_').replace(/^_|_$/g, '');
        ...
        fixedCode = fixedCode.replace(/-->\s*([a-zA-Zа-яА-ЯёЁ0-9_]+)\["(?:call|reference|read|write)\s*\|([^|]+)\|"\]/g, '-->|$2| $1');
        // Also fix: --> NodeId["type"] to -->|type| NodeId (for simple types without methods)
        fixedCode = fixedCode.replace(/-->\s*([a-zA-Zа-яА-ЯёЁ0-9_]+)\["(call|reference|read|write)"\]/g, '-->|$2| $1');
        return fixedCode;
    }
    /**
     * Generate simple Mermaid diagram without LLM (fallback)
     */
    generateSimpleMermaid(analysis) {
     ...
     */
    async generate(directoryPath, analysisResult, isTopLevel, childrenContent, outputDir) {
        ...
        try {
            ...
            let bslFiles = analysisResult.analyzedFiles.filter(f => f.path.toLowerCase().endsWith('.bsl'));
...
|-----|-----|---------------|-------------------|--------|
${mergedAnalysis.dependencies.map(d => `| ${d.type} | ${d.name} | ${d.usageCount} | ${d.usageType} | ${d.methods && d.methods.length > 0 ? d.methods.slice(0, 5).join(', ') + (d.methods.length > 5 ? '...' : '') : '-'} |`).join('\n')}
${callGraphSection}
---
...
//# sourceMappingURL=dependency-graph-tool.js.map