...
import { codeReviewPrompts, updateExistingContentPrompt, bslCodeReviewPrompts, isBSLContent, getModuleTypeRules } from '../prompt-config.js';
/**
...
export class ReviewTool extends BaseTool {
    ...
    async generate(directoryPath, analysisResult, isTopLevel = false, childrenContent, outputDir) {
        ...
        try {
            // Check if we're dealing with BSL/1C code
            const hasBSLFiles = analysisResult.analyzedFiles.some(file => isBSLContent(file.path, file.content));
            // Determine the appropriate system prompt based on context and content type
            let systemPrompt;
            if (hasBSLFiles) {
                // Use specialized BSL prompts for 1C code
                console.error('[BSL Review] Detected BSL/1C code, using specialized review prompts');
                if (isTopLevel) {
                    systemPrompt = bslCodeReviewPrompts.topLevelPrompt;
                }
                else if (childrenContent && childrenContent.length > 0) {
                    systemPrompt = bslCodeReviewPrompts.withChildrenPrompt;
                }
                else {
                    systemPrompt = bslCodeReviewPrompts.systemPrompt;
                }
                // Add module-type specific rules if applicable
                const firstBSLFile = analysisResult.analyzedFiles.find(file => isBSLContent(file.path, file.content));
                if (firstBSLFile) {
                    const moduleRules = getModuleTypeRules(firstBSLFile.path);
                    if (moduleRules) {
                        systemPrompt += '\n\n' + moduleRules;
                    }
                }
                ...
                // Use standard prompts for other languages
                if (isTopLevel) {
