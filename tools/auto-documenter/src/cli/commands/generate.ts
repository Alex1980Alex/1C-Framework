...
// Phase 2 modules
import { ResponseCache, createCache } from '../../cache/response-cache.js';
import { ChangeTracker, runIncremental } from '../../incremental/change-tracker.js';
 ...
 */
async function executeGenerate(targetPath: string, options: CLIOptions, output: Output): Promise<void> {
  ...
  if (mergedOptions.cache) output.info('Cache: enabled');
  if (mergedOptions.incremental) output.info('Mode: incremental');
  if (mergedOptions.watch) output.info('Mode: watch');
  output.newline();
  ...
  const generateDocs = async (filesToProcess?: string[]): Promise<void> => {
