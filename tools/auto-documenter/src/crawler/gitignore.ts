...
import * as ignoreImport from 'ignore';

// Workaround for TypeScript module resolution with ignore package
const ignore = (ignoreImport as any).default || ignoreImport;
  ...
  public loadRules(): boolean {
    try {
      ...
      if (fs.existsSync(gitignorePath)) {
        ...
        this.loaded = true;
        console.error('GitIgnore rules loaded successfully');
        return true;
      } else {
        console.error('No .gitignore file found, no ignore rules will be applied');
        this.loaded = false;
        return false;
      }
    } catch (error) {
