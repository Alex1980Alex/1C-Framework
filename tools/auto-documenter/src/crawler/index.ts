...
export class DirectoryCrawler {
  ...
  /**
   * Creates a bottom-up processing order grouped by depth level
   * Directories at the same level can be processed in parallel
   * @returns Array of arrays, where each inner array contains directories at the same depth level
   *          Ordered from deepest (index 0) to shallowest (last index = root)
   */
  public async createBottomUpOrderByLevel(): Promise<string[][]> {
    const dirTree = await this.scan();
    const levelMap = new Map<number, string[]>();

    // Helper function to collect directories with their depth
    const collectByLevel = (dir: Directory, depth: number) => {
      // Add this directory to its level
      if (!levelMap.has(depth)) {
        levelMap.set(depth, []);
      }
      levelMap.get(depth)!.push(dir.path);

      // Process all subdirectories at depth + 1
      for (const subdir of dir.subdirectories) {
        collectByLevel(subdir, depth + 1);
      }
    };

    // Collect all directories with their levels (root is depth 0)
    collectByLevel(dirTree, 0);

    // Find the maximum depth
    const maxDepth = Math.max(...levelMap.keys());

    // Build result array from deepest to shallowest (bottom-up)
    const result: string[][] = [];
    for (let depth = maxDepth; depth >= 0; depth--) {
      const dirsAtLevel = levelMap.get(depth);
      if (dirsAtLevel && dirsAtLevel.length > 0) {
        result.push(dirsAtLevel);
      }
    }
