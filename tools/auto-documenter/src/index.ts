 ...
 */
function is1CConfiguration(dirPath: string): boolean {
  try {
 ...
 */
function calculate1COutputDir(inputPath: string): string {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      if (!isValidAutoToolArgs(request.params.arguments)) {
        throw new McpError(
          ...
          `Invalid arguments for ${request.params.name} tool. Expected: path (string), outputDir? (string), openRouterApiKey? (string), model? (string), updateExisting? (boolean)`
        );
      }
