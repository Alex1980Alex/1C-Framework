...
export type EventHandlerType =
  | 'FormEvent'           // Form-level events (ПриСозданииНаСервере, ПриОткрытии, etc.)
  | 'ControlEvent'        // Control events (field/button events)
  | 'CommandHandler'      // Command handlers (Подключаемый_*)
  | 'NotificationHandler' // Notification handlers (ОбработкаОповещения)
...
export type EventContext =
  ...
  | 'Unknown';            // No directive specified
...
export class EventHandlerDetector {
  ...
  async analyzeFormModule(bslFilePath: string): Promise<IEventHandlerAnalysis> {
    ...
    let currentContext: EventContext = 'Unknown';
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (procedureMatch) {
        ...
        const handler = this.detectHandlerType(
          ...
          isExported,
          lineNumber,
          currentComment.length > 0 ? currentComment.join('\n') : undefined
        );
  ...
  private detectHandlerType(
    ...
    const baseHandler: IEventHandler = {
      name,
      type: 'Unknown',
      context,
      parameters,
   ...
   */
  private categorizeHandlers(handlers: IEventHandler[]): IEventHandlerAnalysis {
    const result: IEventHandlerAnalysis = {
  ...
  generateSummary(analysis: IEventHandlerAnalysis): string {
    ...
    if (analysis.handlersByType.controlEvents.length > 0) {
      ...
      const byControl = new Map<string, IEventHandler[]>();
   ...
   */
  private contextToRussian(context: EventContext): string {
    switch (context) {
