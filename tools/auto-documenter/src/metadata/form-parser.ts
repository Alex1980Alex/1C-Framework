...
export class FormParser {
  ...
  async parseFormXML(xmlFilePath: string): Promise<IFormStructure> {
    ...
    const rootControls: IFormControl[] = [];
   ...
   */
  private parseFormEvents(formRoot: any): IFormEvent[] {
    const events: IFormEvent[] = [];
   ...
   */
  private parseFormAttributes(formRoot: any): IFormAttribute[] {
    const attributes: IFormAttribute[] = [];
  ...
  private parseChildItems(
    childItemsNode: any,
    allControls: IFormControl[],
    parentArray: IFormControl[],
    parentName?: string
  ): void {
   ...
   */
  private parseControlEvents(controlNode: any): IFormEvent[] {
    const events: IFormEvent[] = [];
   ...
   */
  private parseTitle(node: any): ILocalizedTitle[] | undefined {
   ...
   */
  private extractTypeFromNode(typeNode: any): string | undefined {
  ...
  generateSummary(formStructure: IFormStructure): string {
    ...
    const controlsByType = new Map<FormControlType, IFormControl[]>();
   ...
   */
  private buildControlPrompt(control: IFormControl, level: number): string {
   ...
   */
  private parseFormCommands(formRoot: any): IFormCommand[] {
    const commands: IFormCommand[] = [];
   ...
   */
  private parseToolTip(node: any): ILocalizedTitle[] | undefined {
    ...
    if (toolTipNode['v8:item']) {
      const toolTips: ILocalizedTitle[] = [];
   ...
   */
  private parseConditionalAppearance(formRoot: any): IConditionalAppearanceItem[] {
    const items: IConditionalAppearanceItem[] = [];
   ...
   */
  private parseAppearanceProperties(itemNode: any): any {
    const props: any = {};
   ...
   */
  private parseAppearanceConditions(itemNode: any): any[] {
