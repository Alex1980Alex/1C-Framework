...
export type FormControlType =
  | 'InputField'           // Поле ввода (текст, число, дата, справочник)
  | 'Button'               // Кнопка
  | 'Label'                // Надпись (декорация)
...
export interface IFormValidationResult {
  ...
  /** Coverage metrics */
  coverage: {
    controlCoverage: number;    // % controls with handlers
    eventCoverage: number;      // % events with handlers
  };
