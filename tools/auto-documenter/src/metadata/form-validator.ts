...
import { EventHandlerDetector, IEventHandlerAnalysis, IEventHandler } from './event-handler-detector.js';
import {
  IFormStructure,
...
export class FormValidator {
  ...
  private handlerDetector: EventHandlerDetector;
  ...
  private performValidation(
    ...
    handlerAnalysis: IEventHandlerAnalysis
  ): IFormValidationResult {
    const errors: string[] = [];
    const warnings: string[] = [];

    // Build handler lookup map (handler name → IEventHandler)
    const handlersMap = new Map<string, IEventHandler>();
    ...
    // Find missing handlers (referenced in Form.xml but not in Module.bsl)
    const missingHandlers: IMissingHandler[] = [];
    ...
    for (const event of formStructure.allEvents) {
      ...
      if (!handlerExists) {
        ...
        errors.push(
          `Missing handler: ${event.handlerName}() referenced by ${controlName} (${event.eventType})`
        );
      }
    }
    ...
    // Find orphaned handlers (exist in Module.bsl but not referenced in Form.xml)
    const orphanedHandlers: IOrphanedHandler[] = [];
    ...
    for (const handler of [
      ...
      if (!referencedHandlers.has(handler.name)) {
        ...
        warnings.push(
          `Orphaned handler: ${handler.name}() at line ${handler.lineNumber} not referenced in Form.xml`
        );
      }
    }

    // Find unused controls (controls with no events)
    const unusedControls: IFormControl[] = [];
    for (const control of formStructure.controls) {
    ...
    // Calculate coverage metrics
    const totalControls = formStructure.controls.filter(
      c => c.type !== 'UsualGroup' && c.type !== 'Label' && c.type !== 'LabelDecoration'
    ).length;

    const controlsWithHandlers = formStructure.controls.filter(
      c => c.events.length > 0
    ).length;
