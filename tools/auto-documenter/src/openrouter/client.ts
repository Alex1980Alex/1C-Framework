...
import { ProviderRotationManager, createDefaultRotationManager } from '../providers/provider-rotation.js';
 ...
 */
async function checkRotationHttpAvailable(url: string): Promise<boolean> {
  try {
...
export class OpenRouterClient {
  ...
  private useLlmRotationHttp: boolean = false;
  private llmRotationHttpUrl: string = 'http://localhost:8000';
   ...
   */
  async initializeClient(apiKey?: string, model?: string): Promise<void> {
    if (this.useRotation) {
      ...
      if (rotationAvailable) {
        ...
        if (apiKey) {
          ...
          this.rotationManager.setApiKey(provider as any, apiKey);
        }
        if (model) {
          const provider = process.env.PRIMARY_PROVIDER || 'gemini';
          this.rotationManager.setModel(provider as any, model);
        }
      ...
      if (!this.config.openRouter.apiKey) {
        throw new Error(
          'OpenRouter API key is required. Set OPENROUTER_API_KEY environment variable or pass it to the constructor. ' +
   ...
   */
  private isProviderUnavailableError(error: any): boolean {
  ...
  async generateWithCustomPrompt(
    ...
    try {
      ...
      if (this.useLlmRotationHttp && (completion as any).x_rotation_info) {
        const info = (completion as any).x_rotation_info;
