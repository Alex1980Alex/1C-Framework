...
import { DiffResult, DocumentationChange, ChangeType, DiffSummary } from './diff-tool.js';
 ...
 */
export class MarkdownFormatter implements DiffFormatter {
  format(result: DiffResult): string {
    const lines: string[] = [];
  ...
  private formatChange(change: DocumentationChange): string {
  ...
  private getSeverityBadge(severity: 'info' | 'warning' | 'breaking'): string {
    switch (severity) {
      case 'breaking':
        return '`BREAKING`';
      case 'warning':
  ...
  ): Record<string, DocumentationChange[]> {
    const grouped: Record<string, DocumentationChange[]> = {};
 ...
 */
export class ConsoleFormatter implements DiffFormatter {
  private colors = {
    reset: '\x1b[0m',
    bright: '\x1b[1m',
    dim: '\x1b[2m',
  ...
  format(result: DiffResult): string {
    ...
    lines.push('');
    lines.push(
      `${this.colors.bright}${this.colors.cyan}📊 Documentation Diff Report${this.colors.reset}`
    );
    ...
    lines.push('');
    lines.push(
      `  ${this.colors.green}+${result.summary.linesAdded} lines${this.colors.reset}  ` +
    ...
    if (result.summary.breakingChanges > 0) {
      lines.push('');
      lines.push(
        `${this.colors.yellow}⚠️  ${result.summary.breakingChanges} breaking change(s) detected!${this.colors.reset}`
      );
    }
  ...
  private getChangeColor(type: ChangeType): string {
    switch (type) {
      case ChangeType.ADDED:
 ...
 */
export class GitHubFormatter implements DiffFormatter {
  format(result: DiffResult): string {
    const lines: string[] = [];
    ...
    if (result.summary.breakingChanges > 0) {
      lines.push(
        `::warning::Documentation contains ${result.summary.breakingChanges} breaking change(s)`
      );
      lines.push('');
    }
    ...
    for (const change of result.changes) {
      if (change.severity === 'breaking') {
        ...
        lines.push(
          `::warning file=${file},line=${line}::Breaking change detected in documentation`
        );
      }
    }
 ...
 */
export function getFormatter(format: OutputFormat): DiffFormatter {
  switch (format) {
    case 'markdown':
