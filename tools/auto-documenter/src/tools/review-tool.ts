...
export class ReviewTool extends BaseTool<ReviewToolConfig> {
  ...
  public async generate(
    ...
    try {
      ...
      const hasBSLFiles = analysisResult.analyzedFiles.some(
        file => isBSLContent(file.path, file.content)
      );
      ...
      if (hasBSLFiles) {
        ...
        const firstBSLFile = analysisResult.analyzedFiles.find(
          file => isBSLContent(file.path, file.content)
        );
        if (firstBSLFile) {
