...
export class MCTS002AlphaStrategy extends MonteCarloTreeSearchStrategy {
    ...
    calculatePolicyScore(node, parent) {
    ...
    estimateValue(node) {
        ...
        return (0.5 * immediateValue +
            0.3 * depthPotential +
            0.2 * noveltyValue);
    }
    calculateNovelty(node) {
    ...
    thoughtCoherence(thought1, thought2) {
    ...
    async runPolicyGuidedSearch(node) {
    ...
    async simulateWithValueGuidance(node) {
        let current = node;
        let totalReward = 0;
        let depth = 0;
    ...
    async backpropagateWithPolicyUpdate(node, reward) {
        let current = node;
        while (current) {
            current.visits++;
            ...
            if (current.parentId) {
                const parentNode = await this.getNode(current.parentId);
    ...
    async updatePolicyMetrics(node, parent) {
    ...
    calculatePolicyEnhancedScore(path) {
        if (path.length === 0)
            return 0;
