...
export class InputValidator {
    ...
    validateThought(thought) {
        ...
        if (thought.length > this.config.maxThoughtLength) {
            throw ErrorFactory.invalidThought(thought, `exceeds maximum length of ${this.config.maxThoughtLength} characters`);
        }
     ...
     */
    validateThoughtNumber(thoughtNumber) {
        ...
        if (thoughtNumber < this.config.minThoughtNumber) {
            throw ErrorFactory.invalidInput('thoughtNumber', thoughtNumber, `>= ${this.config.minThoughtNumber}`);
        }
    }
    /**
     * Validate total thoughts
     */
    validateTotalThoughts(totalThoughts) {
        ...
        if (totalThoughts > this.config.maxTotalThoughts) {
            throw ErrorFactory.invalidInput('totalThoughts', totalThoughts, `<= ${this.config.maxTotalThoughts}`);
        }
    }
    /**
     * Validate nextThoughtNeeded
     */
    validateNextThoughtNeeded(nextThoughtNeeded) {
    ...
    validateBeamWidth(beamWidth) {
        ...
        if (beamWidth > this.config.maxBeamWidth) {
            throw ErrorFactory.invalidInput('beamWidth', beamWidth, `<= ${this.config.maxBeamWidth}`);
        }
    }
    /**
     * Validate number of simulations
     */
    validateNumSimulations(numSimulations) {
        ...
        if (numSimulations > this.config.maxSimulations) {
            throw ErrorFactory.invalidInput('numSimulations', numSimulations, `<= ${this.config.maxSimulations}`);
        }
    }
    /**
     * Check for potentially malicious content
     */
    checkForMaliciousContent(thought) {
